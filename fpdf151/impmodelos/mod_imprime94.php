...
for ($ii = 0; $ii < $this->linhasdositens; $ii++) {
	...
	if ($iVoltaImp == 0) {
		...
		$qtdItem = $qtdItem == '' ? 0 : $qtdItem;

		$oMatOrdemItem = db_utils::getDao('matordemitem');
		$numItem = pg_result($this->recorddositens, $ii, 'e62_numemp');
		$seqItem = pg_result($this->recorddositens, $ii, 'e62_sequen');
		$rsOrdemItem   = $oMatOrdemItem->sql_record($oMatOrdemItem->sql_query_emiteordem('', 'matunidaut.m61_abrev as abrev',
			'm52_numemp = ' . $numItem .' and m52_sequen = '.$seqItem));

		$abrev = db_utils::fieldsMemory($rsOrdemItem, 0)->abrev;
