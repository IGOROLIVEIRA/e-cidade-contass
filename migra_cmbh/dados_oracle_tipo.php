...
foreach ($aDadosAgrupadosOracle as $aDados) {
	...
	$rsResult = pg_query($conexao_postgre, "select * from pctipo WHERE pc05_codtipo = {$aDados['COD_CLAMAT']}");
	
	if (pg_num_rows($rsResult) > 0) {
		
		pg_query($conexao_postgre, "UPDATE pctipo SET pc05_descr = '{$aDados['DESC_CLAMAT']}', 
		pc05_ativo = 't' WHERE pc05_codtipo = {$aDados['COD_CLAMAT']}");
		
	} else {
	
	  $sSql_insert = "INSERT INTO pctipo (pc05_codtipo,pc05_descr,pc05_ativo) VALUES ({$aDados['COD_CLAMAT']},
