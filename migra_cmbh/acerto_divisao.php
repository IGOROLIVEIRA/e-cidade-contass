...
while (OCIFetch($sql_parse)) {
  ...
  $dados_oracle = array();
	...
	$dados_oracle['DATA_AQUISICAO']    = date('Y-m-d', strtotime(OCIResult($sql_parse,"DATA_AQUISICAO")));
 ...
 $dados_oracle['COD_SALA_ATUAL']       = OCIResult($sql_parse,"COD_SALA_ATUAL");
 $dados_oracle['COD_CC_ATUAL']       = OCIResult($sql_parse,"COD_CC_ATUAL");
...
//if ({$dados['COD_DIVISAO'] == "111127" || {$dados['COD_DIVISAO'] == "11119" || {$dados['COD_DIVISAO'] == "121114") {;
echo "{$dados['COD_BEM']} / {$dados['COD_DIVISAO']} /  {$dados['COD_SALA_ATUAL']} /  {$dados['COD_CC_ATUAL']} <br>";
	$i++;
	//echo "<pre>";
	//print_r($dados);exit;
//}
