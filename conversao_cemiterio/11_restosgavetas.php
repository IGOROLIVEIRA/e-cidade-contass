...
pg_query($dbportal,"drop sequence restosgavetas_cm26_i_codigo_seq");
pg_query($dbportal,"create sequence restosgavetas_cm26_i_codigo_seq start 1");
pg_query($dbportal,"truncate restosgavetas");
pg_query($dbportal,"create table tmp_restosgavetascod(resto_i_codigo int, restosgavetas int)");
//pg_query($dbportal,"begin;");

$arq = "txt/cem_restosgavetas_erro.txt";
system( "clear" );
system("> $arq");
...
for($x=0;$x<$rows;$x++){
 ...
 $sepultamento = pg_result($query2,0,0);
  ...
  $restosgavetas = pg_result(pg_query($dbportal,"select nextval('restosgavetas_cm26_i_codigo_seq')"),0,0);
  ...
  $tmp_restosgavetas = pg_query($dbportal,"insert into tmp_restosgavetascod values($array[resto_i_codigo],$restosgavetas)");
  
  $insert = "INSERT INTO restosgavetas(cm26_i_codigo,
         			       cm26_i_sepultamento,
                                       cm26_i_ossoariojazigo,
                                       cm26_d_entrada)
                               VALUES ($restosgavetas,
                                       $sepultamento,
                                       $ossoario,";
