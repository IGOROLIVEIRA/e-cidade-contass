...
if(!($conn = @pg_connect("host=$DB_SERVIDOR dbname=alegreteteste port=$DB_PORTA user=$DB_USUARIO password=$DB_SENHA"))) {
  echo "Não conectou!"; 
  exit;
}
...
function php_verifica_mae($fonte,$proces=null){
  sleep(0.5);
    ...
    //rotina que verifica se a fonte possui conta mãe
    ...
    if($numrows>0){
      ...
      php_verifica_mae($mae,$proces);
    }

    //rotina que pea dados da tabela cadconplano para incluir nas outras
    $sql = " select * from cadconplano where o56_elemento ='$proces'" ;
    $result = @pg_query($sql);
    $numrows = @pg_numrows($result);

    if($numrows==0 && $proces==null){
      echo "Fonte $fonte não existe na tabela cadconplano";
    }else{
       $result01 = @pg_query("select nextval('conplano_c60_codcon_seq') as codcon"); 
       $codcon   =  pg_result($result01,0,"codcon");
       
       $fonte    =  pg_result($result,0,"o56_elemento")."00";
       $descr    =  pg_result($result,0,"o56_descr");
       $finali   =  pg_result($result,0,"o56_finali");
       $codsis   = '1';
       $codcla   =  '1';
...
$result = pg_query("select * from cadconplano ");
$numrows = pg_num_rows($result);
for($i=0; $i<$numrows; $i++){
  $fonte   =  pg_result($result,$i,"o56_elemento");
