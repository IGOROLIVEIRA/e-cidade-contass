...
db_inicio_transacao();
//REGULARIZA O ESTOQUE
db_query("CREATE TEMP TABLE temp_estoque AS (
  SELECT m70_codigo,
  m70_codmatmater,
  m70_coddepto,
  ...
  FROM
  (SELECT coalesce(round(m82_quant, 2),0) AS quantidade,
  round(m89_precomedio, 4) AS preco_medio,
  round(m89_valorunitario, 4) AS valorunitario,
  m81_tipo,
  m80_codigo,
  m80_codtipo
  FROM matestoqueini
  ...
  WHERE m82_matestoqueitem IN
  (SELECT m71_codlanc
  FROM
  (SELECT DISTINCT dpartestoque.coddepto AS codigo_departamento,
  dpartestoque.descrdepto AS descricao_departamento,
  m71_codmatestoque,
  m71_codlanc,
  m71_quant,
  m60_codmater,
  m60_descr,
  dpartini.coddepto AS codigo_almoxarifado,
  m77_lote,
  dpartini.descrdepto AS descrisao_almoxarifado,
  m71_valor,
  m77_dtvalidade,
  m76_nome AS fabricante,
  m71_data AS data_movimeto
  FROM matestoqueinimei
  INNER JOIN matestoqueini ON matestoqueini.m80_codigo = matestoqueinimei.m82_matestoqueini
  INNER JOIN matestoquetipo ON m80_codtipo = m81_codtipo
  INNER JOIN matestoqueinimeipm ON m82_codigo = m89_matestoqueinimei
  INNER JOIN db_usuarios ON m80_login = id_usuario
  INNER JOIN db_depart AS dpartini ON m80_coddepto = dpartini. coddepto
  INNER JOIN matestoqueitem ON m82_matestoqueitem = m71_codlanc
  INNER JOIN matestoque ON m71_codmatestoque = m70_codigo
  INNER JOIN db_depart AS dpartestoque ON m70_coddepto = dpartestoque.coddepto
  INNER JOIN matmater ON m60_codmater = m70_codmatmater
  LEFT JOIN matestoqueitemlote ON m77_matestoqueitem = m71_codlanc
  LEFT JOIN matestoqueitemfabric ON m78_matestoqueitem = m71_codlanc
  LEFT JOIN matfabricante ON m76_sequencial = m78_matfabricante
  WHERE m70_coddepto IN (m.m70_coddepto)
  AND m70_codmatmater IN (m.m70_codmatmater)
  AND dpartini.instit IN (1)
  AND m60_ativo = 't'
  AND m71_servico IS FALSE
  ORDER BY m60_descr) AS x)
  AND m81_tipo IN (1)
  ORDER BY m80_data,
  m80_hora,
  m80_codigo) AS estoque) -
  (SELECT sum(quantidade) AS atendido
  FROM
  (SELECT coalesce(round(m82_quant, 2),0) AS quantidade,
  round(m89_precomedio, 4) AS preco_medio,
  round(m89_valorunitario, 4) AS valorunitario,
  ...
  ORDER BY m80_data,
  m80_hora,
  m80_codigo) AS atendido) AS saldo) AS estoque,

  (SELECT m89_precomedio
  FROM matestoqueinimeipm
  ...
  WHERE m70_coddepto IN (m.m70_coddepto)
  AND m70_codmatmater IN (m.m70_codmatmater)
  ORDER BY m89_sequencial DESC
  LIMIT 1) AS preco_medio
  from matestoque as m where m70_codmatmater in (SELECT m70_codmatmater
  FROM matestoque
  JOIN inventariomaterial ON m70_codigo=i77_estoque
  WHERE i77_inventario = {$oParam->iInventario})
);

UPDATE temp_estoque
SET preco_medio = round(round(preco_medio,2)*estoque,2);
...
UPDATE matestoque
SET m70_quant=
(SELECT estoque::double precision
 FROM temp_estoque
 WHERE matestoque.m70_codmatmater = temp_estoque.m70_codmatmater
 AND matestoque.m70_coddepto = temp_estoque.m70_coddepto),
 m70_valor=
 (SELECT preco_medio::double precision
 FROM temp_estoque
 WHERE matestoque.m70_codmatmater = temp_estoque.m70_codmatmater
 AND matestoque.m70_coddepto = temp_estoque.m70_coddepto)
 WHERE m70_codmatmater in (SELECT m70_codmatmater
 FROM matestoque
 JOIN inventariomaterial ON m70_codigo=i77_estoque
 WHERE i77_inventario = {$oParam->iInventario}
);
...
if($aResult->i77_contagem > $aResult->i77_estoqueinicial){
  ...
  if($clmatestoqueitem->erro_status==0){
    ...
    //echo 'MATESTOQUEITEM'.$clmatestoqueitem->erro_msg;
  }
  ...
  if($clmatestoqueinimei->erro_status==0){
   throw new ParameterException($clmatestoqueinimei->erro_msg);
    ...
    //echo 'clmatestoqueinimei'.$clmatestoqueinimei->erro_msg;
 }
  //
