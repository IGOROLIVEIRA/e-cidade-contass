...
if (isset($incluir)) {
    ...
    if (strtotime($dtDataUsu) <= strtotime($c99_data)) {
        ...
        $erro_msg   .=  " ERRO: DATA INV�LIDA. LIMITE: {$c99_data} \\n\\n";
        $erro_status = "0";
    ...
    if (pg_num_rows(db_query($sSqlLicitacao))) {
        ...
        if (strtotime(db_utils::fieldsMemory(db_query($sSqlLicitacao), 0)->e54_emiss) > db_getsession('DB_datausu')) {
            ...
            db_msgbox("N�o � permitido emitir empenhos de licita��es cuja data da autoriza��o (" . date("d/m/Y", strtotime(db_utils::fieldsMemory(db_query($sSqlLicitacao), 0)->e54_emiss)) . ") seja maior que a data de emiss�o do empenho (" . date("d/m/Y", db_getsession('DB_datausu')) . ").");
    ...
    if ($sqlerro == false) {
        ...
        */
        ...
        if ($veConvMSC > 0) {
            ...
            if (!$rsResult) {
                $sqlerro  = true;
                $erro_msg = "Inclus�o Abortada!\n � obrigat�rio informar o conv�nio para os empenhos de fontes 122, 123, 124 e 142.\n";
            }
        }
        ...
        if ($sqlerro == false) {
            ...
            /**
             * Valida��o solicitada pela OC 8359
             *
             */
            ...
            if (in_array(substr($o56_elemento, 0, -6), $elementosemp)) {
                if ($clempempenho->e60_datasentenca == null) {
                    if (substr($o56_elemento, 0, -6) == '3319091') {
                        $erro_msg = "Usu�rio: Para este elemento � obrigat�rio informar a data da senten�a judicial";
                        db_msgbox($erro_msg);
                        $sqlerro = true;
                    }
                }
            ...
            if (pg_num_rows(db_query($sSqlDataAutEmp))) {
                if (strtotime(db_utils::fieldsMemory(db_query($sSqlDataAutEmp), 0)->e54_emiss) > db_getsession('DB_datausu')) {
                    $sqlerro = true;
                    $erro_msg = "N�o � permitido emitir empenho cuja data da autoriza��o (" . date("d/m/Y", strtotime(db_utils::fieldsMemory(db_query($sSqlDataAutEmp), 0)->e54_emiss)) . ") seja maior que a data de emiss�o do empenho (" . date("d/m/Y", db_getsession('DB_datausu')) . ").";
                }
            }
