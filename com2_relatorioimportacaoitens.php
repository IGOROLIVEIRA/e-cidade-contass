...
for ($i = 0; $i < pg_numrows($itens); $i++) {
    ...
    if (strlen($item->pc01_descrmater) > 58) {
        $pdf->cell(70, 6, substr($item->pc01_descrmater, 0, 58) . "...", 1, 0, "L", 0);
    } else {
        $pdf->cell(70, 6, substr($item->pc01_descrmater, 0, 58), 1, 0, "L", 0);
    }
    //$pdf->cell(70, 6, substr($item->pc01_descrmater, 0, 58) . "...", 1, 0, "L", 0);
    if (strlen($item->pc01_complmater) > 58) {
        $pdf->cell(70, 6, substr($item->pc01_complmater, 0, 58) . "...", 1, 0, "L", 0);
    } else {
        $pdf->cell(70, 6, substr($item->pc01_complmater, 0, 58), 1, 0, "L", 0);
    }
    //$pdf->cell(70, 6, $item->pc01_complmater,     1, 0, "C", 0);
