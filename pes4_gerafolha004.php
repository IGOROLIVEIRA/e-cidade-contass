 ...
 *
 * @Version: $Id: pes4_gerafolha004.php,v 1.403 2015/05/25 18:41:33 dbrenan.silva Exp $
 */
...
for($Ipessoal=0;$Ipessoal<count($pessoal);$Ipessoal++){
   ...
   if( db_at(db_str($situacao_funcionario,1),"2-8") > 0 || !db_empty($pessoal[$Ipessoal]["r01_recis"]) || ( trim($db21_codcli)=="parobe"
...
function gerffer($opcao_geral=null,$opcao_tipo=1){
  ...
  for ($Ipessoal=0; $Ipessoal<count($pessoal); $Ipessoal++) {
    ...
    if( db_at(db_str($situacao_funcionario,1),"2-8") > 0){
    ...
    if( $situacao_funcionario == 6 || $situacao_funcionario == 8 ){ // Afastado Doenca + 15 Dias
      if( $achou_tabela){
    ...
    if( strtolower(trim($db21_codcli)) == "999999999"){
      $bbru = 0;
      $birf = 0;
      $bfgt = 0;
      $bpre = 0;
...
LogCalculoFolha::write("SQL: Calculo Geral Diferente de Salário");
  ...
  for ($Ipessoal=0; $Ipessoal<count($pessoal); $Ipessoal++) {
        ...
        for ($Itransacao=0; $Itransacao< count($transacao); $Itransacao++) {
          ...
          if ($dias_pagamento > 0) {
            ...
            if (round(( $transacao[$Itransacao]["r53_valor"] - ($valor_salario_fam)
              -(strtolower($db21_codcli)=="999999999"?0:$valor_ferias_ )),2) <= 0 ) {
              if ($transacao[$Itransacao]["rh27_pd"] == 2) {
              ...
              $valor = ( $transacao[$Itransacao]["r53_valor"] - ($valor_salario_fam)
                - (strtolower($db21_codcli)=="999999999"? 0:$valor_ferias_ ));
              ...
              if (!db_empty($transacao[$Itransacao]["r53_quant"]) && ('t' == $transacao[$Itransacao]["rh27_propq"])) {
                // F019 - Numero de dias a pagar no mes
                if ($F019 > 0 &&  'f' == $cadferia[0]["r30_paga13"] ) {
                  $quantidade = ( $qtd_salario/30) * ( $dividir_por - $dias_maternidade - (strtolower($db21_codcli)=="999999999"? 0: $F019 ) );
                } else {
    ...
    if (($situacao_funcionario == 6  && !db_empty($rubrica_licenca_saude ) ) // Afastado Doenca + 15 Dias
      ||($situacao_funcionario == 8  && !db_empty($rubrica_licenca_saude ) ) // Afastado Doenca + 30 Dias
      ...
      if (!db_empty($vlr_sal_saude_ou_acidente)) {
        ...
        $matriz2[1] = $r110_regist;
        $matriz2[2] = ($situacao_funcionario == 6 || $situacao_funcionario == 8?$rubrica_licenca_saude:$rubrica_acidente);
        $matriz2[3] = $pessoal[$Ipessoal]["r01_lotac"];
...
// Inicio do Calculo das rubricas do desconto do IRRF e(ou) do desconto da Previdencia
  ...
  if ($pessoal[$Ipessoal]["r01_tbprev"] == 0 || ( $db21_codcli == "999999999"
    ...
    $rsSalario = db_query("
      select sum(r14_valor) as valor
       ...
       where r14_rubric in (
         ...
         and r14_regist in (select rh01_regist from rhpessoal where rh01_numcgm = $iNumeroCGM);    
    ");
    $rsComplementar = db_query("
      select sum(r48_valor) as valor
       ...
       where r48_rubric in (
         ...
         and r48_regist in (select rh01_regist from rhpessoal where rh01_numcgm = $iNumeroCGM);    
    ");
...
function le_tbprev($r20_rubr=null, $area=null, $sigla=null, $sigla2=null, $nro_do_registro=null, $operacao=null) {
  ...
  for ($n=1; $n < 4; $n++) {
    ...
    if ($n==1) {                                         // <-- R985 - BASE PREVIDENCIA
      if (( ($situacao_funcionario == 6 ) && !db_empty($rubrica_licenca_saude ) ) || ( ($situacao_funcionario == 8 ) && !db_empty($rubrica_licenca_saude ) )) {
...
function gravb_base_irf ($area_grava,$r20_rubrp){
  ...
  if( $r20_rubr  == "R981" || $r20_rubr  == "R982" || $r20_rubr  == "R983"){
    ...
    // R983 BASE IRF FERIAS
     ...
     if($r20_rubr == "R981"){
       if( $situacao_funcionario == 6 || $situacao_funcionario == 8 ){
...
function grava_gerf($area_grava,$grava_tpp=" ") {
  ...
  if ($r14_valor > 0.01 || ($oRubrica->getTipo() == Rubrica::TIPO_BASE  && $r14_valor == 0 ) ) {
  ...
  if ( $r20_rubr == "R913" || $r20_rubr == "R915" ) {
    ...
    if ($opcao_geral == 1) {
      ...
      if (db_selectmax("transacao", "select * from gerfs13 ".bb_condicaosubpes("r35_" ).$condicaoaux )) {
        $base_irf  -= $transacao[0]["r35_valor"]  ;
        $r07_valor -= $transacao[0]["r35_valor"]  ;
        //echo "<br> Valor da Base IRF depois de R980 ADIANTAMENTO PENSAO (13.SALARIO) --> $r07_valor -= ".$transacao[$Itra]["r35_valor"];
      }
    }
  }
...
function grava_rubricas_ferias_especiais($r30_proc,$r30_peri,$r30_perf)
{
  ...
  if (DBPessoal::verificarUtilizacaoEstruturaSuplementar() && ($oFolhaAtual->getTipoFolha() == FolhaPagamento::TIPO_FOLHA_SALARIO || $oFolhaAtual->getTipoFolha() == FolhaPagamento::TIPO_FOLHA_COMPLEMENTAR)) {
