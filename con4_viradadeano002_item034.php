...
if($sqlerro==false) {
    ...
    //vinculopcaspmsc elemdespmsc natdespmsc
    //De/Para Vinculo Pcasp MSC
    $sqldeporigem = "select * from vinculopcaspmsc where c210_anousu = $anoorigem limit 1";
    $resultdeporigem = db_query($sqldeporigem);
    $linhasdeporigem = pg_num_rows($resultdeporigem);

    $sqldepdestino = "select * from vinculopcaspmsc where c210_anousu = $anodestino limit 1";
    ...
    if (($linhasdeporigem > 0) && ($linhasdepdestino == 0 )) {
        ...
        $sqldepartorg = "select fc_duplica_exercicio('vinculopcaspmsc', 'c210_anousu', ".$anoorigem.",".$anodestino.",null);";
    ...
    $sqldeporigem = "select * from elemdespmsc where c211_anousu = $anoorigem limit 1";
    $resultdeporigem = db_query($sqldeporigem);
    $linhasdeporigem = pg_num_rows($resultdeporigem);

    $sqldepdestino = "select * from elemdespmsc where c211_anousu = $anodestino limit 1";
    $resultdepdestino = db_query($sqldepdestino);
    $linhasdepdestino = pg_num_rows($resultdepdestino);

    if (($linhasdeporigem > 0) && ($linhasdepdestino == 0 )) {

        $sqldepartorg = "select fc_duplica_exercicio('elemdespmsc', 'c211_anousu', ".$anoorigem.",".$anodestino.",null);";
        $resultdepartorg = db_query($sqldepartorg);
        if ($resultdepartorg==true) {
            $sqlerro = false;
        } else {
    ...
    $sqldeporigem = "select * from natdespmsc where c212_anousu = $anoorigem limit 1";
    $resultdeporigem = db_query($sqldeporigem);
    $linhasdeporigem = pg_num_rows($resultdeporigem);

    $sqldepdestino = "select * from natdespmsc where c212_anousu = $anodestino limit 1";
    $resultdepdestino = db_query($sqldepdestino);
    $linhasdepdestino = pg_num_rows($resultdepdestino);

    if (($linhasdeporigem > 0) && ($linhasdepdestino == 0 )) {

        $sqldepartorg = "select fc_duplica_exercicio('natdespmsc', 'c212_anousu', ".$anoorigem.",".$anodestino.",null);";
        $resultdepartorg = db_query($sqldepartorg);
        if ($resultdepartorg==true) {
            $sqlerro = false;
        } else {
