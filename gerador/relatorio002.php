...
if($limite==0||$limite=="" ){
  ...
  $resultsql = pg_exec(str_replace('\\','','select arrehist.* from arrehist inner join arrepaga on arrepaga.k00_numpre = arrehist.k00_numpre where k00_histtxt like \\\\\\\'%TP\\08%\\\\\\\' '.@$limit));
  if($resultsql==false){
    echo "Verifique os dados a serem gerados.<br>";
    echo "select arrehist.* from arrehist inner join arrepaga on arrepaga.k00_numpre = arrehist.k00_numpre where k00_histtxt like \\\\\\\'%TP\\08%\\\\\\\' ";
  }
  $head1 = $nome;
  $head2 = "";
  $head3 = "";
