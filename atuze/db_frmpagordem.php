...
?>
    ...
    <td nowrap title="<?=@$Te50_numemp?>">
       <?=db_ancora($Le50_numemp,"js_JanelaAutomatica('empempenho','".@$e50_numemp."')",$db_opcao_ancora)?>        
    </td>
    <td> 
<?
...
<table cellpadding='0' cellspacing='0' border='0'>  
  ...
  <tr>
	...
	<?
	db_textarea('e50_obs',6,70,$Ie50_obs,true,'text',$db_opcao_desab,"")
	?>
	<?
...
function js_pesquisa_cgm(mostra){
  if(mostra==true){
    js_OpenJanelaIframe('top.corpo.iframe_pagordem','db_iframe_cgm','func_nome.php?funcao_js=parent.js_mostracgm1|e60_numcgm|z01_nome','Pesquisa',true,0);
  }else{
  ...
  //atualiza os valores  
     ...
     for(w=0; w<obj.length; w++){
	  ...
	  if(campo.checked==true){
 	         ...
 	         if(soma_total > vlrdis.toFixed(2)  ){
     ...
     if(campo.checked==true){
        ...
        document.form1.vlrpag.value =  soma_total + antigo;
     }else{
...
function js_pesquisa_emp(){
  js_OpenJanelaIframe('top.corpo.iframe_pagordem','db_iframe_empempenho','func_empempenho.php?funcao_js=parent.js_preenchepesquisa|e60_numemp','Pesquisa',true,'0','1','775','390');
}
function js_pesquisa_ordem(){
 <? if(isset($procedimento) && $procedimento=="anulacao"){?>//variavel procedimento, é para indicar quando esta anulando, dae abre uma func que tras só as ordens com saldo
     js_OpenJanelaIframe('top.corpo.iframe_pagordem','db_iframe_pagordem','func_pagordem_anula.php?funcao_js=parent.js_preenchepesquisa|e50_codord','Pesquisa',true,'0','1','775','390');
 <? }else{?>
