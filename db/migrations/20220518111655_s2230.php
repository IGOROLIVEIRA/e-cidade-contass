        ...
        FROM
            (SELECT cgm.z01_cgccpf AS cpftrab,
             ...
             UNION SELECT cgm.z01_cgccpf AS cpftrab,
                          ...
                          CASE
                              WHEN (cadferia.r30_peraf - cadferia.r30_perai)+1 < 365 THEN cadferia.r30_peraf
                              WHEN (cadferia.r30_peraf - cadferia.r30_perai)+1 > 365 THEN cadferia.r30_peraf
                              ELSE NULL
                          END AS dtfim,
                          r30_per1f AS dttermafast2
             FROM cadferia
             ...
             WHERE date_part(''month'',cadferia.r30_per1i::date) = date_part(''month'',fc_getsession(''DB_datausu'')::date)
                 AND date_part(''year'',cadferia.r30_per1i::date) = fc_getsession(''DB_anousu'')::int) AS x";
        $this->execute($sql);
    }
}
