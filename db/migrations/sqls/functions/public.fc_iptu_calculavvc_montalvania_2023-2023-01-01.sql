...
CREATE OR REPLACE FUNCTION public.fc_iptu_calculavvc_montalvania_2023(integer, integer, numeric, boolean, boolean) RETURNS tp_iptu_calculavvc LANGUAGE 'plpgsql' COST 100 VOLATILE PARALLEL UNSAFE AS $BODY$
declare
...
for rConstr in execute tSql loop
		...
		select j34_area,
            ...
            j81_valorterreno, j81_valorconstr
      	from lote
...
values (iAnousu,iMatricula,rConstr.j39_idcons,rConstr.j39_area,nVm2c,rConstr.j31_pontos,nValorConstr);
...
select
into nValor
    round(fc_INFLA('URM',rConstr.j22_vm2,(iAnousu||'-01-01')::DATE,(iAnousu||'-01-01')::DATE),2) as valorm2 ;
...
where j11_matric = rConstr.j39_matric
  and j11_idcons = rConstr.j39_idcons and j10_anousu = iAnousu;
...
bAtualiza = false;
    ...
    perform j39_matric from iptuconstr where j39_matric = iMatricula and j39_dtdemo is null and not exists (select 1 from carconstr inner join iptubase on j01_matric = j48_matric where j48_caract = 36) limit 1;
