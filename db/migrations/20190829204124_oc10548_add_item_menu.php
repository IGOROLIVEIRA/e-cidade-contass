...
class Oc10548AddItemMenu extends PostgresMigration
{
    ...
    public function up()
    {
        ...
        INSERT INTO db_itensmenu VALUES ((select max(id_item)+1 from db_itensmenu),'Gerar Siope', 'Gerar Siope','',1,1,'Gerar Siope','true');

        INSERT INTO db_itensmenu VALUES ((select max(id_item)+1 from db_itensmenu),'Despesa', 'Despesa','con4_gerarsiope.php',1,1,'Despesa','true');

        INSERT INTO db_menu VALUES (32, (select max(id_item) from db_itensmenu)-1, (select max(menusequencia) from db_menu where id_item = 32 and modulo = 2000025)+1, 2000025);
