...
class Oc10813 extends PostgresMigration
{
    ...
    public function up()
    {
        ...
        INSERT INTO public.db_sysarquivo (codarq, nomearq, descricao, sigla, dataincl, rotulo, tipotabela, naolibclass, naolibfunc, naolibprog, naolibform) VALUES ((select max(codarq)+1 from db_sysarquivo), 'prevconvenioreceita', 'Previsão Receita Convênio', 'c229 ', '2019-11-05', 'Previsão Receita Convênio', 0, false, false, false, false);
         
        -- INSERINDO db_sysarqmod
        INSERT INTO public.db_sysarqmod (codmod, codarq) VALUES (32, (select max(codarq) from db_sysarquivo));
        ...
        INSERT INTO public.db_sysarqcamp (codarq, codcam, seqarq, codsequencia) VALUES ((select max(codarq) from db_sysarquivo), (select codcam from db_syscampo where nomecam = 'c229_fonte'), 1, 0);
        INSERT INTO public.db_sysarqcamp (codarq, codcam, seqarq, codsequencia) VALUES ((select max(codarq) from db_sysarquivo), (select codcam from db_syscampo where nomecam = 'c229_convenio'), 2, 0);
        INSERT INTO public.db_sysarqcamp (codarq, codcam, seqarq, codsequencia) VALUES ((select max(codarq) from db_sysarquivo), (select codcam from db_syscampo where nomecam = 'c229_vlprevisto'), 3, 0);
        INSERT INTO public.db_sysarqcamp (codarq, codcam, seqarq, codsequencia) VALUES ((select max(codarq) from db_sysarquivo), (select codcam from db_syscampo where nomecam = 'c229_anousu'), 4, 0);
         
        -- INSERINDO db_sysforkey
        INSERT INTO public.db_sysforkey (codarq, codcam, sequen, referen, tipoobjrel) VALUES ((select max(codarq) from db_sysarquivo), (select codcam from db_syscampo where nomecam = 'c229_fonte'), 1, 780, 0);
