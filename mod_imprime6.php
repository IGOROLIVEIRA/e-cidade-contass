...
for ($xxx = 0; $xxx < $this->nvias; $xxx ++) {
  ...
  $this->objpdf->text($xcol +2, $xlin +35.5, 'Prazo de Entrega');
  ...
  //Dado Prazo de entrega
  $this->objpdf->text($xcol +26, $xlin +35.5, ':  '.$this->prazo_ent);
  ...
  $this->objpdf->rect($xcol +15, $xlin +60, 30, 100, 2, 'DF', '');
  ...
  $this->objpdf->rect($xcol +15, $xlin +54, 30, 6, 2, 'DF', '12');
  ...
  $this->objpdf->text($xcol +20, $xlin +58, 'REFERÊNCIA');
  ...
  for ($ii = 0; $ii < $this->linhasdositens; $ii ++) {
    $this->objpdf->SetWidths(array(15, 30, 95, 25, 25));
    ...
    $descricaoitemimprime = $this->objpdf->Row_multicell(array($quantitem,
    															pg_result($this->recorddositens, $ii, $this->referencia), 
                                                                 $descricaoitem."\n".$obsitem, 
	                                                             $valoritemuni, 
	                                                             $valoritemtot
	                                                             ), 
    ...
    ($this->objpdf->gety() > $this->objpdf->h - 30 && $pagina != 1)) {
      ...
      $this->objpdf->rect($xcol +15, $xlin +60, 30, 262, 2, 'DF', '');
