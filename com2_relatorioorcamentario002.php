...
$pdf->ln($alt + 4);
$pdf->cell(270,4,"SOLICITAÇÃO DE PARECER DE DISPONIBILIDADE FINANCEIRA",0,1,"C",0);
$pdf->ln($alt + 4);
$pdf->SetFont('arial','',11);
$pdf->cell(270,4,"De: Pregoeira/ Comissão permanente de Licitação"  ,0,1,"L",0);
$pdf->cell(270,4,"Para: Setor contábil"                             ,0,1,"L",0);
$pdf->ln($alt + 4);
//$pdf->MultiCell(270,3,"     Solicito do departamento contábil se há disponibilidade financeira que atenda {$objeto}, no valor total estimado de R$ ".trim(db_formatar($vlrtotal,'f')),0,"L",0);
$pdf->MultiCell(270,4,"Solicito ao departamento contábil se há no orçamento vigente, disponibilidade financeira que atenda {$objeto}, no valor total estimado de R$".trim(db_formatar($vlrtotal,'f')),0,"J",0);
$pdf->ln($alt + 4);
$data = db_getsession('DB_datausu');
$sDataExtenso     = db_dataextenso($data);
$pdf->cell(270,4,$munic.','.strtoupper($sDataExtenso)                      ,0,1,"C",0);
$pdf->ln($alt+6);
$pdf->cell(270,4,"________________________"                                ,0,1,"C",0);
$pdf->cell(270,4,"Presidente da CPL"                                       ,0,1,"C",0);
$pdf->ln($alt+3);
$pdf->cell(270,4,"E/OU Presidente da Comissão de Licitação"                ,0,1,"C",0);
