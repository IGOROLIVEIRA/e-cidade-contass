...
function dbViewCadastroDocumento() {
  ...
  */
  ...
  this.retorno_getFormDocumento = function (oAjax) {
    ...
    if(oRetorno.aAtributos[0].db45_descricao != 'CNH'){
      var campos = [];
      oRetorno.aAtributos.each(function(item){
        if(!campos.includes(item))
          campos.push(item);
      });

      me.getCampos().each(function(item){
        obj = new Object();
        obj.db45_sequencial = item.db46_sequencial;
        obj.db45_descricao  = item.db46_descricao;
        obj.db45_tipo       = item.db46_tipo;
        obj.db45_tamanho    = item.db46_tamanho;
        obj.db45_valordefault = item.db46_valordefault;
        obj.referencia      = null;
        campos.push(obj);
      });
  ...
  this.createFormDocument = function(aAtributos){
    var listaAtributos = [];
    listaAtributos = aAtributos;

    descricao = aAtributos[0].db45_descricao;
    if(descricao != 'CNH'){
      me.carregaCampos(aAtributos[0].db45_sequencial);
    }
    ...
    // console.log('LIst..',listaAtributos);
    // // if(listaAtributos.length == 1)
    // //   listaAtributos = [];

    // console.log('LIst 2..',listaAtributos);

    // if(listaAtributos.length > 1)
    listaAtributos.each(function(oAtributo) {
      console.log('atr..',oAtributo);
      var input         = "";
