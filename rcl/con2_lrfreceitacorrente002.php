    ...
    if (in_array($estrutural,$param[$p])){       
      ...
      if ($p <=15){
        ...
        if (substr($estrutural,0,3) == "497"){
          if (!isset($Trec[1][1]))  $Trec[1][1]  = abs($janeiro);    else { if ($janeiro   > 0) { $Trec[1][1]  -= $janeiro;   } else { $Trec[1][1]  += abs($janeiro);   } }; 
          if (!isset($Trec[1][2]))  $Trec[1][2]  = abs($fevereiro);  else { if ($fevereiro > 0) { $Trec[1][2]  -= $fevereiro; } else { $Trec[1][2]  += abs($fevereiro); } }; 
          if (!isset($Trec[1][3]))  $Trec[1][3]  = abs($marco);      else { if ($marco     > 0) { $Trec[1][3]  -= $marco;     } else { $Trec[1][3]  += abs($marco);     } }; 
          if (!isset($Trec[1][4]))  $Trec[1][4]  = abs($abril);      else { if ($abril     > 0) { $Trec[1][4]  -= $abril;     } else { $Trec[1][4]  += abs($abril);     } }; 
          if (!isset($Trec[1][5]))  $Trec[1][5]  = abs($maio);       else { if ($maio      > 0) { $Trec[1][5]  -= $maio;      } else { $Trec[1][5]  += abs($maio);      } }; 
          if (!isset($Trec[1][6]))  $Trec[1][6]  = abs($junho);      else { if ($junho     > 0) { $Trec[1][6]  -= $junho;     } else { $Trec[1][6]  += abs($junho);     } }; 
          if (!isset($Trec[1][7]))  $Trec[1][7]  = abs($julho);      else { if ($julho     > 0) { $Trec[1][7]  -= $julho;     } else { $Trec[1][7]  += abs($julho);     } }; 
          if (!isset($Trec[1][8]))  $Trec[1][8]  = abs($agosto);     else { if ($agosto    > 0) { $Trec[1][8]  -= $agosto;    } else { $Trec[1][8]  += abs($agosto);    } }; 
          if (!isset($Trec[1][9]))  $Trec[1][9]  = abs($setembro);   else { if ($setembro  > 0) { $Trec[1][9]  -= $setembro;  } else { $Trec[1][9]  += abs($setembro);  } }; 
          if (!isset($Trec[1][10])) $Trec[1][10] = abs($outubro);    else { if ($outubro   > 0) { $Trec[1][10] -= $outubro;   } else { $Trec[1][10] += abs($outubro);   } }; 
          if (!isset($Trec[1][11])) $Trec[1][11] = abs($novembro);   else { if ($novembro  > 0) { $Trec[1][11] -= $novembro;  } else { $Trec[1][11] += abs($novembro);  } }; 
          if (!isset($Trec[1][12])) $Trec[1][12] = abs($dezembro);   else { if ($dezembro  > 0) { $Trec[1][12] -= $dezembro;  } else { $Trec[1][12] += abs($dezembro);  } }; 
        }else{
    ...
    if (in_array($estrutural,$param[$p])) {
      ...
      */       
      if ($p <=15) {
        ...
        // Trec da linha 1 contem o total da dedução da receita corrente
        if (substr($estrutural,0,3) == "497"){
          if (!isset($TrecB[1][1]))  $TrecB[1][1]= abs($janeiro);    else $TrecB[1][1] += abs($janeiro);
          if (!isset($TrecB[1][2]))  $TrecB[1][2]= abs($fevereiro);  else $TrecB[1][2] += abs($fevereiro);
          if (!isset($TrecB[1][3]))  $TrecB[1][3]= abs($marco);      else $TrecB[1][3] += abs($marco);
          if (!isset($TrecB[1][4]))  $TrecB[1][4]= abs($abril);      else $TrecB[1][4] += abs($abril);
          if (!isset($TrecB[1][5]))  $TrecB[1][5]= abs($maio);       else $TrecB[1][5] += abs($maio);
          if (!isset($TrecB[1][6]))  $TrecB[1][6]= abs($junho);      else $TrecB[1][6] += abs($junho);
          if (!isset($TrecB[1][7]))  $TrecB[1][7]= abs($julho);      else $TrecB[1][7] += abs($julho);
          if (!isset($TrecB[1][8]))  $TrecB[1][8]= abs($agosto);     else $TrecB[1][8] += abs($agosto);
          if (!isset($TrecB[1][9]))  $TrecB[1][9]= abs($setembro);   else $TrecB[1][9] += abs($setembro);
          if (!isset($TrecB[1][10])) $TrecB[1][10]= abs($outubro);   else $TrecB[1][10]+= abs($outubro);
          if (!isset($TrecB[1][11])) $TrecB[1][11]= abs($novembro);  else $TrecB[1][11]+= abs($novembro);
          if (!isset($TrecB[1][12])) $TrecB[1][12]= abs($dezembro);  else $TrecB[1][12]+= abs($dezembro);	  
          if (!isset($TrecB[1][13])) $TrecB[1][13]= abs($o70_valor); else $TrecB[1][13]+= abs($o70_valor);
        }else{
