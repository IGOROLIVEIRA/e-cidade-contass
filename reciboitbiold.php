...
if(pg_numrows($result) == 0) {
  db_msgbox('Recibo nao gerado. Entre em contato com a prefeitura');
      exit;
  //redireciona();
}
...
$fc_febraban = pg_result($resultcod,0);
...
//   $pdf->cell(110,$altura,'Logradouro : '.$j14_nome,1,1,"L",0);
   ...
   $pdf->SetFont('Arial','',8);
   $pdf->cell(13,$altura,$matricula,1,0,"L",0);

   $pdf->SetFont('Arial','B',8);
   $pdf->cell(30,$altura,'Numero do imovel: ',1,0,"L",1);
   $pdf->SetFont('Arial','',8);
   ...
   $pdf->SetFont('Arial','',8);
   $pdf->cell(15,$altura,$j34_lote,1,1,"L",0);
   ...
   $pdf->SetFont('Arial','',8);
   $pdf->cell(21,$altura,db_formatar($mfrente,'f') . 'm',1,0,"R",0);
   $pdf->SetFont('Arial','B',8);
   $pdf->cell(22,$altura,'Fundos : ',1,0,"L",1);
   $pdf->SetFont('Arial','',8);
   $pdf->cell(23,$altura,db_formatar($mfundos,'f').'m',1,1,"R",0);
   $pdf->SetFont('Arial','B',8);
   $pdf->cell(22,$altura,'Lado Esquerdo: ',1,0,"L",1);
   $pdf->SetFont('Arial','',8);
   $pdf->cell(21,$altura,db_formatar($mladoesquerdo,'f').'m',1,0,"R",0);
   $pdf->SetFont('Arial','B',8);
   $pdf->cell(22,$altura,'Lado Direito: ',1,0,"L",1);
   $pdf->SetFont('Arial','',8);
   $pdf->cell(23,$altura,db_formatar($mladodireito,'f').'m',1,1,"R",0);
   $pdf->SetFont('Arial','B',8);
...
//   $pdf->cell(33,$altura,db_formatar($areaedificada+0,'f').'m2',1,1,"R",0);

   $pdf->cell(33,$altura,($xarea == 0?'':db_formatar($xarea,'f',' ',' ',' ',5).'m2'),1,1,"R",0);

   $pdf->SetXY(100,$y);

   $pdf->SetFont('Arial','B',8);
   $pdf->cell(30,$altura,'Descrição',1,0,"C",1);
   $pdf->cell(25,$altura,'Descrição',1,0,"C",1);
   ...
   for ($ii = 1;$ii <= 10 ; $ii++){
       ...
       $pdf->cell(25,$altura,'',1,0,"L",0);
       $pdf->cell(22,$altura,'',1,0,"L",0);
   ...
   for ($num = 0;$num < pg_numrows($result2) ; $num++){
       ...
       $pdf->cell(25,$altura,$descricao,0,0,"L",0);
       $pdf->cell(22,$altura,db_formatar($area,'f',' ',' ',' ',5),0,0,"R",0);
   ...
   $pdf->sety($yy);   
   $pdf->cell(60,$altura,'Valor Terreno : '.($valoravterr==0?'':'R$').@db_formatar($valoravterr,'f'),1,0,"L",0);
   $pdf->cell(60,$altura,'Valor Prédio : '.($valoravconst==0?'':'R$').@db_formatar($valoravconst,'f'),1,0,"L",0);
   $pdf->cell(60,$altura,'Valor Avaliação : '.($valoravaliacao==0?'':'R$').db_formatar($valoravaliacao,'f'),1,1,"L",0);
   $pdf->cell(60,$altura,'Valor Informado : '.($valortransacao==0?'':'R$').db_formatar($valortransacao,'f'),1,0,"L",0);
   $pdf->cell(60,$altura,'Alíquota : '.db_formatar($aliquota,'f').'%',1,0,"L",0);
   $pdf->SetFont('Arial','B',10);
   $pdf->cell(60,$altura,'Valor a Pagar : R$ '.db_formatar(($valorpagamento + $tx_banc),'f'),1,1,"L",0);
   $pdf->setfont('Arial','B',11); 
   $pdf->ln(3);
//   $pdf->multicell(180,4,$munic.', '.date('d').' de '.db_mes(date('m')).' de '.date('Y').'.',0,"R",0);
   $pdf->multicell(180,4,$munic.', '.substr($datasolicitacao,8,2).' de '.db_mes(substr($datasolicitacao,5,2)).' de '.substr($datasolicitacao,0,4).'.',0,"R",0);
   $pdf->Ln(4);
