...
<form name='frmAnularEmpenho' id='frmAnularEmpenho' action="" method="POST">
    ...
    <input name="confirmar" type="button" id="confirmar" value="Confirmar" onclick="return js_verificacordo(<?=$iTipoResto?>)" disabled>
...
<script>
    ...
    function js_verificacordo(iTipo){
            ...
            for (i = 0; i < aItens.length; i++) {
                ...
                with (aItens[i]) {
                    ...
                    if (checked) {
                        ...
                        sJsonItens    += sVirg+"{'e62_sequencial':" + value + ",'vlrtot':" + nValorItem + ",'quantidade':"+nQtdItem+"}";
                        sVirg = ',';
                    }
                }
    ...
    function js_saidaverificacordo(oResponse){
        var oRetorno = eval("(" + oResponse.responseText + ")");

        if(oRetorno.status == 2){
            mensagem = oRetorno.mensagem.replace(/\+/g," ");
            mensagem = unescape(mensagem);
            alert(mensagem);
        }else if(oRetorno.status == 3){
            mensagem = oRetorno.mensagem.urlDecode().replace(/\+/g," ")+' Deseja continuar ?';
            mensagem = unescape(mensagem);
            if(confirm(mensagem)){
                js_estornaRP(1);
            }
