...
//    x.rh05_recis_dia.select();
    ...
    alert("Informe o motivo da rescis�o.");
...
function js_mostrarhpessoal(chave,chave2,chave3,chave4,chave5,chave6,chave7,chave8,chave9,chave10,chave11,erro,temresci){
  ...
  if(erro==true||temresci=='s'){
    ...
    if(per1f >= ultdi || per2f >= ultdi){
      alert("Gozo de f�rias integrais neste ou no pr�ximo m�s.");
      location.href = "pes4_rhpesrescis001.php";
    }else if(chave4 == anmes || chave5 == anmes || per1f >= subps || per2f >= subps){
//      alert("Funcion�rio tem pagamento/gozo de f�rias no m�s");
    }
    if(chave9 != "" && chave10 != ""){
      alert("Se funcion�rio vinculado ao IPE, informe nova situa��o (Cadastro de IPE).");
    }
  }
