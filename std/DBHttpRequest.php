...
class DBHttpRequest
{
    ...
    public function getResponseCode()
    {
        if (!isset($this->responseHeaders['response_code'])) {
            throw new \Exception('Requisi��o não executada.');
        }
    ...
    private function __sendFileWrapper($url, $method = 'GET', array $options = array())
    {
        ...
        if ($result === false) {
            ...
            throw new BusinessException("Erro ao executar requisi��o:\nRetorno: " . $error['message']);
        }
        // print_r($result);
        return $result;
    }
    ...
    public function parseXmlToObject($url)
    {
        try {
            ...
            if (strpos($e->getMessage(), 'String could not be parsed as XML') !== false &&
                ...
                throw new Exception("Erro api :\n Não foi posssível analisar a cadeia de caracteres como XML - Retorno API: {$this->body} - Corrija o cpf dos dependentes.");
            }
            if (strpos($e->getMessage(), 'String could not be parsed as XML') !== false &&
                strpos($this->body, '[dmdev] NULL encontrado') !== false) {
                throw new Exception("Erro api :\n Não foi posssível analisar a cadeia de caracteres como XML - Retorno API: {$this->body} - Não existe contra cheque para este servidor na competencia enviada.");
            }
            if (strpos($e->getMessage(), 'String could not be parsed as XML') !== false &&
            ...
            if (strpos($e->getMessage(), 'String could not be parsed as XML') !== false &&
                ...
                throw new Exception("Erro api :\n Não foi posssível analisar a cadeia de caracteres como XML - Retorno API: {$this->body} - Tempo limete para retono esgotado. Verifique no site do e-social se o evento foi enviado ou envie novamete esta matricula.");
            }
            if (strpos($e->getMessage(), 'String could not be parsed as XML') !== false) {
                throw new Exception("Erro api :\n Não foi posssível analisar a cadeia de caracteres como XML - Retorno API: {$this->body}");
            }
