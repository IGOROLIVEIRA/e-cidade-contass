...
if ($k60_tipo != 'M'){
  $sql = "select $xxcodigo as notifica,$xxcodigo1 as $xcodigo1, $xcampos, sum(k22_vlrcor+k22_juros+k22_multa-k22_desconto) as xvalor
  from lista
  inner join listanotifica on k63_codigo = k60_codigo
  inner join debitos on k22_data = '$k60_datadeb' and k22_numpre = k63_numpre
  $xxmatric  and $xxcodigo = k63_notifica $xxmatric2 $limite $contr
  where k60_codigo = $lista and k22_dtvenc < '" . date("Y-m-d", db_getsession("DB_datausu")) . "'
  group by $xxcodigo,
  $xxcodigo1,
  z01_numcgm,
  z01_nome
  ...
  sum(k22_vlrcor+k22_juros+k22_multa-k22_desconto) as xvalor
  from lista
  inner join listanotifica on k63_codigo = k60_codigo
  inner join debitos on k22_data = '$k60_datadeb' and k22_numpre = k63_numpre
  $xxmatric  and $xxcodigo = k63_notifica $xxmatric2 $limite $contr
...
if ( $tiporel == 1 or $tiporel == 11 ) {
  ...
  for($x=$lim1;$x < $lim2;$x++) {
    ...
    where k53_notifica = $notifica";
    $resultaproced = pg_exec($sqlproced);
    $virgula = '';
    global $procedencias;
		$procedencias = '';
    ...
    $cgm = $z01_numcgm;
    $sql1 = "select k22_tipo,k00_descr,sum(k22_vlrcor+k22_juros+k22_multa-k22_desconto) as valor from debitos  inner join arretipo on k00_tipo = k22_tipo where $xmatinsc $jtipos k22_data = '$k60_datadeb' and k22_dtvenc < '" .  date("Y-m-d",db_getsession("DB_datausu")) . "' group by k22_tipo,k00_descr";
    $result1 = pg_exec($sql1);
    
    if ($k60_tipo == 'M'){
      
      $sqlpropri = "select z01_nome, codpri, nomepri, j39_numero, j39_compl, j34_setor, j34_quadra, j34_lote, j34_zona, j34_area, j01_tipoimp, j13_descr from proprietario where j01_matric = $matric";
    ...
    for ($doc = 0; $doc < pg_numrows($resultdocparagr); $doc++) {
      ...
      if (strtoupper($db02_descr) == "TOTALPORANO" and $somenteparc == false) {
        ...
        $pdf->cell(35,05,"VLR ORIGINAL"  ,1,0,"C",1);
        ...
        for ($totano = 0; $totano < pg_numrows($resultanos); $totano++) {
          ...
          $pdf->cell(35,05,trim(db_formatar($k22_vlrhis,'f')),1,0,"R",0);
        ...
        $pdf->cell(35,05,trim(db_formatar($totvlrhis,'f')),1,0,"R",1);
        ...
        where k53_notifica = $notifica
        group by case when k22_exerc is null then extract (year from k22_dtoper) else k22_exerc end";
        $resultcompara1 = pg_exec($sqlanos) or die($sqlanos);
        // TOTALGERALPORANO
        ...
        group by case when k22_exerc is null then extract (year from k22_dtoper) else k22_exerc end";
        $resultcompara2 = pg_exec($sqlanostipos) or die($sqlanostipos);
        ...
        //$sql = "select fc_recibo($k03_numpre,'" . $db_datausu . "'::date,(current_date + '1 months'::interval)::date,".db_getsession("DB_anousu").")";
        $sql = "select fc_recibo($k03_numpre,'$db_datausu'::date,'$db_datausu'::date,".db_getsession("DB_anousu").")";
        ...
        $sqlinstit = "select nomeinst,ender,numero,munic,email,telef,uf,logo,to_char(tx_banc,'9.99') as tx_banc,numbanco from db_config where codigo = ".db_getsession("DB_instit");
        ...
        if ($k60_tipo == 'M'){
          ...
          $sqlIdentificacao = "select z01_numcgm, z01_nome,z01_ender,z01_numero,z01_compl,z01_munic,z01_uf,z01_cep,nomepri,j39_compl,j39_numero,j13_descr,j34_setor||'.'||j34_quadra||'.'||j34_lote as sql,z01_cgccpf
          from proprietario
        ...
        $sqlvalor = "select k00_tercdigrecnormal, k00_msgrecibo from arretipo 
        ...
        if (1 == 1) {
          ...
          //// RECIBO
          if (strpos($db02_texto, "imprimirdesconto") >= 0) {
            
            $pdf->ln();
            $pdf->SetFont('Arial','B',12);
           // $pdf->cell(190,05,"SEU DÉBITO COM DESCONTO DE R$ " . trim(db_formatar($valordesconto, 'f')) . " FICARÁ EM R$ " . trim(db_formatar($tottotal - $valordesconto, 'f')),1,0,"R",0);
          ...
          for ($i = 0;$i < 2;$i++) {
            ...
            if ($i == 0) {
              ...
              $pdf->SetY($xlin+26);
              $pdf->SetX($xcol+3);
              $pdf->multicell(115,4,'HISTÓRICO :   '.$pdf->historico);
              $pdf->SetX($xcol+3);
        ...
        $pdf->cell(30,05,"PARCELAMENTO",	 1,0,"C",1);
        ...
        $pdf->cell(30,05,$v07_parcel,	                      1,0,"C",0);
        $pdf->cell(45,05,trim(db_formatar($k22_vlrcor,'f')) ,1,1,"R",0);
        $pdf->cell(45,05,trim(db_formatar($k22_juros,'f')) ,1,1,"R",0);
        $pdf->cell(45,05,trim(db_formatar($k22_multa,'f')) ,1,1,"R",0);
        ...
        sum(k22_juros) as k22_juros, 
        sum(k22_multa) as k22_multa, 
        sum(k22_total) as k22_total from (
        select	v07_parcel,
        k22_numpar,
        sum(k22_vlrcor) as k22_vlrcor, 
        sum(k22_juros) as k22_juros, 
        ...
        k22_data = '$k60_datadeb'
        where k22_dtvenc < '$k60_datadeb'
        group by k22_numpar, v07_parcel) as x";
        ...
        */
      ...
      } elseif (strtoupper($db02_descr) == "VERSO") {
        ...
        for ($xxx=0; $xxx < sizeof($opcoesverso); $xxx++) {
          ...
          if ($dadosopcoes[0] == "parte1") {
            ...
            $pdf->Roundedrect(5,98,200,95,2,'DF','1234');
            ...
            for ($contamsg = 0; $contamsg < sizeof($opcoesverso); $contamsg++) {
              ...
              if ($msgparte2[0] == "msgparte2") {
                ...
                break;
						...
						$pdf->Image('imagens/files/correios.jpg',173,100,30);
            ...
            $pdf->SetFont('Arial','',12);
            $pdf->ln(0);
            $pdf->cell(40,5,"REMETENTE: ",0,0,"L",0);
            $pdf->SetFont('Arial','B',12);
            $pdf->cell(100,5,$nomeinst,0,1,"L",0);
            $pdf->SetFont('Arial','',12);
            $pdf->cell(40,5,"ENDEREÇO: ",0,0,"L",0);
            $pdf->SetFont('Arial','B',12);
            $pdf->cell(50,5,strtoupper(trim($db_ender)),0,1,"L",0);
            
            $pdf->SetFont('Arial','',12);
            $pdf->cell(40,5,($bairro == ""?"":"BAIRRO: "),0,0,"L",0);
            $pdf->SetFont('Arial','B',12);
            $pdf->cell(20,5,$bairro,0,1,"L",0);
            
            $pdf->SetFont('Arial','',12);
            $pdf->cell(40,5,"MUNICIPIO:",0,0,"L",0);
            $pdf->SetFont('Arial','B',12);
            $pdf->cell(50,5,$munic ."/".$uf . " - " . substr($cep,0,5)."-".substr($cep,5,3),0,1,"L",0);
            ...
            $pdf->cell(50,5,"Motivos da não entrega",1,0,"C",0);
            $pdf->SetFont('Arial','B',12);
            $pdf->cell(95,5,"Para uso do carteiro",1,1,"C",0);
            ...
            $pdf->SetXY(57,265);
            $pdf->SetX(57);
            $pdf->cell(35,7,"Assinatura do entregador: ________________________________ ",0,1,"L",0);
      ...
      if ($tiporel == 11) {
        ...
        $pdf->AddPage();
        global $variavel;
        $variavel = "CENTRAL DE ATENDIMENTO";
        ...
        //   for($x=0;$x < 20;$x++){
          //   for($x=0;$x < pg_numrows($result);$x++){
            ...
            for($x=$lim1;$x < $lim2;$x++){
              ...
              $inscricao = $inscr;
              $cgm = $z01_numcgm;
              $sql10 = "select distinct k22_tipo,k00_descr from debitos inner join arretipo on k00_tipo = k22_tipo where $xmatinsc $jtipos k22_data = '$k60_datadeb' ";
