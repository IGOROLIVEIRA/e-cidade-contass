...
class cl_acordoposicaoaditamento
{
  ...
  function alterar($ac35_sequencial = null)
  {
    ...
    if (trim($this->ac35_veiculodivulgacao) != "" || isset($GLOBALS["HTTP_POST_VARS"]["ac35_veiculodivulgacao"])) {
      ...
      if (trim($this->ac35_veiculodivulgacao) == null) {
        $this->erro_sql = " Campo Veiculo divulgao nao Informado.";
  ...
  function excluir($ac35_sequencial = null, $dbwhere = null)
  {
    ...
    if ($result == false) {
      $this->erro_banco = str_replace("\n", "", @pg_last_error());
      $this->erro_sql   = "Saldos dos Aditamentos nao Excludo. Excluso Abortada.\\n";
      ...
      if (pg_affected_rows($result) == 0) {
        $this->erro_banco = "";
        $this->erro_sql = "Saldos dos Aditamentos nao Encontrado. Excluso no Efetuada.\\n";
        $this->erro_sql .= "Valores : " . $ac35_sequencial;
        $this->erro_msg   = "Usurio: \\n\\n " . $this->erro_sql . " \\n\\n";
        $this->erro_msg   .=  str_replace('"', "", str_replace("'", "",  "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
        $this->erro_status = "1";
