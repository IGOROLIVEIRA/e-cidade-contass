...
class cl_pcorcamjulg { 
  ...
  function sqlQueryPreviaJulgamento($tipoJulgamento,$codigoOrcamento){

    $sWhere = "";
    $sOrderBy = "";
    // Julgamento por lote
    if($tipoJulgamento == 3){
      $sWhere = " exists (select * from (select
      l04_descricao,pc23_orcamforne,SUM(pc23_valor) as valortotal,
        ROW_NUMBER() OVER(PARTITION by l04_descricao ORDER BY l04_descricao,SUM(pc23_valor),pc23_orcamforne ASC) 
        from  
          pcorcamitem 
      ...
      inner join liclicitemlote on
          l04_liclicitem = pc26_liclicitem
      where
          pc22_codorc = $codigoOrcamento
          GROUP BY l04_descricao,pc23_orcamforne
      order by
          l04_descricao,valortotal,pc23_orcamforne) x 
          where
          l04_descricao = liclicitemlote.l04_descricao 
          and pc23_orcamforne = pcorcamval.pc23_orcamforne and
        ROW_NUMBER = 1)";

        $sOrderBy = " l04_descricao,l21_ordem,pc23_orcamitem,pc23_orcamforne";
    }

    //Julgamento por item 
    if($tipoJulgamento == 1){
      $sWhere = " exists (select * from (select
          pc22_orcamitem,pc23_orcamforne,pc23_valor,
          ROW_NUMBER() OVER(PARTITION by pc22_orcamitem ORDER BY pc22_orcamitem,SUM(pc23_valor) ASC) as pontuacao
      from
          pcorcamitem
      inner join pcorcamval on
          pc23_orcamitem = pc22_orcamitem
      where
          pc22_codorc = $codigoOrcamento 
      group by 
          pc22_orcamitem,pc23_valor,pc23_orcamforne
      order by 
          pc22_orcamitem,pc23_valor) x
          where pontuacao = 1 and pc23_orcamforne = pcorcamval.pc23_orcamforne and pc22_orcamitem = pcorcamval.pc23_orcamitem) ";

          $sOrderBy = " l21_ordem,l04_descricao,pc23_orcamitem,pc23_orcamforne";
    }

    return "select
              l21_ordem,
              pc81_codprocitem,
              pcorcamval.pc23_orcamitem,
              pcorcamval.pc23_orcamforne,
              pc01_codmater,
              pc01_descrmater,
              pc23_obs,
              z01_numcgm,
              ...
              pc23_valor,
              pc11_resum,
              liclicitemlote.l04_descricao,
              l20_tipojulg,
              pc11_vlrun as valor_registro_preco,
              case
            ...
            inner join liclicitem on
              liclicitem.l21_codigo = liclicitemlote.l04_liclicitem
            inner join pcorcamitemlic on
              pcorcamitemlic.pc26_liclicitem = liclicitem.l21_codigo
            ...
            inner join pcorcamforne on
              pcorcamforne.pc21_orcamforne = pcorcamval.pc23_orcamforne
            inner join cgm on
              cgm.z01_numcgm = pcorcamforne.pc21_numcgm
            left join pcorcamdescla on
              pcorcamdescla.pc32_orcamitem = pcorcamval.pc23_orcamitem
              and pcorcamdescla.pc32_orcamforne = pcorcamval.pc23_orcamforne
            left join pcorcamjulg on
              pcorcamjulg.pc24_orcamitem = pcorcamitemlic.pc26_orcamitem
              and pcorcamjulg.pc24_orcamforne = pcorcamforne.pc21_orcamforne
            where $sWhere
            order by $sOrderBy";
  }

  function sqlPontuacaoPorLote($codigoOrcamento){

    return "select
            l04_descricao,pc23_orcamforne,SUM(pc23_valor) as valortotal,
              ROW_NUMBER() OVER(PARTITION by l04_descricao ORDER BY l04_descricao,SUM(pc23_valor),pc23_orcamforne ) AS pontuacao
              from
              pcorcamitem
            inner join pcorcamval on
              pc23_orcamitem = pc22_orcamitem
            ...
            where
              ...
              GROUP BY l04_descricao,pc23_orcamforne
            order by
              l04_descricao,valortotal,pc23_orcamforne;";

  }

  function sqlPontuacaoPorItem($codigoOrcamento){
    return "select
              pc22_orcamitem,pc23_orcamforne,pc23_valor,
                ROW_NUMBER() OVER(PARTITION by pc22_orcamitem ORDER BY pc22_orcamitem,SUM(pc23_valor) ASC) as pontuacao
            from
            ...
            group by 
              pc22_orcamitem,pc23_valor,pc23_orcamforne
            order by 
              pc22_orcamitem,pc23_valor;";
  }

  function sqlItensDeLoteEFornecedor($codigoOrcamento,$lote,$fornecedor){
    return "select l04_descricao,pc23_orcamforne,pc23_orcamitem from pcorcamval
            inner join pcorcamitem on pc23_orcamitem = pc22_orcamitem
            inner join pcorcamitemlic on
              pc26_orcamitem = pc23_orcamitem
              inner join liclicitemlote on
              l04_liclicitem = pc26_liclicitem
            where pc22_codorc = $codigoOrcamento and l04_descricao = '$lote' and pc23_orcamforne = $fornecedor
            order by l04_descricao,pc23_orcamforne;";
  }

  function julgamentoLicitacao($tipoJulgamento,$codigoOrcamento){

    // Julgamento por lote
    if($tipoJulgamento == 3){
      $rsPontuacaoPorLote = db_query($this->sqlPontuacaoPorLote($codigoOrcamento));
      for($i=0; $i < pg_num_rows($rsPontuacaoPorLote); $i++){

        $aPontuacao = db_utils::fieldsMemory($rsPontuacaoPorLote, $i);
        $lote = $aPontuacao->l04_descricao;
        $fornecedor = $aPontuacao->pc23_orcamforne;

        $rsItensDeLoteEFornecedor = db_query($this->sqlItensDeLoteEFornecedor($codigoOrcamento,$lote,$fornecedor));

        for($j=0; $j < pg_num_rows($rsItensDeLoteEFornecedor); $j++){
          $item = db_utils::fieldsMemory($rsItensDeLoteEFornecedor, $j);

          $this->pc24_orcamitem  = $item->pc23_orcamitem;
          $this->pc24_orcamforne = $item->pc23_orcamforne;
          $this->pc24_pontuacao  = $aPontuacao->pontuacao;
          $this->incluir($item->pc23_orcamitem, $item->pc23_orcamforne);
    ...
    $rsPontuacaoPorItem = db_query($this->sqlPontuacaoPorItem($codigoOrcamento));
    
    for($i=0; $i < pg_num_rows($rsPontuacaoPorItem); $i++){
      $item = db_utils::fieldsMemory($rsPontuacaoPorItem, $i);
      $this->pc24_orcamitem  = $item->pc22_orcamitem;
      $this->pc24_orcamforne = $item->pc23_orcamforne;
      $this->pc24_pontuacao  = $item->pontuacao;
      $this->incluir($item->pc22_orcamitem, $item->pc23_orcamforne);
      if($this->erro_status == "0"){
        return false;
      } 
    }
