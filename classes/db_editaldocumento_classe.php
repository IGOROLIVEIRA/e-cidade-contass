...
//       $resac = db_query("insert into db_acountkey values($acount,18488,'$this->l48_sequencial','I')");
//       $resac = db_query("insert into db_acount values($acount,3267,18488,'','".AddSlashes(pg_result($resaco,0,'l48_sequencial'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
//       $resac = db_query("insert into db_acount values($acount,3267,18489,'','".AddSlashes(pg_result($resaco,0,'l48_edital'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
//       $resac = db_query("insert into db_acount values($acount,3267,18491,'','".AddSlashes(pg_result($resaco,0,'l48_tipo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
//       $resac = db_query("insert into db_acount values($acount,3267,18492,'','".AddSlashes(pg_result($resaco,0,'l48_nomearquivo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
//       $resac = db_query("insert into db_acount values($acount,3267,18490,'','".AddSlashes(pg_result($resaco,0,'l48_arquivo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
//     }
    return true;
  }
  ...
  function alterar ($l48_sequencial=null) {
    $this->atualizacampos();
    $sql = " update editaldocumento set ";
    $virgula = "";
    ...
    if(trim($this->l48_edital)!="" || isset($GLOBALS["HTTP_POST_VARS"]["l48_edital"])){
      $sql  .= $virgula." l48_edital = $this->l48_edital ";
      $virgula = ",";
      if(trim($this->l48_edital) == null ){
        $this->erro_sql = " Campo Acordo nao Informado.";
        $this->erro_campo = "l48_edital";
        $this->erro_banco = "";
        $this->erro_msg   = "Usuário: \\n\\n ".$this->erro_sql." \\n\\n";
    ...
    if($l48_sequencial!=null){
      $sql .= " l48_sequencial = $this->l48_sequencial";
    }
//    $resaco = $this->sql_record($this->sql_query_file($this->l48_sequencial));
//    if($this->numrows>0){
...
//     }
  ...
  function sql_query ( $l48_sequencial=null,$campos="*",$ordem=null,$dbwhere=""){
    ...
    $sql .= " from editaldocumento ";
    $sql .= "      inner join liclancedital  on  liclancedital.l47_sequencial = editaldocumento.l48_edital";
    $sql2 = "";
    if($dbwhere==""){
      if($l48_sequencial!=null ){
        $sql2 .= " where editaldocumento.l48_sequencial = $l48_sequencial ";
      }
    }else if($dbwhere != ""){
      $sql2 = " where $dbwhere";
    }
