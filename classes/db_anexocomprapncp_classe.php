...
class cl_anexocomprapncp
{
    ...
    function incluir()
    {
        ...
        if ($this->l216_sequencial == "" || $this->l216_sequencial == null) {
            $result = db_query("select nextval('licanexopncp_l216_sequencial_seq')");
            if ($result == false) {
                $this->erro_banco = str_replace("\n", "", @pg_last_error());
                $this->erro_sql   = "Verifique o cadastro da sequencia: licanexopncp_l216_sequencial_seq do campo: l216_sequencial";
                $this->erro_msg   = "Usuário: \\n\\n " . $this->erro_sql . " \\n\\n";
                $this->erro_msg   .=  str_replace('"', "", str_replace("'", "",  "Administrador: \\n\\n " . $this->erro_banco . " \\n"));
                $this->erro_status = "0";
                return false;
            ...
            $result = db_query("select last_value from licanexopncp_l216_sequencial_seq");
    ...
    public function sql_anexos_licitacao($l20_codigo)
    {
       ...
       SELECT l216_nomedocumento
        FROM licanexopncp
        INNER JOIN licanexopncpdocumento ON l216_sequencial = l216_licanexospncp
