...
class cl_cvc402024
{
  ...
  function incluir($si149_sequencial)
  {
    ...
    if ($si149_sequencial == "" || $si149_sequencial == null) {
      $result = db_query("select nextval('cvc402024_si149_sequencial_seq')");
      if ($result == false) {
        $this->erro_banco = str_replace("
", "", @pg_last_error());
        $this->erro_sql = "Verifique o cadastro da sequencia: cvc402024_si149_sequencial_seq do campo: si149_sequencial";
        $this->erro_msg = "Usuário: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
        $this->erro_status = "0";
      ...
      $result = db_query("select last_value from cvc402024_si149_sequencial_seq");
...
", "", @pg_last_error());
      if (strpos(strtolower($this->erro_banco), "duplicate key") != 0) {
        $this->erro_sql = "cvc402024 ($this->si149_sequencial) nao Incluído. Inclusao Abortada.";
        $this->erro_msg = "Usuário: \n\n " . $this->erro_sql . " \n\n";
...
", "", @pg_last_error());
  ...
  function sql_query($si149_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "")
  {
    ...
    if ($dbwhere == "") {
      if ($si149_sequencial != null) {
        $sql2 .= " where cvc402024.si149_sequencial = $si149_sequencial ";
      }
    } else if ($dbwhere != "") {
      $sql2 = " where $dbwhere";
    }
