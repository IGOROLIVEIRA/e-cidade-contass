...
class cl_relatorios {
  ...
  function atualizacampos($exclusao=false) {
    if ($exclusao==false) {
       ...
       $this->rel_modulo = ($this->rel_modulo == ""?@$GLOBALS["HTTP_POST_VARS"]["rel_modulo"]:$this->rel_modulo);
  ...
  function incluir ($rel_sequencial) {
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if (($resaco!=false)||($this->numrows!=0)) {
         ...
         $resac = db_query("insert into db_acount values($acount,1010192,2012517,'','".AddSlashes(pg_result($resaco,0,'rel_modulo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if ($this->numrows>0) {
         ...
         for($conresaco=0;$conresaco<$this->numrows;$conresaco++) {
           ...
           if (isset($GLOBALS["HTTP_POST_VARS"]["rel_modulo"]) || $this->rel_modulo != "")
             $resac = db_query("insert into db_acount values($acount,1010192,2012517,'".AddSlashes(pg_result($resaco,$conresaco,'rel_modulo'))."','$this->rel_modulo',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
     ...
     if (!isset($lSessaoDesativarAccount) || (isset($lSessaoDesativarAccount)
       ...
       if (($resaco != false) || ($this->numrows!=0)) {
         ...
         for ($iresaco = 0; $iresaco < $this->numrows; $iresaco++) {
           ...
           $resac  = db_query("insert into db_acount values($acount,1010192,2012517,'','".AddSlashes(pg_result($resaco,$iresaco,'rel_modulo'))."',".db_getsession('DB_datausu').",".db_getsession('DB_id_usuario').")");
  ...
  function sql_query ( $rel_sequencial=null,$campos="*",$ordem=null,$dbwhere="") {
     ...
     $sql .= " from relatorios ";
     $sql .= "      inner join db_modulos  on  db_modulos.id_item = relatorios.rel_modulo";
     $sql2 = "";
     if ($dbwhere=="") {
       if ($rel_sequencial!=null ) {
