...
class cl_regadesao202024
{
  ...
  function incluir($si73_sequencial)
  {
    ...
    if ($si73_sequencial == "" || $si73_sequencial == null) {
      $result = db_query("select nextval('regadesao202024_si73_sequencial_seq')");
      if ($result == false) {
        $this->erro_banco = str_replace("
", "", @pg_last_error());
        $this->erro_sql = "Verifique o cadastro da sequencia: regadesao202024_si73_sequencial_seq do campo: si73_sequencial";
        $this->erro_msg = "Usuário: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
        $this->erro_status = "0";
        return false;
      ...
      $result = db_query("select last_value from regadesao202024_si73_sequencial_seq");
...
", "", @pg_last_error());
      if (strpos(strtolower($this->erro_banco), "duplicate key") != 0) {
        $this->erro_sql = "regadesao202024 ($this->si73_sequencial) nao Incluído. Inclusao Abortada.";
        $this->erro_msg = "Usuário: \n\n " . $this->erro_sql . " \n\n";
...
", "", @pg_last_error());
  ...
  function sql_query($si73_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "")
  {
    ...
    if ($dbwhere == "") {
      if ($si73_sequencial != null) {
        $sql2 .= " where regadesao202024.si73_sequencial = $si73_sequencial ";
      }
    } else if ($dbwhere != "") {
      $sql2 = " where $dbwhere";
    }
