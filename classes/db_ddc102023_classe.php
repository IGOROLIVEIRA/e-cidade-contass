...
class cl_ddc102023
{
  ...
  function incluir($si150_sequencial)
  {
    ...
    if ($si150_sequencial == "" || $si150_sequencial == null) {
      $result = db_query("select nextval('ddc102023_si150_sequencial_seq')");
      if ($result == false) {
        $this->erro_banco = str_replace("
", "", @pg_last_error());
        $this->erro_sql = "Verifique o cadastro da sequencia: ddc102023_si150_sequencial_seq do campo: si150_sequencial";
        $this->erro_msg = "Usuário: \n\n " . $this->erro_sql . " \n\n";
        $this->erro_msg .= str_replace('"', "", str_replace("'", "", "Administrador: \n\n " . $this->erro_banco . " \n"));
        $this->erro_status = "0";
      ...
      $result = db_query("select last_value from ddc102023_si150_sequencial_seq");
...
", "", @pg_last_error());
      if (strpos(strtolower($this->erro_banco), "duplicate key") != 0) {
        $this->erro_sql = "ddc102023 ($this->si150_sequencial) nao Incluído. Inclusao Abortada.";
        $this->erro_msg = "Usuário: \n\n " . $this->erro_sql . " \n\n";
...
", "", @pg_last_error());
  ...
  function sql_query($si150_sequencial = null, $campos = "*", $ordem = null, $dbwhere = "")
  {
    ...
    if ($dbwhere == "") {
      if ($si150_sequencial != null) {
        $sql2 .= " where ddc102023.si150_sequencial = $si150_sequencial ";
      }
    } else if ($dbwhere != "") {
      $sql2 = " where $dbwhere";
    }
