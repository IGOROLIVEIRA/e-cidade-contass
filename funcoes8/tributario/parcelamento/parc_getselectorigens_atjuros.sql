...
begin
  ...
  if dDataCorrecao is null then
    ...
    sCamposSql :=               ' distinct k00_numcgm, k00_dtoper, k00_receit, k00_hist, k00_valor, k00_dtvenc, k00_numpre, k00_numpar, k00_numtot, k00_numdig, k00_tipo, k00_tipojm,  ';
    if iTipoAnulacao = 1 then
      ...
      sCamposSql := sCamposSql || ' ( arreold.k00_valor * coalesce( fc_juros(arreold.k00_receit,arreold.k00_dtvenc,\''||dDataCorrecao||'\',\''||dDataCorrecao||'\',false,'||iAnoUsu||'),0)) as juros, ';
      sCamposSql := sCamposSql || ' ( arreold.k00_valor * coalesce( fc_multa(arreold.k00_receit,arreold.k00_dtvenc,\''||dDataCorrecao||'\',arreold.k00_dtoper,'||iAnoUsu||'),0)) as multa ';
    end if;
