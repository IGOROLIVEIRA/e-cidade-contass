...
//			db_criatabela($Identificacao);exit;
...
//        db_msgbox("entrou2");exit;
      ...
      //  } else {
          ...
          $z01_numcgm = $q02_numcgm;
					$pdf1->precgccpf = $z01_cgccpf;
     //   }
  //      db_msgbox("entrou1");exit;

      } else {
    //    db_msgbox("entrou");exit;
      ...
      if ($fc_febraban == ""){
        ...
        exit;
          ...
          if (!empty ($HTTP_POST_VARS["ver_matric"])) {
            ...
            $pdf1->predescr3_1 = $z01_cgmpri." - ".$z01_nome;//$proprietario;
            $pdf1->descr3_2    = $xender;
            $pdf1->predescr3_2 = $xender;
            $pdf1->descr17     = $bql; //variavel q guarda o setor/quadra/lote


	//					die("$z01_cgmpri  $proprietario  $xender  $z01_cgmpri  $proprietario  $z01_nome  $xender ");
          } else {
//						die("$z01_cgmpri  $proprietario  $xender  $z01_cgmpri  $proprietario  $z01_nome  $xender ");
...
//    die("bairro ".$z01_numcgm);
    ...
    if (!empty ($HTTP_POST_VARS["ver_matric"])) {
      ...
      $pdf1->precomplpri   = $j39_compl;
			/*********************************/
//      $sqlendercgm = "select z01_ender,z01_numero,z01_compl from cgm where z01_numcgm = $z01_numcgm";
//			$rsEnderCgm  = pg_query( $sqlendercgm);
 ///   	db_fieldsmemory($rsEnderCgm,0);
//		db_criatabela($rsEnderCgm);
//  	die($xxender);
			/************************************/
