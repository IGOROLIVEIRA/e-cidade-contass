...
if ($oParam->exec == "addRetencao") {
  ...
  $oRetencaoNota->getRetencoesFromDB($oParam->params[0]->iCodOrd, true, 2, $iMesUsu, $iAnoUsu);
  $oRetencaoNota->setInSession(true);
  $oRetorno->aRetencoes = $oRetencaoNota->getRetencoes();
  $oRetorno->status     = 1;
  $oRetorno->message    = ""; 
  ...
  try {
    ...
    db_fim_transacao(false);
    echo $oJson->encode(array("status" => 1, "message"=> urlencode("Retenções lançadas com sucesso")));
  ...
  try {
    ...
    if ($oParam->params[0]->iTipoCalc == 1 || $oParam->params[0]->iTipoCalc == 2) {
      ...
      foreach ($aRetencoes as $oRetencaoAtiva) {
        ...
        if ($oRetencaoAtiva->e21_retencaotipocalc == 3 || $oRetencaoAtiva->e21_retencaotipocalc == 3) {
  ...
  $oRetorno->status = 1;
  $oRetorno->aRetencoes = $oRetencaoNota->getRetencoesByMovimento($oParam->iCodMov, $iMesUsu, $iAnoUsu, null, true, true);
  if (count($oRetorno->aRetencoes) == 0 ) {
    $oRetorno->status == 2;
  }
  echo $oJson->encode($oRetorno);
