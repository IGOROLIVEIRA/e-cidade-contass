...
class retencaoNota {
   ...
   */
  function getRetencoesFromDB($iNotaLiquidacao, $lInSession = true, $iTipo = 2,$iMes = "", $iAno = "") {
    ...
    if (!isset($_SESSION["retencaoNota{$this->iCodNota}"]) || $lInSession == false) {
      ...
      $sSqlRetencaoReceitas  = $oDaoRetencaoReceitas->sql_query_notas(null,
                                                             "tabrec.*,retencaotiporec.*,
                                                              retencaoreceitas.*,e27_empagemov",
                                                             "e21_sequencial",
  ...
  function getValorRetencao($iNotaLiquidacao) {
    ...
    $sSqlTotalRetencao = "select fc_valorretencao({$iNotaLiquidacao}) as valortotal";
   ...
   * @return float
   */
  function getValorRetencaoMovimento($iCodMov, $lrecolhido = false) {
    
    if (empty($iCodMov)){
      throw new Exception("Erro [1] - movimento da Agenda não informado");
    }
    $sRecolhido = $lrecolhido?"true":"false";
    $sSqlTotalRetencao = "select fc_valorretencaomov({$iCodMov}, {$sRecolhido}) as valortotal";
    return db_utils::fieldsMemory(db_query($sSqlTotalRetencao), 0)->valortotal;
   ...
   * emite os recibos avulsos, e planilçhas de retencao.
   *
    ...
    foreach ($aRetencoes as $oRetencao) {
      ...
      if ($oRetencao->e23_valorretencao > 0) {
        ...
        if ($oRetencao->e21_retencaotipocalc != 5 || $oInstit->prefeitura == "f") {
            ...
            $oReciboAvulso->adicionarReceita($oRetencao->k02_codigo, $oRetencao->e23_valorretencao);
            if (isset($oReciboAvulso)) {
              ...
              $rsRecurso  = $oDaoSaltes->sql_record($oDaoSaltes->sql_query($this->getConta(),"c61_codigo"));
              if ($oDaoSaltes->numrows == 1) {
                $oReciboAvulso->adicionarRecurso(db_utils::fieldsMemory($rsRecurso, 0)->c61_codigo);
              }
          ...
          $oNotaPlanilha->sCnpj               = $oCgmCredor->z01_cgccpf;
          ...
          $oNotaPlanilha->sNome               = $oNotaLiquidacao->oDadosOrdem->z01_nome; 
          ...
          $oReciboDebito->setHistorico($sHistoricoRecibo);
          $oReciboDebito->emiteRecibo();
          $oReciboDebito->autenticarRecibo();
          
          //throw new Exception("Parando........");
   ...
   * @param integer $iMes Mes da retencao
   * @param integer $iAno ano da retencao
   * @param integer $iTipoRetencao tipo do calculo da retencao
   */
  function getRetencoesByMovimento($iCodMovimento, $iMes, $iAno, $iTipoRetencao='', $lRecolhida = false, $lPrincipal = false) {
     ...
     $sSqlRetencaoReceitas = $oDaoRetencaoReceitas->sql_query_notas(null,
                                                             "distinct tabrec.*,retencaotiporec.*,
                                                              retencaoreceitas.*,e27_principal,k108_slip ",
                                                             "e21_sequencial
                                                               ",
                                                              "e27_empagemov = {$iCodMovimento}
                                                              and e23_recolhido = {$sRecolhida}
                                                              and e23_ativo     = true
                                                              and extract(year from e23_dtcalculo) = {$iAno}
                                                              and extract(month from e23_dtcalculo) = {$iMes}
   ...
   * , caso houver
   *
   * @param unknown_type $iCodigoRetencao
   * @return unknown
   */
    ...
    if ($oRetencao->e23_valorretencao > 0) {
      ...
      if ($oRetencao->e21_retencaotipocalc != 5 || $oInstit->prefeitura == "f") {
        ...
        if (isset($oReciboAvulso)) {
          ...
          $oReciboAvulso->estornarRecibo($iNumpre);
          
        }
      } else {
        
        require_once ('model/planilhaRetencao.model.php');
        $oReciboDebito = new  recibo(2, null);
        $oReciboDebito->setConta($this->getConta());
        $oReciboDebito->setGrupoAutenticacao($this->getGrupoAutenticacao());
        $iNumpre = $this->getNumpreRetencao($oRetencao->e23_sequencial);
        $oReciboDebito->estornarRecibo($iNumpre);
