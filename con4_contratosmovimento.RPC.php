...
switch($oParam->exec) {
  ...
  case "assinarContrato":
      ...
      /*
       * Validaчѕes caso a origem do contrato seja Licitaчуo
       * O sistema nуo deve permitir a inclusуo de acordos quando a data de assinatura do acordo for anterior a data de homologaчуo da licitaчуo.
       * Para dispensa/inexigibilidade deve se validar a data de ratificaчуo presente no cadastro de licitaчуo
       */
        if($oAcordo->getOrigem() == 2){
            foreach ($oAcordo->getLicitacoes() as $oLicitacao){
                ...
                if (!$oAcordo->validaDataAssinatura($oLicitacao->getCodigo(), $oParam->dtmovimentacao, $bValidaDispensa)) {
                    $lAcordoValido = false;
                     throw new Exception("A data de assinatura do acordo nуo pode ser anterior a data de homologaчуo da licitaчуo.");
                }
            }
        }
        ...
        if(strtotime($dtMovimento) > strtotime(str_replace("/","-",$oAcordo->getDataFinal()))){
            $lAcordoValido = false;
            throw new Exception("A data de assinatura do acordo {$oParam->dtmovimentacao} nуo pode ser posterior ao perэodo de vigъncia do contrato {$oAcordo->getDataFinal()}.");
        }

        /**
         * Validaчуo solicitada: nуo seja possэvel incluir assinatura de acordos que nуo tenha as penalidades e garantias cadastradas.?
         * @see OC 3495
         */

        if(count($oAcordo->getPenalidades()) == 0 || count($oAcordo->getGarantias()) == 0){
            $lAcordoValido = false;
            throw new Exception("Nуo щ permitido assinar um acordos que nуo tenha as penalidades e garantias cadastradas.");
        }

          /**
           * Validaчуo soliciatada: Validar o sistema para que nуo seja possэvel assinar acordos de origem Manual que nуo tenha itens vinculados.
           * @see OC 3499
           */
