...
if( !isset($argv[1]) ){
	echo "\n Passagem de Parametros incoreta, deverá ser informado Ano Mês:  Ex: 200810 ";
	exit;
}
...
@pg_query("insert into sau_rubrica values(0,'','',$ano,$mes)");

//Procedimentos
$file = file("TabelaUnificada_".$comp."/tb_procedimento.txt");

$erro = false;
pg_query("begin;");

for($x=0; $x < count($file); $x++){
	$financiamento  = pg_result(pg_query("select sd65_i_codigo 
											from sau_financiamento 
										    where sd65_c_financiamento = '".trim(substr($file[$x],312,2))."'"),0,0);
	if(trim(substr($file[$x],314,6)) == ""){
		$rubrica = "null";
	}else{
		$rubrica = pg_result(pg_query("select sd64_i_codigo 
										from sau_rubrica       
									where sd64_c_rubrica = '".trim(substr($file[$x],314,6))."'"),0,0);
	}
	...
	if($query){
		...
		echo "$c1 - $c2 - $c3/$c4 Não Incluido ".pg_errormessage()."<br>\n";
		$count2++;
