<?
 ...
 if(!($conn = pg_connect("host=$DB_SERVIDOR dbname=$DB_BASE port=$DB_PORTA user=$DB_USUARIO password=$DB_SENHA"))){
  ...
  $arq1 = "txt/03_sau_financiamento_erro.txt";
  system( "clear" );
  system("> $arq1");
  ...
  //financiamento
  $file = file("arquivos/tb_financiamento.txt");
  for($x=0; $x < count($file); $x++){
   ...
   $sql = "INSERT INTO sau_financiamento(sd65_i_codigo,
										 sd65_c_financiamento,
										 sd65_c_nome   ,
										 sd65_i_anocomp,
										 sd65_i_mescomp )
   		    		    VALUES(nextval('sau_financiamento_sd65_i_codigo_seq'),
   			    	           '$c1',
   				               '$c2',
   				               '$c3',
   				               '$c4')";
