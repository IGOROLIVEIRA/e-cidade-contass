 ...
 for($t=0;$t<$linhas2;$t++){
  ...
  if(trim($situacaoaluno=="CANDIDATO")){
   ...
   if($cal_ultima_matricula!=$calendarioaluno){
    ...
    if($codalunocurso!=1423 && $codalunocurso!=1458 && $codalunocurso!=1428){
     $sql3 = "DELETE FROM alunopossib WHERE ed79_i_alunocurso = $codalunocurso";
     $result3 = pg_query($sql3);
     $sql4 = "DELETE FROM alunocurso WHERE ed56_i_codigo = $codalunocurso";
     $result4 = pg_query($sql4);
     $codalunosexc .= $sep.$codigoaluno;
     $sep = ",";
     echo ($r+1)."--> EXC - $codalunocurso--$codigoaluno | $situacaoaluno | $descrcalaluno ($seriealuno - $ensinoaluno)\n";
     $exclusao .= $sepexc."EXC";
     $sepexc = "|";
    }else{
     $codalunosexc .= $sep.$codigoaluno;
     $sep = ",";
     echo ($r+1)."--> NAO - $codalunocurso--$codigoaluno | $situacaoaluno | $descrcalaluno ($seriealuno - $ensinoaluno)\n";
     $exclusao .= $sepexc."NAO";
     $sepexc = "|";
    }
   }else{
    if($codalunocurso==636){
     $sql3 = "DELETE FROM alunopossib WHERE ed79_i_alunocurso = $codalunocurso";
     $result3 = pg_query($sql3);
     $sql4 = "DELETE FROM alunocurso WHERE ed56_i_codigo = $codalunocurso";
     $result4 = pg_query($sql4);
  ...
  if($exclusao=="NAO|NAO") echo "NNNNNNNNNNNNNNNNNNNNNN\n";
  if($exclusao=="EXC|EXC") echo "EEEEEEEEEEEEEEEEEEEEEE\n";
 }
 echo "------------------------------------------------------------------------\n";
}
