<?
set_time_limit(0);
$host="";
$base="";
$user="";
$pass="";
$port="5432";
if(!($conn = pg_connect("host=$host dbname=$base port=$port user=$user password=$pass"))) {
 echo "Erro ao conectar...\n\n";
 exit;
}else{
 echo "conectado...\n\n";
}

function maiusculo(&$string){
 $string = strtoupper($string);
 $string = str_replace("Ã¡","Ã",$string);
 $string = str_replace("Ã©","Ã",$string);
 $string = str_replace("Ã­","Ã",$string);
 $string = str_replace("Ã³","Ã",$string);
 $string = str_replace("Ãº","Ã",$string);
 $string = str_replace("Ã¢","Ã",$string);
 $string = str_replace("Ãª","Ã",$string);
 $string = str_replace("Ã´","Ã",$string);
 $string = str_replace("Ã®","Ã",$string);
 $string = str_replace("Ã»","Ã",$string);
 $string = str_replace("Ã£","Ã",$string);
 $string = str_replace("Ãµ","Ã",$string);
 $string = str_replace("Ã§","Ã",$string);
 $string = str_replace("Ã ","Ã",$string);
 $string = str_replace("Ã¨","Ã",$string);
 return $string;
}

function TiraAcento($string){
 set_time_limit(240);
 $acentos = 'áéíóúÁÉÍÓÚàÀÂâÊêôÔüÜïÏöÖñÑãÃõÕçÇªºäÄ\'';
 $letras  = 'AEIOUAEIOUAAAAEEOOUUIIOONNAAOOCCAOAA ';
 $new_string = '';
 for($x=0; $x<strlen($string); $x++){
  $let = substr($string, $x, 1);
  for($y=0; $y<strlen($acentos); $y++){
   if($let==substr($acentos, $y, 1)){
    $let=substr($letras, $y, 1);
    break;
   }
  }
  $new_string = $new_string . $let;
 }
 return $new_string;
}
function Progresso($linha,$total,$dado1,$dado2,$titulo){
 $linha++;
 $percent = ($linha/$total)*100;
 $percorrido = floor($percent);
 $restante = 100-$percorrido;
 $tracos = "";
 for($t=0;$t<$percorrido;$t++){
  $tracos .= "#";
 }
 $brancos = "";
 for($t=0;$t<$restante;$t++){
  $brancos .= ".";
 }
 echo " $titulo";
 echo " $linha de $total registros.\n";
 echo " [".$tracos.$brancos."] ".number_format($percent,2,".",".")."%\n";
 if($titulo!=" PROGRESSÃO TOTAL DA TAREFA"){
  echo " ---> ".trim($dado1)." -- ".trim($dado2)."\n";
 }
}
sleep(1);
system("clear");
echo " ->Iniciando processo...\n\n";
pg_exec("begin");
echo " ->Alterando sequência...\n\n";
$sql0 = "ALTER SEQUENCE cursoformacao_ed94_i_codigo_seq START 50";
$result0 = pg_query($sql0);

$t = 0;
$total = 32041;

echo " ->Atualizando cursoformacao atual...\n\n";
//////////CURSOFORMACAO-ATUAL
$ponteiro = fopen("cursoformacao_capivari.txt","r");
$erro = false;
$codvalidos = "";
$codcursoformacao = "";
$sep = "";
while (!feof($ponteiro)){
 $linha = fgets($ponteiro,200);
 if(substr($linha,0,2)=="--"){
  $array_linha = explode("|",$linha);
  $array_linha[1] = (int)str_replace(chr(39),"",$array_linha[1]);
  $sql1 = "DELETE FROM formacao WHERE ed27_i_cursoformacao = $array_linha[1]";
  $result1 = pg_query($sql1);
  $sql2 = "DELETE FROM cursoformacao WHERE ed94_i_codigo = $array_linha[1]";
  $result2 = pg_query($sql2);
  if($result1==false || $result2==false){
   $erro = true;
   break;
  }
 }else{
  $array_linha = explode("|",$linha);
  $array_linha[0] = (int)str_replace(chr(39),"",$array_linha[0]);
  $array_linha[2] = str_replace(chr(39),"",$array_linha[2]);
  $array_linha[2] = trim(strtoupper(maiusculo(TiraAcento($array_linha[2]))));
  $codvalidos .= $sep."#".$array_linha[2]."#";
  $codcursoformacao .= $sep.$array_linha[0];
  $sep = ",";
 }
}
fclose($ponteiro);
if($erro==true){
 echo "\n\n ERRO: ".pg_errormessage()."\n\n";
 pg_exec("rollback");
 exit;
}else{
 sleep(1);
}

//////////CURSOFORMACAO-NOVA
$ponteiro = fopen("tabela_cursoformacao.txt","r");
$erro = false;
$x = 0;
while (!feof($ponteiro)){
 $linha = fgets($ponteiro,200);
 $array_linha = explode("|",$linha);
 $array_linha[0] = (int)str_replace(chr(39),"",$array_linha[0]);
 $array_linha[1] = str_replace(chr(39),"",$array_linha[1]);
 $array_linha[1] = trim(strtoupper(maiusculo(TiraAcento($array_linha[1]))));
 $array_linha[2] = str_replace(chr(39),"",$array_linha[2]);
 $array_linha[2] = trim(strtoupper(maiusculo(TiraAcento($array_linha[2]))));
 $array_linha[3] = str_replace(chr(39),"",$array_linha[3]);
 $array_linha[3] = trim(strtoupper(maiusculo(TiraAcento($array_linha[3]))));
 if(strstr($codvalidos,"#".$array_linha[2]."#")){
  $arr_cod = explode(",",$codvalidos);
  $arr_codcurso = explode(",",$codcursoformacao);
  for($f=0;$f<count($arr_cod);$f++){
   if($arr_cod[$f]=="#".$array_linha[2]."#"){
    $coded94 = $arr_codcurso[$f];
   }
  }
  $sql4 = "UPDATE cursoformacao SET
            ed94_i_codclasse = $array_linha[0],
            ed94_c_descrclasse = '$array_linha[1]',
            ed94_c_codigocenso = '$array_linha[2]',
            ed94_c_descr = '$array_linha[3]'
           WHERE ed94_i_codigo = $coded94
          ";
  $result4 = pg_query($sql4);
 }else{
  $sql4 = "INSERT INTO cursoformacao
            (ed94_i_codigo,
             ed94_i_codclasse,
             ed94_c_descrclasse,
             ed94_c_codigocenso,
             ed94_c_descr)
           VALUES
            (nextval('cursoformacao_ed94_i_codigo_seq'),
             $array_linha[0],
             '$array_linha[1]',
             '$array_linha[2]',
             '$array_linha[3]')
          ";
  $result4 = pg_query($sql4);
 }
 if($result4==false){
  $erro = true;
  break;
 }else{
  system("clear");
  echo Progresso($x,1224,"( ".$array_linha[0]." ) ".$array_linha[2],$array_linha[3]," PROGRESSÃO: TABELA CURSOFORMACAO");
  echo Progresso($t,$total,"( ".$array_linha[0]." ) ".$array_linha[2],$array_linha[3]," PROGRESSÃO TOTAL DA TAREFA");
 }
 $x++;
 $t++;
}
if($erro==true){
 echo "\n\n ERRO: ".pg_errormessage()."\n\n";
 pg_exec("rollback");
 exit;
}else{
 echo "  \nConcluída CURSOFORMACAO\n\n";
 sleep(1);
}

//////////CENSOINSTSUPERIOR
$ponteiro = fopen("tabela_censoinstsuperior.txt","r");
$erro = false;
$x = 0;
$primeiro = "";
while (!feof($ponteiro)){
 $linha = fgets($ponteiro,500);
 $array_linha = explode("|",$linha);
 $array_linha[0] = str_replace(chr(39),"",$array_linha[0]);
 $array_linha[0] = trim(strtoupper(maiusculo(TiraAcento($array_linha[0]))));
 $array_linha[2] = (int)str_replace(chr(39),"",$array_linha[2]);
 $array_linha[3] = str_replace(chr(39),"",$array_linha[3]);
 $array_linha[3] = trim(strtoupper(maiusculo(TiraAcento($array_linha[3]))));
 $array_linha[4] = (int)str_replace(chr(39),"",$array_linha[4]);
 $array_linha[6] = (int)str_replace(chr(39),"",$array_linha[6]);
 $array_linha[9] = (int)str_replace(chr(39),"",$array_linha[9]);
 if($primeiro!=$array_linha[2]){
  $sql1 = "INSERT INTO censoinstsuperior
            (ed257_i_codigo,
             ed257_c_nome,
             ed257_i_dependencia,
             ed257_i_tipo,
             ed257_i_censomunic)
           VALUES
            ($array_linha[2],
             '$array_linha[3]',
             $array_linha[4],
             $array_linha[6],
             $array_linha[9])
          ";
  $result1 = pg_query($sql1);
  $primeiro = $array_linha[2];
 }
 $sql2 = "SELECT ed94_i_codigo FROM cursoformacao WHERE ed94_c_codigocenso = '$array_linha[0]'";
 $result2 = pg_query($sql2);
 $codcurso = pg_result($result2,0,0);
 $sql3 = "INSERT INTO censoinstcursos
            (ed258_i_codigo,
             ed258_i_cursoformacao,
             ed258_i_censoinstsuperior)
           VALUES
            (nextval('censoinstcursos_ed258_i_codigo_seq'),
             $codcurso,
             $array_linha[2])
          ";
 $result3 = pg_query($sql3);
 if($result1==false || $result2==false || $result3==false){
  $erro = true;
  break;
 }else{
  system("clear");
  echo Progresso($x,30817,"( ".$array_linha[0]." ) ".$array_linha[2],$array_linha[3]," PROGRESSÃO: TABELA CENSOINSTSUPERIOR");
  echo Progresso($t,$total,"( ".$array_linha[0]." ) ".$array_linha[2],$array_linha[3]," PROGRESSÃO TOTAL DA TAREFA");
 }
 $x++;
 $t++;
}
if($erro==true){
 echo "\n\n ERRO: ".pg_errormessage()."\n\n";
 pg_exec("rollback");
 exit;
}else{
 echo "  \nConcluída CENSOINSTSUPERIOR\n\n";
 sleep(1);
}

system("clear");
echo Progresso($total-1,$total,$array_linha[2],$array_linha[3]," PROGRESSÃO TOTAL DA TAREFA");
echo " \n->Terminado processo...\n\n";
if($erro==false){
 pg_exec("commit");
}
?>