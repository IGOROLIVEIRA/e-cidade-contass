...
$ponteiro = fopen("medrefcerto.txt","r");
while (!feof($ponteiro)){
...
while (!feof($ponteiro)){
 ...
 $sql2  ="select fa18_c_classetera from far_classeterapeutica where fa18_c_classetera='".$array_linha[5]."'";
 $result2 = pg_query($sql2) or die("\n !Erro = ".pg_errormessage()." \n >>> SQL:".$sql2);
 $linhas=pg_num_rows($result2);
 if($array_linha[5]!=""){
  if($linhas==0){   
     $sql4 = "INSERT INTO far_classeterapeutica (fa18_i_codigo,fa18_c_classetera)
           VALUES(nextval('far_classeterapeutica_fa18_codigo_seq'),'$array_linha[5]')
          ";
     echo".";
     $result4 = pg_query($sql4) or die("\n !Erro = ".pg_errormessage()." \n >>> SQL:".$sql4);
  }
...
while (!feof($ponteiro)){
 ...
 $sql9  ="select fa18_i_codigo from far_classeterapeutica where fa18_c_classetera='".$array_linha[5]."'";
...
while (!feof($ponteiro)){
 ...
 @$for= pg_result($result9,0,0);
  ...
  if(trim($linha)!=""){
   $sql15  ="select fa35_i_medanvisa,fa35_i_listacontrolado from far_listacontroladomed where  fa35_i_medanvisa=$med and fa35_i_listacontrolado=$for";
   $result15 = pg_query($sql15) or die("\n !Erro = ".pg_errormessage()." \n >>> SQL:".$sql15);
   $linhas15=pg_num_rows($result15);
    if($linhas15==0) {
...
while (!feof($ponteiro)){
 ...
 $sql2  ="select fa20_c_prescricao from far_prescricaomedica where fa20_c_prescricao='".$array_linha[7]."'";
 $result2 = pg_query($sql2) or die("\n !Erro = ".pg_errormessage()." \n >>> SQL:".$sql2);
 $linhas=pg_num_rows($result2);
 if($array_linha[7]!=""){
  if($linhas==0){   
     $sql4 = "INSERT INTO far_prescricaomedica (fa20_i_codigo,fa20_c_prescricao)
           VALUES(nextval('far_prescricaomedica_fa20_codigo_seq'),'$array_linha[7]')
          ";
     echo".";
     $result4 = pg_query($sql4) or die("\n !Erro = ".pg_errormessage()." \n >>> SQL:".$sql4);
  }
...
while (!feof($ponteiro)){
 ...
 $sql9  ="select fa20_i_codigo from far_prescricaomedica where fa20_c_prescricao='".$array_linha[7]."'";
 $result9 = pg_query($sql9) or die("\n !Erro = ".pg_errormessage()." \n >>> SQL:".$sql7);
 $linhas9=pg_num_rows($result9);
 @$for= pg_result($result9,0,0);
