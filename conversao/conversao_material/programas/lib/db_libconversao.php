...
/* Wrapper para db_log porem fazendo sempre log em arquivo
 *
 * $sLog       String     Mensagem a ser logada
 * $sArquivo   String     Nome do arquivo onde sera logada a mensagem
 *
 * returns     Timestamp  Timestamp no momento da execucao da funcao
 */

function db_log_arquivo($sLog="", $sArquivo=""){
  return db_log($sLog, $sArquivo, 2) ;
}
...
function db_gera_backup($pConexao, $sTabela, $sTabelaWork="", $sWhere) {
	
 /*
  * Verificamos se foi informado o nome de trabalho da tabela
  * Caso não tenha sido informado passamos o nome da table para consistencia_datadageracao_[valor do parâmetro $sTabela]
  */
  if ($sTabelaWork == "") {
  	$sTabelaWork = "consistencia_".date("Ymd")."_{$sTabela}";
  }
  
  $lErro = false;
  
  /*
   * Verificamos se a tabela de trabalho já existe na base de dados
   * Caso ela exista inserimos os regsitros através do select na tabela pelos registros retornados do where  
   * Caso contrário criamos a tabela apartir do select na tabela pelos registros retornados do where
   */ 
  $sVerificaTabelaWork  = "select relname from pg_class where relname = '$sTabelaWork'";
  $rsVerificaTabelaWork = db_query($pConexao, $sVerificaTabelaWork);
  if (pg_numrows($rsVerificaTabelaWork) > 0) {
  	$sSqlTableWork = "insert into {$sTabelaWork} select * from {$sTabela} where {$sWhere}"; 	
  } else {
  	$sSqlTableWork = "create table {$sTabelaWork} as select * from {$sTabela} where {$sWhere}";
  }
  $rsGeraBackup = db_query($pConexao, $sSqlTableWork);
  if (!$rsGeraBackup) {
  	db_log("Erro ao gerar backup dos dados manipulados. Erro:".pg_last_error(),"",1);
  	$lErro = true;
  }
...
function db_inicio_transacao($pConexao) {
...
function db_fim_transacao($pConexao,$erro = false) {
    //#00#//db_fim_transacao
