...
switch ($oParam->exec) {
    case 'adjudicarLicitacao':
        ...
        try {
            ...
            //Verifica data de julgamento da licita√ß√£o
            if ($dataJulgamentoLicitacao > $l202_dataAdjudicacao) {
                throw new Exception("Data de Julgamento maior que a Data de AdjudicaÁ„o");
            }
            ...
            if ($l20_naturezaobjeto == "1") {
                if ($itens != null || $itens != "") {
                    throw new Exception("Itens obras n„o cadastrados. Codigos:" . $itens);
                }
            }

            /**
             * Verificar Encerramento Periodo Patrimonial e data do julgamento da licita√ß√£o
             */
            ...
            if (!empty($l202_dataAdjudicacao)) {
                ...
                if ($l202_dataAdjudicacao <= $dtencerramento) {
                    throw new Exception("O perÌodo j· foi encerrado para envio do SICOM. Verifique os dados do lanÁamento e entre em contato com o suporte.");
                }
            }
            ...
            if ($clhomologacaoadjudica->verificaPrecoReferencia($l202_licitacao) >= 1 || $precomedio >= 1) {
                if ($parecer >= 1) {
                    ...
                    if ($tipoparecer < 1) {
                        $erro = "LicitaÁ„o sem Parecer Cadastrado.";
                    ...
                    if ($clhomologacaoadjudica->erro_status == "0") {
                        ...
                        $erro = "AdjudicaÁ„o salva com sucesso!";
                        $oRetorno->message = urlencode($erro);
                        $oRetorno->status = 1;
                    }
                    /**
                     * Incluindo nova situa√ß√£o a licitacao ADJUDICADA
                     */
                    ...
                    $clliclicitasituacao->l11_obs         = "LicitaÁ„oo Adjudicada";
            ...
            if ($clhomologacaoadjudica->verificaPrecoReferencia($l202_licitacao) >= 1 || $precomedio >= 1) {
                if ($parecer >= 1) {
                    ...
                    if ($clhomologacaoadjudica->erro_status == "0") {
                        ...
                        $erro = "AdjudicaÁ„o alterada com sucesso!";
                        $oRetorno->message = urlencode($erro);
                        $oRetorno->status = 1;
                    }
                    /**
                     * Alterado nova situa√ß√£o a licitacao ADJUDICADA
                     */
    ...
    case 'excluiradjudicarLicitacao':
        ...
        try {
            ...
            if ($clhomologacaoadjudica->erro_status == "0") {
                ...
                $erro = "AdjudicaÁ„o Excluida com sucesso!";
                $oRetorno->message = urlencode($erro);
                $oRetorno->status = 1;
            }
    ...
    case 'homologarLicitacao':
        ...
        /**
         * VERIFICA SE E REGISTRO DE PRE√áO
         */
        ...
        try {
            ...
            //Verifica data de adjudicacao da licita√ß√£o
            if ($l202_datahomologacao < $datadeAdjudicacao) {
                throw new Exception("Data de homologaÁ„o menor que a data de adjudicaÁ„o.");
            }
            ...
            if ($l20_naturezaobjeto == "1") {
                if ($l20_cadinicial != "3" && $l20_cadinicial != "4") {
                    throw new Exception("Usu·rio: Edital n„o cadastrado ou com o status de Aguardando envio. Gentileza verificar!");
                }
            }
            ...
            if ($clhomologacaoadjudica->verificaPrecoReferencia($l202_licitacao) >= 1 || $precomedio >= 1) {
                if ($parecer >= 1) {
                    ...
                    if ($tipoparecer < 1) {
                        throw new Exception("LicitaÁ„o sem Parecer Cadastrado.");
                    }
                    db_inicio_transacao();
                    /**
                    ...
                    if ($l20_tipnaturezaproced == '1' || $l20_tipnaturezaproced == '3') {
                        ...
                        if ($clhomologacaoadjudica->erro_status == "0") {
                            ...
                            $erro = "HomologaÁ„o salva com sucesso!";
                            $oRetorno->message = urlencode($erro);
                            $oRetorno->sequencial = $clhomologacaoadjudica->l202_sequencial;
                            $oRetorno->status = 1;
                        }
                        /**
                         * Incluindo nova situa√ß√£o a licitacao Homologada
                         */
                        $clhomologacaoadjudica->alteraLicitacao($l202_licitacao, 10);

                        $clliclicitasituacao->l11_data        = date("Y-m-d", db_getsession("DB_datausu"));
                        $clliclicitasituacao->l11_hora        = db_hora();
                        $clliclicitasituacao->l11_obs         = "LicitaÁ„o Homologada";
                        ...
                        if ($clhomologacaoadjudica->erro_status == "0") {
                            ...
                            /*Verifica se √© registro de preco*/
                            $result = $clliclicita->sql_record($clliclicita->sql_query($l202_licitacao));
                            $l20_tipnaturezaproced  = db_utils::fieldsMemory($result, 0)->l20_tipnaturezaproced;
                            $oRetorno->regpreco = $l20_tipnaturezaproced;

                            /*cria consulta da solicita√ß√£o com licita√ß√£o*/
    ...
    case 'alterarHomologacao':
        ...
        try {
            ...
            if ($ac16_sequencial != "") {
                throw new Exception("N√£o e Permitida alteraÁ„o de HomologaÁ„o com Contrato lanÁado!");
            }
    ...
    case 'excluirhomologacao':
        ...
        try {
            ...
            if (!empty($l202_dataAdjudicacao)) {
                ...
                if ($l202_dataAdjudicacao <= $dtencerramento) {
                    throw new Exception("O perÌ≠odo j· foi encerrado para envio do SICOM. Verifique os dados do lanÁamento e entre em contato com o suporte.");
                }
            }
            if ($l20_tipnaturezaproced == '1' || $l20_tipnaturezaproced == '3') {
                ...
                foreach ($oParam->aItens as $Item) {
                    ...
                    if (!empty($rsItensContrato)) {
                        throw new Exception("ERRO! Existe Contrato para itens dessa HomologaÁ„o.");
                    }
                ...
                if ($ItensHomologacao <= 0) {
                    ...
                    $erro = "HomologaÁ„o Excluida com sucesso!";
                    $oRetorno->message = urlencode($erro);
                    $oRetorno->status = 1;
                } else {
