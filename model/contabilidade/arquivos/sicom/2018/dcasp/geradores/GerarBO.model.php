...
class GerarBO extends GerarAM
{
  ...
  public function gerarDados()
  {
    ...
    $sSql = "select * from bodcasp102018 where si201_ano = {$this->iAno} AND si201_periodo = {$this->iPeriodo} AND si201_institu = " . db_getsession("DB_instit");
    $rsBO10 = db_query($sSql);

    $sSql = "select * from bodcasp202018 where si202_anousu = {$this->iAno} AND si202_periodo = {$this->iPeriodo} AND si202_instit = " . db_getsession("DB_instit");
    $rsBO20 = db_query($sSql);

    $sSql = "select * from bodcasp302018 where si203_anousu = {$this->iAno} AND si203_periodo = {$this->iPeriodo} AND si203_instit = " . db_getsession("DB_instit");
    $rsBO30 = db_query($sSql);

    $sSql = "select * from bodcasp402018 where si204_ano = {$this->iAno} AND si204_periodo = {$this->iPeriodo} AND si204_institu = " . db_getsession("DB_instit");
    $rsBO40 = db_query($sSql);

    $sSql = "select * from bodcasp502018 where si205_ano = {$this->iAno} AND si205_periodo = {$this->iPeriodo} AND si205_institu = " . db_getsession("DB_instit");
    ...
    if (pg_num_rows($rsBO10) == 0
      ...
      for ($iCont = 0; $iCont < pg_num_rows($rsBO40); $iCont++) {
        ...
        $aCSVBO40['si204_vlrspnaoprocpessoalencarsociais']  = $this->sicomNumberReal($aBO40['si204_vlrspnaoprocpessoalencarsociais'], 2);
