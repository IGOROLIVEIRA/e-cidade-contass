...
class SicomArquivoPagamentosDespesas extends SicomArquivoBase implements iPadArquivoBaseCSV {
  ...
  public function gerarDados() {
    ...
    /**
  	 ...
  	 */
    $sArquivo = "legacy_config/sicom/{$sCnpj}_sicomidentresponsavel.xml";
    ...
    /**
		 ...
		 */
		$sArquivo = "legacy_config/sicom/{$sCnpj}_sicomelementodespesa.xml";
		if (!file_exists($sArquivo)) {
...
where c71_coddoc in (5,35,37) and c71_data >= '".$this->sDataInicial."' and  c71_data <= '".$this->sDataFinal."'
    	...
    	if ($iQuantPagamento > $iQuantExtorno) {
	      ...
	      foreach ($oResponsaveis as $oResponsavel) {
	    	  ...
	    	  if ($oResponsavel->getAttribute('instituicao') == db_getsession("DB_instit")
	    	      && $oResponsavel->getAttribute('tipoResponsavel') == "01"
	    	      && implode("-", array_reverse(explode("/", $oResponsavel->getAttribute('dtFinal')))) >= $oPagamento->dtpagamento
	    	      && implode("-", array_reverse(explode("/", $oResponsavel->getAttribute('dtInicio')))) <= $oPagamento->dtpagamento) {

	          $sSql          = "SELECT z01_nome, z01_cgccpf FROM cgm WHERE z01_numcgm = ".$oResponsavel->getAttribute('numCgm');
	          $rsResponsavel = db_query($sSql);
	          $sNomeResp     = db_utils::fieldsMemory($rsResponsavel, 0)->z01_nome;
	          $sCpfResp      = db_utils::fieldsMemory($rsResponsavel, 0)->z01_cgccpf;
	          break;
	        ...
	        if (!$oPagamento->e50_obs) {
	          ...
	          $sEspecificacaoOp = substr(str_replace($aCaracteres, "", $oPagamento->e50_obs), 0, 200);
		    	...
		    	$oDadosPagamento->codReduzidoOP       = substr($oPagamento->numordem, 0, 15);
		    	$oDadosPagamento->tipoOP              = $iTipoOp;
		    	...
		    	$oDadosPagamento->codUnidade          = $sCodUnidade;
		    	$oDadosPagamento->nroEmpenho          = substr($oPagamento->e60_codemp, 0, 22);
		    	$oDadosPagamento->dtEmpenho           = implode(array_reverse(explode("-", $oPagamento->dtempenho)));
		    	$oDadosPagamento->nroLiquidacao       = substr($oPagamento->e71_codnota, 0, 9);
		    	$oDadosPagamento->dtLiquidacao        = implode(array_reverse(explode("-", $oPagamento->dtliquida)));
		    	$oDadosPagamento->codFuncao           = str_pad($oPagamento->o58_funcao, 2, "0", STR_PAD_LEFT);
		    	$oDadosPagamento->codSubFuncao        = str_pad($oPagamento->o58_subfuncao, 3, "0", STR_PAD_LEFT);
		    	$oDadosPagamento->codPrograma         = str_pad($oPagamento->o58_programa, 4, "0", STR_PAD_LEFT);
		    	$oDadosPagamento->idAcao              = str_pad($oPagamento->o58_projativ, 4, "0", STR_PAD_LEFT);
		    	$oDadosPagamento->elementoDespesa     = substr($sElemento, 0, 6);
		    	$oDadosPagamento->subElemento         = substr($sElemento, 6, 2);
		    	$oDadosPagamento->dotOrig             = $iDotOrig;
		    	$oDadosPagamento->vlOP                = number_format($oPagamento->vlrordem, 2, "", "");
		    	$oDadosPagamento->tipoDocumentoCredor = strlen($oPagamento->z01_cgccpf) == 11 ? 1 : 2;
		    	$oDadosPagamento->nroDocumento        = $oPagamento->z01_cgccpf;
		    	$oDadosPagamento->nomeCredor          = substr($oPagamento->z01_nome, 0, 120);
		    	$oDadosPagamento->especificacaoOP     = $sEspecificacaoOp;
		    	$oDadosPagamento->nomeRespPgto        = substr($sNomeResp, 0, 50);
		    	$oDadosPagamento->cpfRespPgto         = $sCpfResp;
	    	 ...
	    	 *registro 12
	    	 */
	    	$sSql = "select e50_codord,e50_data,c63_banco,c63_agencia,c63_dvagencia,c63_conta,c63_dvconta,k12_valor,k12_cheque, e96_codigo, e96_descr
	from empagemov
	...
	and c61_anousu = c60_anousu
	      join conplanoconta on c63_codcon = c60_codcon
	...
	where k105_corgrupotipo != 2 and e80_instit = 1 and k12_codord = {$oPagamento->numordem} and e81_cancelado is null";
	      ...
	      for ($iCont2 = 0; $iCont2 < pg_num_rows($rsMovimentacao); $iCont2++) {
	      	...
	      	$oDadosMovimentacao->digitoVerificadorAgencia       = $oMovimentacao->c63_dvagencia == "0" ? " " : substr($oMovimentacao->c63_dvagencia, 0, 2);
    ...
    $sSql = "SELECT  slip.k17_codigo,k17_data, k17_valor, k17_texto, z01_nome, z01_cgccpf, k17_dtaut
	from slip
		...
		join cgm on k17_numcgm = z01_numcgm
where k17_dtaut >= '".$this->sDataInicial."' and k17_dtaut <= '".$this->sDataFinal."' and c60_codsis = 7";
    ...
    for ($iCont = 0; $iCont < pg_num_rows($rsPagamento); $iCont++) {
      ...
      foreach ($oResponsaveis as $oResponsavel) {
    	  ...
    	  if ($oResponsavel->getAttribute('instituicao') == db_getsession("DB_instit")
    	      && $oResponsavel->getAttribute('tipoResponsavel') == "01"
    	      && $oResponsavel->getAttribute('dtFinal') >= implode("/", array_reverse(explode("-", $oPagamento->k17_dtaut)))
    	      && $oResponsavel->getAttribute('dtInicio') <= implode("/", array_reverse(explode("-", $oPagamento->k17_dtaut)))) {
      ...
      $sHash  = "10"."3".substr($oPagamento->k17_codigo.substr($oPagamento->k17_data, 0, 4), 0, 15).$sOrgao;
    	...
    	if (!isset($aDadosAgrupados[$sHash])) {
	      ...
	      $oDadosPagamento = new stdClass();
	    	...
	    	$oDadosPagamento->tipoRegistro        = 10;
	    	$oDadosPagamento->detalhesessao       = 10;
	    	$oDadosPagamento->codReduzidoOP       = substr($oPagamento->k17_codigo.substr($oPagamento->k17_data, 0, 4), 0, 15);
	    	$oDadosPagamento->tipoOP              = "3";
	    	$oDadosPagamento->nroOP               = substr($oPagamento->k17_codigo.substr($oPagamento->k17_data, 0, 4), 0, 15);
	    	$oDadosPagamento->dtPagamento         = implode(array_reverse(explode("-", $oPagamento->k17_dtaut)));
	    	$oDadosPagamento->codOrgao            = $sOrgao;
	    	$oDadosPagamento->codUnidade          = " ";
	    	$oDadosPagamento->nroEmpenho          = " ";
	    	$oDadosPagamento->dtEmpenho           = " ";
	    	$oDadosPagamento->nroLiquidacao       = " ";
	    	$oDadosPagamento->dtLiquidacao        = " ";
	    	$oDadosPagamento->codFuncao           = " ";
	    	$oDadosPagamento->codSubFuncao        = " ";
	    	$oDadosPagamento->codPrograma         = " ";
	    	$oDadosPagamento->idAcao              = " ";
	    	$oDadosPagamento->elementoDespesa     = " ";
	    	$oDadosPagamento->subElemento         = " ";
	    	$oDadosPagamento->dotOrig             = " ";
	    	$oDadosPagamento->vlOP                = number_format($oPagamento->k17_valor, 2, "", "");
	    	$oDadosPagamento->tipoDocumentoCredor = strlen($oPagamento->z01_cgccpf) == 11 ? 1 : 2;
	    	$oDadosPagamento->nroDocumento        = $oPagamento->z01_cgccpf;
	    	$oDadosPagamento->nomeCredor          = substr($oPagamento->z01_nome, 0, 120);
	    	$oDadosPagamento->especificacaoOP     = substr(str_replace($aCaracteres, "", $oPagamento->k17_texto), 0, 200);
	    	$oDadosPagamento->nomeRespPgto        = substr($sNomeResp, 0, 50);
	    	$oDadosPagamento->cpfRespPgto         = $sCpfResp;
	    	$oDadosPagamento->Reg12               = array();

    	    	$sSql = "SELECT k17_codigo,k17_data,c63_banco,c63_agencia,c63_dvagencia,c63_conta,c63_dvconta,k12_id, e96_codigo, k17_valor
      from slip
           ...
           left join corrente on k12_conta = k12_conta and slip.k17_dtaut = k12_data and slip.k17_autent = k12_autent and k12_valor = k17_valor
           left join empageslip on e89_codigo = k17_codigo
           inner join empagemov on e81_codmov = e89_codmov and e81_cancelado is null
           left join empagemovforma on e97_codmov = e89_codmov
           left join empageforma on e97_codforma = e96_codigo
           left join corgrupocorrente cg on cg.k105_autent = corrente.k12_autent
               and cg.k105_data = corrente.k12_data
               and cg.k105_id = corrente.k12_id
where k17_codigo =  {$oPagamento->k17_codigo}  and c60_anousu = ".db_getsession("DB_anousu");
      ...
      for ($iCont2 = 0; $iCont2 < pg_num_rows($rsMovimentacao); $iCont2++) {
      	...
      	if ($oMovimentacao->e96_codigo == 2) {
             ...
             where empageslip.e89_codigo=".$oMovimentacao->k17_codigo."
                   and e81_cancelado is null";
      		$rsCheque = db_query($sSql);
      		$oCheque = db_utils::fieldsMemory($rsCheque, 0);
      		$sNroDocumento    = $oCheque->e91_cheque;
      	...
      	$oDadosMovimentacao->tipoRegistro                   = 12;
      	$oDadosMovimentacao->detalhesessao                  = 12;
      	$oDadosMovimentacao->codReduzidoOP                  = substr($oMovimentacao->k17_codigo.substr($oMovimentacao->k17_data, 0, 4), 0, 15);
      	$oDadosMovimentacao->tipoDocumentoOP                = $sTipoDocumentoOP;
      	$oDadosMovimentacao->nroDocumento                   = $sNroDocumento;
      	$oDadosMovimentacao->banco                          = $sBanco;
      	$oDadosMovimentacao->agencia                        = substr($oMovimentacao->c63_agencia, 0, 6);
      	...
      	$oDadosMovimentacao->contaCorrente                  = substr($oMovimentacao->c63_conta, 0, 12);
      	$oDadosMovimentacao->digitoVerificadorContaBancaria = substr($oMovimentacao->c63_dvconta, 0, 2);
      	$oDadosMovimentacao->dtEmissao                      = implode(array_reverse(explode("-", $oMovimentacao->k17_data)));
      	$oDadosMovimentacao->vlDocumento                    = number_format($oMovimentacao->k17_valor, 2, "", "");
    	  ...
    	  $aDadosAgrupados[$sHash] = $oDadosPagamento;
    		...
    		$aDadosAgrupados[$sHash]->vlOP += number_format($oPagamento->k17_valor, 2, "", "");;
    	}
