...
class SicomArquivoDetalhamentoMetasFiscais extends SicomArquivoBase implements iPadArquivoBaseCSV
{
   ...
   */
		...
		// Define todos anos utilizados no relatï¿½rio apartir do ano de referï¿½ncia
		/*
		 ...
		 * so funciona deixando os anos da forma que tï¿½ aqui
		 */
		...
		/**
		 * Retirado pela validaÃ§Ã£o do PPA
		 */
		//$iAno3   = $iAnoRef+2;
		...
		// Cria objeto para cada linha do relatï¿½rio
		...
		// Cria array com todos objetos "Linhas" do relatï¿½rio
		$aLista = array(
			$oReceitaTotal,
			$oReceitasPrimarias,
			$oDespesaTotal,
		...
		// Cria dinï¿½micamente o objeto valor para cada linha evitando referï¿½ncia de objetos
		...
		// Seta descriï¿½ï¿½o de cada linha
		$oReceitaTotal->Descricao      = "Receita Total";
		$oReceitasPrimarias->Descricao = "Receitas Primï¿½rias(I)";
		$oDespesaTotal->Descricao      = "Despesa Total";
		$oDespesasPrimarias->Descricao = "Despesas Primï¿½rias(II)";
		$oResultadoPrimario->Descricao = "Resultado Primï¿½rio(III) = (I-II)";
		$oResultadoNominal->Descricao  = "Resultado Nominal";
		$oDivPublicConsol->Descricao   = "Dï¿½vida Pï¿½blica Consolidada";
		$oDivConsolLiquid->Descricao   = "Dï¿½vida Consolidada Lï¿½quida";
		...
		if (!array_key_exists($iAno1, $aPIB) || !array_key_exists($iAno2, $aPIB)) {
			throw new Exception("Valor do PIB do Cenï¿½rio Macroeconï¿½mico não informado!", 4);
		}

		//print_r($aValorReceitaTotal);exit; 

		// Busca taxa de inflaï¿½ï¿½o de cada ano
		...
		if (!array_key_exists($iAno1, $aValTaxaDefla) || !array_key_exists($iAno2, $aValTaxaDefla)) {
			throw new Exception("Valor das taxa de inflaï¿½ï¿½o do Cenï¿½rio Macroeconï¿½mico não informado!");
		}

		// Calcula taxa de deflaï¿½ï¿½o de cada ano
		...
		foreach ($aEstimativaReceita as $iInd => $oReceita) {
			...
			if (
				$oReceita->iEstrutural == 413250000000000 ||
		...
		foreach ($aEstimativaDespesa as $iInd => $oDespesa) {
			...
			if (
				substr($oDespesa->iElemento, 0, 3) != 332 &&
		...
		for ($iCont = 1; $iCont <= 3; $iCont++) {
			...
			$oDadosMTFIS->vlResultadoPrimario                 = number_format($oResultadoPrimario->aValores[${$iAnoUsu}]->Corrente, 2, ",", "");
