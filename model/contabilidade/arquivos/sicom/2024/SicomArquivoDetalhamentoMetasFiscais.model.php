...
//    }
      ...
      $oDadosLdo  = db_utils::getCollectionByRecord($rsLdo, true);
      ...
      if ($clmtfis_ldo->numrows > 0) {
        foreach ($oDadosLdo as $dadosLdo) {
          for ($iCont = 0; $iCont <= 3; $iCont++) {
            $oDadosMTFIS = new stdClass();
            if($iCont == 1) {
                $oDadosMTFIS->exercicio = db_getsession("DB_anousu");
                $oDadosMTFIS->vlMetasFiscaisRPPS = db_utils::fieldsMemory($clmtfis_anexo->sql_record($clmtfis_anexo->sql_query('', '*', '', "mtfisanexo_especificacao = 'Receita Total' and mtfisanexo_ldo = $dadosLdo->mtfis_sequencial")), 0)->mtfis_mfrpps;
