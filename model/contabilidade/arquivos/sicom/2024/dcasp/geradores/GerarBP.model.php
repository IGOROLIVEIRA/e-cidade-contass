...
class GerarBP extends GerarAM
{
  ...
  public function gerarDados()
  {
    ...
    $sSql = "select * from bpdcasp102024 where si208_ano = {$this->iAno} AND si208_periodo = {$this->iPeriodo} AND si208_institu = " . db_getsession("DB_instit");
    $rsBP10 = db_query($sSql);

    $sSql = "select * from bpdcasp202024 where si209_ano = {$this->iAno} AND si209_periodo = {$this->iPeriodo} AND si209_institu = " . db_getsession("DB_instit");
    $rsBP20 = db_query($sSql);

    $sSql = "select * from bpdcasp302024 where si210_ano = {$this->iAno} AND si210_periodo = {$this->iPeriodo} AND si210_institu = " . db_getsession("DB_instit");
    $rsBP30 = db_query($sSql);

    $sSql = "select * from bpdcasp402024 where si211_ano = {$this->iAno} AND si211_periodo = {$this->iPeriodo} AND si211_institu = " . db_getsession("DB_instit");
    $rsBP40 = db_query($sSql);

    $sSql = "select * from bpdcasp502024 where si212_ano = {$this->iAno} AND si212_periodo = {$this->iPeriodo} AND si212_institu = " . db_getsession("DB_instit");
    $rsBP50 = db_query($sSql);

    $sSql = "select * from bpdcasp602024 where si213_ano = {$this->iAno} AND si213_periodo = {$this->iPeriodo} AND si213_institu = " . db_getsession("DB_instit");
    $rsBP60 = db_query($sSql);

    $sSql = "select * from bpdcasp702024 where si214_ano = {$this->iAno} AND si214_periodo = {$this->iPeriodo} AND si214_institu = " . db_getsession("DB_instit");
    $rsBP70 = db_query($sSql);

    $sSql = "select * from bpdcasp712024 where si215_ano = {$this->iAno} AND si215_periodo = {$this->iPeriodo} AND si215_institu = " . db_getsession("DB_instit");
    ...
    if (pg_num_rows($rsBP10) == 0
      ...
      for ($iCont = 0; $iCont < pg_num_rows($rsBP20); $iCont++) {
        ...
        $aCSVBP20['si209_vlpassivopirculapdiantaclidemaisobrigacurtoprazo']   = $this->sicomNumberReal($aBP20['si209_vlpassicircudemaiobrigcurtoprazo'], 2);
      ...
      for ($iCont = 0; $iCont < pg_num_rows($rsBP71); $iCont++) {
        ...
        $aCSVBP71['si215_codfontrecursos24']= $aBP71['si215_codfontrecursos24'] == 0 || $aBP71['si215_codfontrecursos24'] == '' ? '' : $aBP71['si215_codfontrecursos24'];
