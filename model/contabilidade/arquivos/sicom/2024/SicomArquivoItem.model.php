...
class SicomArquivoItem extends SicomArquivoBase implements iPadArquivoBaseCSV {
  ...
  public function gerarDados(){
  	 ...
  	 */
  	$clitem102024 = new cl_item102024();

  	/**
     * excluir informacoes do mes selecioado
     */
    db_inicio_transacao();
    $result = db_query($clitem102024->sql_query(NULL,"*",NULL,"si43_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']." and si43_instit=".db_getsession("DB_instit")));
    if (pg_num_rows($result) > 0) {
    	$clitem102024->excluir(NULL,"si43_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']." and si43_instit=".db_getsession("DB_instit"));
      if ($clitem102024->erro_status == 0) {
    	  throw new Exception($clitem102024->erro_msg);
      }
    }
    ...
    for ($iCont10 = 0; $iCont10 < pg_num_rows($rsResult10); $iCont10++) {
    	...
    	$sSqlitem="select si43_coditem,si43_unidademedida from item102024  where si43_coditem=".$oDados10->coditem." and si43_unidademedida='{$oDados10->unidademedida}'";
    	 ...
    	 */
        	 if(pg_num_rows($rsResultitem) == 0 ){
		          ...
		          $clitem102024->si43_tiporegistro           = 10;
				  $clitem102024->si43_coditem                = $oDados10->coditem;
				  $clitem102024->si43_dscItem                = addslashes($oDados10->dscitem);
				  $clitem102024->si43_unidademedida          = $oDados10->unidademedida;
				  $clitem102024->si43_tipocadastro           = $oDados10->tipocadastro;
				  $clitem102024->si43_justificativaalteracao = $oDados10->justificativaalteracao;
				  $clitem102024->si43_instit		   				   = db_getsession("DB_instit");
				  $clitem102024->si43_mes                    = $this->sDataFinal['5'].$this->sDataFinal['6'];

				  $clitem102024->incluir(null);
				  if ($clitem102024->erro_status == 0) {
				  	throw new Exception($clitem102024->erro_msg);
				  }
        	 }
