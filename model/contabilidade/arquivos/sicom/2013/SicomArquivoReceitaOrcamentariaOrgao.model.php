...
class SicomArquivoReceitaOrcamentariaOrgao extends SicomArquivoBase implements iPadArquivoBaseCSV {
  ...
  public function gerarDados(){
    ...
    /**
  	 * selecionar arquivo xml com dados dos orgaos
  	 */
    ...
    foreach ($oOrgaos as $oOrgao) {
      ...
      if ($oReceita->o70_codrec != 0 && $sVerificaReceita == 1) {
	      ...
	      if (!isset($oDadosReceita->FonteRecurso[$sHashFonte])) {
	        ...
	        $oDadosReceita->FonteRecurso[$sHashFonte] = $oDadosFonteRecurso;
	        
	      } else {
	      	$oDadosFonteRecurso = $oDadosReceita->FonteRecurso[$sHashFonte];
	      }
	    
	      $oDadosFonteRecurso->valorFonte += number_format($oReceita->o70_valor, 2, "", "");
    ...
    if (count($aCodReceita) > 0) {
      throw new Exception("Códigos ".implode(",", $aCodReceita)." da receita inexistente no siscom. Deve ser incluído um codigo de receita correspondente.");
	    ...
	    foreach ($aDadosAgrupados as $oDado) {
	    	...
	    	$oDadosReceita->vlPrevisto = abs($oDadosReceita->vlPrevisto);
	     ...
	     */
	    	foreach ($oDado->FonteRecurso as $oFonteRecurso) {
	    		$oFonteRecurso->valorFonte = abs($oFonteRecurso->valorFonte);
