...
class gerarCADOBRAS extends GerarAM
{
    ...
    public function gerarDados()
    {
        ...
        $sSql = "select * from cadobras102022 where si198_mes = " . $this->iMes . " and si198_instit=" . db_getsession("DB_instit");
        $rscadobras102022 = db_query($sSql);

        $sSql = "select * from cadobras202022 where si199_mes = " . $this->iMes . " and si199_instit=" . db_getsession("DB_instit") . " order by si199_codobra,si199_situacaodaobra";
        $rscadobras202022 = db_query($sSql);

        $sSql = "select * from cadobras212022 where si200_mes = " . $this->iMes . " and si200_instit=" . db_getsession("DB_instit");
        $rscadobras212022 = db_query($sSql);

        $sSql = "select * from cadobras302022 where si201_mes = " . $this->iMes . " and si201_instit=" . db_getsession("DB_instit");
        $rscadobras302022 = db_query($sSql);

        /**
         *
         * Registros 10
         */
        if (pg_num_rows($rscadobras102022) == 0 && pg_num_rows($rscadobras202022) == 0 && pg_num_rows($rscadobras212022) == 0 && pg_num_rows($rscadobras302022) == 0) {
            $aCSV['tiporegistro'] = '99';
            $this->sLinha = $aCSV;
            $this->adicionaLinha();
        } else {
            for ($iCont = 0; $iCont < pg_num_rows($rscadobras102022); $iCont++) {

                $aCADORBRAS10 = pg_fetch_array($rscadobras102022, $iCont);
            ...
            for ($iCont20 = 0; $iCont20 < pg_num_rows($rscadobras202022); $iCont20++) {

                $aCADORBRAS20 = pg_fetch_array($rscadobras202022, $iCont20);
                ...
                if ($aCADORBRAS20['si199_situacaodaobra'] == "4" || $aCADORBRAS20['si199_situacaodaobra'] == "3") {
                    for ($iCont21 = 0; $iCont21 < pg_num_rows($rscadobras212022); $iCont21++) {

                        $aCADORBRAS21 = pg_fetch_array($rscadobras212022, $iCont21);
            ...
            for ($iCont = 0; $iCont < pg_num_rows($rscadobras302022); $iCont++) {

                $aCADORBRAS30 = pg_fetch_array($rscadobras302022, $iCont);
