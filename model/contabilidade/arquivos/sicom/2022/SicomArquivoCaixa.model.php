...
require_once ("classes/db_caixa102022_classe.php");
require_once ("classes/db_caixa112022_classe.php");
require_once ("classes/db_caixa122022_classe.php");
...
class SicomArquivoCaixa extends SicomArquivoBase implements iPadArquivoBaseCSV {
  ...
  public function gerarDados() {
    ...
    $clcaixa122022 = new cl_caixa122022();
    $clcaixa112022 = new cl_caixa112022();
    $clcaixa102022 = new cl_caixa102022();
         ...
         */
	    db_inicio_transacao();
	    $result = $clcaixa102022->sql_record($clcaixa102022->sql_query(NULL,"*",NULL,"si103_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']." 
	     and si103_instit = ".db_getsession("DB_instit")));
	    if (pg_num_rows($result) > 0) {
	    	
	    	$clcaixa122022->excluir(NULL,"si105_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']." 
	    	and si105_instit = ".db_getsession("DB_instit"));
	    	$clcaixa112022->excluir(NULL,"si104_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']." 
	    	and si104_instit = ".db_getsession("DB_instit"));
	    	$clcaixa102022->excluir(NULL,"si103_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']." 
	    	and si103_instit = ".db_getsession("DB_instit"));
	   
	      if ($clcaixa102022->erro_status == 0) {
	    	  throw new Exception($clcaixa102022->erro_msg);
	      }
	    }
	    db_fim_transacao();
    ...
    for ($iCont = 0;$iCont < pg_num_rows($rsContasCaixa); $iCont++) {
        ...
        db_inicio_transacao();
    	  ...
    	  $oDadosCaixa = new cl_caixa102022();
    	...
    	$sSql = "select  11 as tiporegistro,
    		...
    		$sSql ="select 12 as tiporegistro,
					...
					left join orcreceita on c74_codrec = o70_codrec and o70_anousu = 2022     
					left join orcfontes on o70_codfon = o57_codfon and o70_anousu = o57_anousu
    		...
    		foreach ($aDadosAgrupados as $mov) {
    			...
    			$clcaixa112022->si104_tiporegistro            = $mov->si104_tiporegistro;
	    		$clcaixa112022->si104_codreduzido             = $mov->si104_codreduzido;
	    		$clcaixa112022->si104_tipomovimentacao        = $mov->si104_tipomovimentacao;
	    		$clcaixa112022->si104_tipoentrsaida           = $mov->si104_tipoentrsaida;
	    		$clcaixa112022->si104_descrmovimentacao       = $mov->si104_descrmovimentacao;
	    		$clcaixa112022->si104_valorentrsaida          = abs($mov->si104_valorentrsaida);
	    		$clcaixa112022->si104_codctbtransf            = $mov->si104_codctbtransf;
	    		$clcaixa112022->si104_codfontectbtransf  	  = $mov->si104_codfontectbtransf;
	    		$clcaixa112022->si104_mes  				      = $mov->si104_mes;
	    		$clcaixa112022->si104_reg10  			      = $mov->si104_reg10;
	    		$clcaixa112022->si104_instit                  = db_getsession("DB_instit");
	    		    			
    			$clcaixa112022->incluir(null);
    			
    		    if ($clcaixa112022->erro_status == 0) {
					throw new Exception($clcaixa112022->erro_msg);
				}
    		    ...
    		    if($mov->si104_tipoentrsaida == 1){
    				...
    				foreach ($mov->registro12 as $reg12){
    					...
    					$clcaixa122022->si105_tiporegistro                      = $reg12->si105_tiporegistro;
				    	$clcaixa122022->si105_codreduzido                       = $reg12->si105_codreduzido;
				    	$clcaixa122022->si105_ededucaodereceita                 = $reg12->si105_ededucaodereceita;
				    	$clcaixa122022->si105_identificadordeducao              = $reg12->si105_identificadordeducao;
				    	$clcaixa122022->si105_naturezareceita                   = $reg12->si105_naturezareceita;
				    	$clcaixa122022->si105_vlrreceitacont                    = $reg12->si105_vlrreceitacont;
				    	$clcaixa122022->si105_mes                               = $reg12->si105_mes;
				    	$clcaixa122022->si105_reg10                             = $oDadosCaixa->si103_sequencial;
				    	$clcaixa122022->si105_instit                            = db_getsession("DB_instit");
				    		
				    	$clcaixa122022->incluir(null);
