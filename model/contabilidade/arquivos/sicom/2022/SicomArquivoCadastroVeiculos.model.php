...
require_once ("classes/db_cvc102022_classe.php");
require_once ("classes/db_cvc202221_classe.php");
require_once ("classes/db_cvc302022_classe.php");
require_once ("classes/db_cvc402022_classe.php");
...
/**
  	 ...
  	 */
  	$clcvc102022 = new cl_cvc102022();
  	$clcvc202221 = new cl_cvc202221();
  	$clcvc302022 = new cl_cvc302022();
  	$clcvc402022 = new cl_cvc402022();
     ...
     */
    db_inicio_transacao();
    $result = db_query($clcvc102022->sql_query(NULL,"*",NULL,"si146_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']." and si146_instit=".db_getsession("DB_instit")));
    if (pg_num_rows($result) > 0) {
    	$clcvc102022->excluir(NULL,"si146_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']." and si146_instit=".db_getsession("DB_instit"));
      if ($clcvc102022->erro_status == 0) {
    	  throw new Exception($clcvc102022->erro_msg);
      }
    }
   
    $result = db_query($clcvc202221->sql_query(NULL,"*",NULL,"si147_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']." and si147_instit=".db_getsession("DB_instit")));
    if (pg_num_rows($result) > 0) {
    	$clcvc202221->excluir(NULL,"si147_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']." and si147_instit=".db_getsession("DB_instit"));
      if ($clcvc202221->erro_status == 0) {
    	  throw new Exception($clcvc202221->erro_msg);
      }
    }
    
    $result = db_query($clcvc302022->sql_query(NULL,"*",NULL,"si148_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']." and si148_instit=".db_getsession("DB_instit")));
    if (pg_num_rows($result) > 0) {
    	$clcvc302022->excluir(NULL,"s148_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']." and si148_instit=".db_getsession("DB_instit"));
      if ($clcvc302022>erro_status == 0) {
    	  throw new Exception($clcvc302022->erro_msg);
      }
    }
     
    $result = db_query($clcvc402022->sql_query(NULL,"*",NULL,"si149_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']." and si149_instit=".db_getsession("DB_instit")));
    if (pg_num_rows($result) > 0) {
    	$clcvc402022->excluir(NULL,"si149_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']." and si149_instit=".db_getsession("DB_instit"));
      if ($clcvc402022->erro_status == 0) {
    	  throw new Exception($clcvc402022->erro_msg);
      }
    }
    ...
    for ($iCont10 = 0; $iCont10 < pg_num_rows($rsResult10); $iCont10++) {
	    ...
	    $clcvc102022->si146_tiporegistro    	= 10;
		$clcvc102022->si146_codorgao			= $oDados10->codorgao;
		$clcvc102022->si146_codunidadesub		= $oDados10->codunidadesub;
		$clcvc102022->si146_codveiculo			= $oDados10->codveiculo;
		$clcvc102022->si146_tpveiculo			= $oDados10->tpveiculo;
		$clcvc102022->si146_subtipoveiculo		= $oDados10->subtipoveiculo;
		$clcvc102022->si146_descveiculo			= $oDados10->descveiculo;   
		$clcvc102022->si146_marca				= $oDados10->marca;        
		$clcvc102022->si146_modelo				= $oDados10->modelo;       
		$clcvc102022->si146_ano					= $oDados10->ano ;         
		$clcvc102022->si146_placa				= $oDados10->placa;        
		$clcvc102022->si146_chassi				= $oDados10->chassi;       
		$clcvc102022->si146_numerorenavam		= $oDados10->numerorenavam;
		$clcvc102022->si146_nroserie			= $oDados10->nroserie;       
		$clcvc102022->si146_situacao			= $oDados10->situacao;     
		$clcvc102022->si146_tpdeslocamento		= $oDados10->tpdeslocament;
		$clcvc102022->si146_instit				= db_getsession("DB_instit");
		$clcvc102022->si146_mes             	= $this->sDataFinal['5'].$this->sDataFinal['6']; 
		
		  
		  
		  $clcvc102022->incluir(null);
		  if ($clcvc102022->erro_status == 0) {
		  	throw new Exception($clcvc102022->erro_msg);
		  }
     ...
     */
		...
		AND db01_anousu=2022) AS codUnidadeSubResp,
		veiculos.ve01_codigo AS codVeiculo,
		1 AS origemGasto,
		...
		for ($iCont20 = 0; $iCont20 < pg_num_rows($rsResult20); $iCont20++) {
			...
			$clcvc202221->si147_tiporegistro 			= 20;
			$clcvc202221->si147_codorgao				= $oDados20->codorgao  ;
			$clcvc202221->si147_codunidadesub			= $oDados20->codunidadesub ;      
			$clcvc202221->si147_codveiculo				= $oDados20->codveiculo   ;        
			$clcvc202221->si147_origemgasto				= $oDados20->origemgasto ;        
			$clcvc202221->si147_codunidadesubempenho	= $oDados20->codunidadesubempenho;
			$clcvc202221->si147_nroempenho				= $oDados20->nroempenho   ;       
			$clcvc202221->si147_dtempenho				= $oDados20->dtempenho  ;         
			$clcvc202221->si147_tipogasto				= $oDados20->tipogasto  ;         
			$clcvc202221->si147_qtdeutilizada			= $oDados20->qtdeutilizada ;      
			$clcvc202221->si147_vlgasto					= $oDados20->vlgasto   ;           
			$clcvc202221->si147_dscpecasservicos		= $oDados20->dscpecasservicos  ;    
			$clcvc202221->si147_atestadocontrole		= $oDados20->atestadocontrole ;   
			$clcvc202221->si147_instit					= db_getsession("DB_instit");
			$clcvc202221->si147_mes            			= $this->sDataFinal['5'].$this->sDataFinal['6'];
		   ...
		   /**
			...
			$marcacaoFinal =  $oDadosAbs->km_inicial_comb;
          	...
          	$clcvc202221->si147_marcacaoinicial		= $marcacaoInicial;
          	$clcvc202221->si147_marcacaofinal		= $marcacaoFinal;
			
			echo $sSqlabs;exit;
		  	
		  $clcvc202221->incluir(null);
		  if ($clcvc202221->erro_status == 0) {
		    throw new Exception($clcvc202221->erro_msg);
		  }
...
//		AND veiculos.ve01_codigo = [ Código do Veículo ] ;
		...
		for ($iCont0 = 30; $iCont30 < pg_num_rows($rsResult30); $iCont30++) {
			...
			$clcvc302022->si148_tiporegistro 				= 30;			 
			$clcvc302022->si148_codorgao					= $oDados30->codorgao;
			$clcvc302022->si148_codunidadesub				= $oDados30->codunidadesubresp;
			$clcvc302022->si148_codveiculo					= $oDados30->codveiculo  ;
			$clcvc302022->si148_nomeestabelecimento			= $oDados30->nomeestabelecimento; 
			$clcvc302022->si148_localidade					= $oDados30->localidade  ;
			$clcvc302022->si148_qtdediasrodados				= $oDados30->qtdediasrodados;
			$clcvc302022->si148_distanciaestabelecimento	= $oDados30->distanciaestabelecimento; 
			$clcvc302022->si148_numeropassageiros			= $oDados30->numeropassageiros; 
			$clcvc302022->si148_turnos						= $oDados30->turnos;
			$clcvc302022->si148_instit						= db_getsession("DB_instit");
			$clcvc302022->si148_mes            				= $this->sDataFinal['5'].$this->sDataFinal['6'];
		  	
		  	$clcvc302022->incluir(null);
		  if ($clcvc302022->erro_status == 0) {
		    throw new Exception($clcvc302022->erro_msg);
		  }
		  ...
		  for ($iCont40 = 0; $iCont40 < pg_num_rows($rsResult40); $iCont40++) {
		    ...
		    $clcvc402022= new cl_cvc402022();
		  	...
		  	$clcvc402022->si149_tiporegistro 				= 40;
		  	$clcvc402022->si149_codorgao					= $oDados40->codorgao;
			$clcvc402022->si149_codunidadesub				= $oDados40->codunidadesubresp;
			$clcvc402022->si149_codveiculo					= $oDados40->codveiculo;
			$clcvc402022->si149_nomeestabelecimento			= $oDados40->nomeestabelecimento;
			$clcvc402022->si149_localidade					= $oDados40->localidade;
			$clcvc402022->si149_qtdediasrodados				= $oDados40->qtdediasrodados;
			$clcvc402022->si149_distanciaestabelecimento	= $oDados40->distanciaestabelecimento;
			$clcvc402022->si149_numeropassageiros			= $oDados40->numeropassageiros;
			$clcvc402022->si149_turnos						= $oDados40->turnos;
			$clcvc402022->si149_instit						= db_getsession("DB_instit");
			$clcvc402022->si149_mes           				= $this->sDataFinal['5'].$this->sDataFinal['6'];
		  	
		  	
		    $clcvc402022->incluir(null);
		    if ($clcvc402022->erro_status == 0) {
		  	  throw new Exception($clcvc402022->erro_msg);
		    }	
		}  
