...
where o46_data >= '".$this->sDataInicial."' and o46_data <= '".$this->sDataFinal."' and o47_valor > 0";
		  ...
		  if (!isset($aDecretosAgrupados[$sHash])) {
		  	...
		  	if (pg_num_rows($rsLoa) > 0) {
		  		...
		  		foreach ($oLeis as $olei) {
		  			...
		  			if (preg_replace("/[^0-9\s]/", "", $olei->getAttribute('nroLeiAlteracao')) == preg_replace("/[^0-9\s]/", "", $oDecretos->o45_numlei)) {
