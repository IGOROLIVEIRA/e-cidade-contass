...
class SicomArquivoBalancete extends SicomArquivoBase implements iPadArquivoBaseCSV
{
    ...
    /**
     * FUNÇÃO PARA GERAR MOVIMENTACAO E SALDO DOS CONTACORRENTE
     *
     *
     *
     */

    public function getSaldos($reduz, $contacorrente, $where)
    {
...
//where c60_anousu = " . db_getsession("DB_anousu") . " and substr(c60_estrut,1,9) = '622130300') as x
        ...
        for ($iCont = 0; $iCont < pg_num_rows($rsReg10); $iCont++) {
            ...
            if (!isset($aDadosAgrupados10[$sHash])) {
                ...
                $obalancete->si177_saldoinicial = $nSaldoInicial;
                $obalancete->si177_totaldebitos = $nDebitos;
                $obalancete->si177_totalcreditos = $nCreditos;
                $obalancete->si177_saldofinal = $nSaldoInicial + $nDebitos - $nCreditos;
                ...
                $obalancete->contas[] = $oReg10->c61_reduz;
                $aDadosAgrupados10[$sHash]->si177_saldoinicial += $nSaldoInicial;
        ...
        foreach ($aDadosAgrupados10 as $reg10Hash => $oContas10) {
            ...
            if ($oContas10->nregobrig == 11) {
                ...
                if (substr($oContas10->si177_contacontaabil, 0, 5) != '62213') {
                    $sSqlDotacoes = "select distinct o58_coddot as c73_coddot,
                                    ...
                                    case when o41_unidade is not null then
                        ...
                        if (!(($oReg12Saldo->saldoanterior == "" || $oReg12Saldo->saldoanterior == 0)  && $oReg12Saldo->debitos == "" && $oReg12Saldo->creditos == "")) {
                            ...
                            foreach ($oNaturezaReceita as $oNatureza) {
                                ...
                                if ($oNatureza->getAttribute('instituicao') == db_getsession("DB_instit")
                                    ...
                                    $sNaturezaReceita = substr(1,8,$oNatureza->getAttribute('receitaSicom'));
                                    break;
             ...
             * @todo: Falta processar os lançamentos para ter registros de empenhos na tabela contacorrentedetalhe
             */
                ...
                for ($iContRp = 0; $iContRp < pg_num_rows($rsRestos); $iContRp++) {
                    ...
                    for ($iContSaldo14 = 0; $iContSaldo14 < pg_num_rows($rsReg14saldos); $iContSaldo14++) {
                        ...
                        if (!(($oReg14Saldo->saldoanterior == "" || $oReg14Saldo->saldoanterior == 0) && $oReg14Saldo->debitos == "" && $oReg14Saldo->creditos == "")) {
                            ...
                            $sHash14 .= $oReg14->idacao . $oReg14->idsubacao . $oReg14->naturezadadespesa . $oReg14->subelemento . $oReg14->codfontrecursos . $oReg14->nroempenho . $oReg14->anoinscricao;
                            ...
                            if (!isset($aContasReg10[$reg10Hash]->reg14[$sHash14])) {
                                ...
                                $obalancete14->si181_naturezadespesa = $oReg14->naturezadadespesa;
                                $obalancete14->si181_subelemento = $oReg14->subelemento;
                ...
                foreach ($oContas10->contas as $oReduz) {
                    ...
                    for ($iContSaldo15 = 0; $iContSaldo15 < pg_num_rows($rsReg15saldos); $iContSaldo15++) {
                        ...
                        if (!isset($aContasReg10[$reg10Hash]->reg15[$sHash15])) {
                            ...
                            $obalancete15->si182_naturezasaldoinicialsf = $oReg15Saldo->saldoanterior == 0 ? $oContas10->naturezasaldo : ($oReg15Saldo->saldoanterior < 0 ? 'C' : 'D');
                            ...
                            if($obalancete15->si182_saldofinalsf == 0){
                                $obalancete15->si182_naturezasaldofinalsf = $oContas10->naturezasaldo;
                            }elseif($obalancete15->si182_saldofinalsf > 0){
                                $obalancete15->si182_naturezasaldofinalsf = 'D';
                            }else{
                                $obalancete15->si182_naturezasaldofinalsf = 'C';
                            }
                            ...
                            $aContasReg10[$reg10Hash]->reg15[$sHash15]->si182_naturezasaldofinalsf = $aContasReg10[$reg10Hash]->reg15[$sHash15]->si182_saldofinalsf == 0 ? $oContas10->naturezasaldo : ($aContasReg10[$reg10Hash]->reg15[$sHash15]->si182_saldofinalsf < 0 ? 'C' : 'D');
                            $aContasReg10[$reg10Hash]->reg15[$sHash15]->si182_naturezasaldoinicialsf = $aContasReg10[$reg10Hash]->reg15[$sHash15]->si182_saldoinicialsf == 0 ? $oContas10->naturezasaldo : ($aContasReg10[$reg10Hash]->reg15[$sHash15]->si182_saldoinicialsf > 0 ? 'D' : 'C');
                        }
                    }
                }
            }
                ...
                for ($iContctb = 0; $iContctb < pg_num_rows($rsSqlCTB); $iContctb++) {
                    ...
                    if (pg_num_rows($rsResultVerifica) == 0 ){
                        ...
                        for ($iContSaldo17 = 0; $iContSaldo17 < pg_num_rows($rsReg17saldos); $iContSaldo17++) {
                            ...
                            $sHash17 = '17' . $oContas10->si177_contacontaabil . $oContas10->identificadorfinanceiro . $objContasctb->codctb . $objContasctb->codfontrecursos;
                        ...
                        for ($iContVerCtb = 0; $iContVerCtb < pg_num_rows($rsResultVerifica); $iContVerCtb++) {
                            ...
                            for ($iContSaldo17 = 0; $iContSaldo17 < pg_num_rows($rsReg17saldos); $iContSaldo17++) {
                                ...
                                $sHash17 = '17' . $oContas10->si177_contacontaabil . $oContas10->identificadorfinanceiro . $oCtb->si95_codctb . $objContasctb->codfontrecursos;
                                           ...
                                           FROM
                                             (SELECT
                                                (SELECT CASE WHEN c29_debito > 0 THEN c29_debito WHEN c29_credito > 0 THEN -1 * c29_credito ELSE 0 END AS saldoanterior
                                                 ...
                                                 WHERE c19_reduz IN (" . implode(',', $oContas10->contas) . ")
                                                   AND c17_sequencial = {$nContaCorrente}
                                                   AND c19_orctiporec = {$objContasfr->o15_codigo}) AS saldoimplantado,
        ...
        */
        ...
        foreach ($aContasReg10 as $oDado10) {
            ...
            foreach ($oDado10->reg11 as $reg11) {
                ...
                $obalreg11->si178_idsubacao = $reg11->si178_subidacao;
