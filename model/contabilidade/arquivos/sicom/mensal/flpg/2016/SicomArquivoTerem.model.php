...
require_once ("classes/db_terem102016_classe.php");
require_once ("model/contabilidade/arquivos/sicom/mensal/geradores/2016/flpg/GerarTEREM.model.php");
...
class SicomArquivoTerem extends SicomArquivoBase implements iPadArquivoBaseCSV {
  ...
  public function gerarDados() {
    ...
    $sSql .= " and (round(te01_valor,2) not in (select round(si194_vlrparateto,2) from terem102016 where si194_mes <= ".($this->sDataFinal['5'].$this->sDataFinal['6'])." )";
    $sSql .= " or round(te01_valor,2) not in (select round(si194_vlrparateto,2) from terem102015)";
    $sSql .= " or round(te01_valor,2) not in (select round(si194_vlrparateto,2) from terem102014)";
