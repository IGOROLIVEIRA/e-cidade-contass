...
require_once("classes/db_aberlic102019_classe.php");
require_once("classes/db_aberlic112019_classe.php");
require_once("classes/db_aberlic122019_classe.php");
require_once("classes/db_aberlic132019_classe.php");
require_once("classes/db_aberlic142019_classe.php");
require_once("classes/db_aberlic152019_classe.php");
require_once("classes/db_aberlic162019_classe.php");

require_once("model/contabilidade/arquivos/sicom/mensal/geradores/2019/GerarABERLIC.model.php");
...
class SicomArquivoAberturaLicitacao extends SicomArquivoBase implements iPadArquivoBaseCSV
{
    ...
    public function gerarDados()
    {
         ...
         */
        $claberlic10 = new cl_aberlic102019();
        $claberlic11 = new cl_aberlic112019();
        $claberlic12 = new cl_aberlic122019();
        $claberlic13 = new cl_aberlic132019();
        $claberlic14 = new cl_aberlic142019();
        $claberlic15 = new cl_aberlic152019();
        $claberlic16 = new cl_aberlic162019();
  ...
  (SELECT CASE
   ...
   WHERE db01_coddepto=l20_codepartamento and db01_anousu=" . db_getsession("DB_anousu") . " LIMIT 1) as codUnidadeSubResp,
      ...
      INNER JOIN db_config on (liclicita.l20_instit=db_config.codigo)
      INNER JOIN liclicitemlote on (liclicitem.l21_codigo=liclicitemlote.l04_liclicitem)
      INNER JOIN aberlic112019 on (liclicitemlote.l04_descricao = aberlic112019.si47_dsclote and aberlic112019.si47_nroprocessolicitatorio = liclicita.l20_edital::varchar)
      LEFT JOIN solicitemunid AS solicitemunid ON solicitem.pc11_codigo = solicitemunid.pc17_codigo
      LEFT JOIN matunid AS matunid ON solicitemunid.pc17_unid = matunid.m61_codmatunid
      LEFT JOIN infocomplementaresinstit on db_config.codigo = infocomplementaresinstit.si09_instit
      WHERE db_config.codigo= " . db_getsession("DB_instit") . " AND liclicita.l20_tipojulg = 3
      AND liclicita.l20_codigo= $oDados10->seqlicitacao";
            ...
            $sSql = " 
SELECT DISTINCT '14' AS tipoRegistro,
                infocomplementaresinstit.si09_codorgaotce AS codOrgaoResp,
    (SELECT CASE
                WHEN o41_subunidade != 0
     ...
     WHERE db01_coddepto=l20_codepartamento
         AND db01_anousu=2019
     LIMIT 1) AS codUnidadeSubResp,
                liclicita.l20_anousu AS exercicioLicitacao,
                liclicita.l20_edital AS nroProcessoLicitatorio,
                CASE
                    WHEN liclicita.l20_tipojulg = 3 THEN aberlic112019.si47_nrolote
                    ELSE 0
                END AS nroLote,
                ...
                '0' AS vlMinAlienBens,
                CASE
                    WHEN pc80_criterioadjudicacao = 1 THEN round((sum(compraspcorcamval.pc23_perctaxadesctabela)/count(compraspcorcamval.pc23_orcamforne)),2)
                    WHEN pc80_criterioadjudicacao = 2 THEN round((sum(compraspcorcamval.pc23_percentualdesconto)/count(compraspcorcamval.pc23_orcamforne)),2)
                END AS mediapercentual,
                compraspcorcamval.pc23_vlrun
FROM  pcorcamitem
INNER JOIN pcorcamval ON pc22_orcamitem = pc23_orcamitem
INNER JOIN pcorcamforne ON pc23_orcamforne = pc21_orcamforne
INNER JOIN cgm ON z01_numcgm = pc21_numcgm
INNER JOIN pcorcamitemlic ON pc23_orcamitem = pc26_orcamitem
INNER JOIN liclicitem ON pc26_liclicitem = l21_codigo
...
LEFT JOIN aberlic112019 ON (liclicitemlote.l04_descricao = aberlic112019.si47_dsclote AND aberlic112019.si47_nroprocessolicitatorio = liclicita.l20_edital::varchar)
INNER JOIN pcprocitem compraspcprocitem on compraspcprocitem.pc81_solicitem = pc11_codigo
JOIN pcorcamitemproc ON compraspcprocitem.pc81_codprocitem = pc31_pcprocitem
JOIN pcorcamitem compraspcorcamitem ON pc31_orcamitem = compraspcorcamitem.pc22_orcamitem
JOIN pcorcamval compraspcorcamval ON compraspcorcamitem.pc22_orcamitem = compraspcorcamval.pc23_orcamitem
JOIN pcorcamforne compraspcorcamforne ON compraspcorcamforne.pc21_orcamforne = compraspcorcamval.pc23_orcamforne
LEFT JOIN precoreferencia ON (pcproc.pc80_codproc = precoreferencia.si01_processocompra)
LEFT JOIN itemprecoreferencia ON (precoreferencia.si01_sequencial = itemprecoreferencia.si02_precoreferencia AND compraspcorcamval.pc23_orcamitem = itemprecoreferencia.si02_itemproccompra)
WHERE db_config.codigo= " . db_getsession("DB_instit") . "
    AND pctipocompratribunal.l44_sequencial != 102
    AND liclicita.l20_codigo= $oDados10->seqlicitacao
    GROUP BY
    ...
    aberlic112019.si47_nrolote,
    pcmater.pc01_codmater,
    matunid.m61_codmatunid,
    precoreferencia.si01_datacotacao,
    itemprecoreferencia.si02_vlpercreferencia,
    itemprecoreferencia.si02_vlprecoreferencia,
    pcorcamval.pc23_quant,
    pcproc.pc80_criterioadjudicacao,
    compraspcorcamval.pc23_vlrun";
   ...
   WHERE db01_coddepto=l20_codepartamento and db01_anousu=" . db_getsession("DB_anousu") . " LIMIT 1) as codUnidadeSubResp,
            ...
            for ($iCont14 = 0; $iCont14 < pg_num_rows($rsResult14); $iCont14++) {
                ...
                if (!isset($aDadosAgrupados14[$sHash14])) {
                    ...
                    if($oDados10->criterioadjudicacao == 1 || $oDados10->criterioadjudicacao == 2){
                        $oDados14->si50_vlrefpercentual = $oResult14->mediapercentual;
                    }
                    else{
   ...
   WHERE db01_coddepto=l20_codepartamento and db01_anousu=" . db_getsession("DB_anousu") . " LIMIT 1) as codUnidadeSubResp,
    ...
    AND liclicita.l20_codigo= $oDados10->seqlicitacao";
            ...
            $sSql = "SELECT tiporegistro,
                            ...
                            naturezadespesa,
                            codfontrecursos,
                            sum(vlrecurso) AS vlrecurso
                    FROM
                        (SELECT DISTINCT '16' AS tipoRegistro,
                                         infocomplementaresinstit.si09_codorgaotce AS codOrgaoResp,
                             (SELECT CASE
                                         WHEN o41_subunidade != 0
                                              OR NOT NULL THEN lpad((CASE
                         ...
                         AND o40_anousu = o41_anousu
                         INNER JOIN orcprojativ ON orcdotacao.o58_projativ = orcprojativ.o55_projativ
                         AND orcdotacao.o58_anousu = orcprojativ.o55_anousu
                         INNER JOIN db_config ON (liclicita.l20_instit=db_config.codigo)
                         INNER JOIN orctiporec ON (orcdotacao.o58_codigo=orctiporec.o15_codigo)
                         INNER JOIN orcelemento ON (orcdotacao.o58_anousu=orcelemento.o56_anousu
                                                    AND orcdotacao.o58_codele=orcelemento.o56_codele)
                         LEFT JOIN infocomplementaresinstit ON db_config.codigo = infocomplementaresinstit.si09_instit
                         WHERE db_config.codigo= " . db_getsession("DB_instit") . "
                             AND liclicita.l20_codigo= $oDados10->seqlicitacao) AS x
