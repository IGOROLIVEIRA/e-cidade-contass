...
require_once("classes/db_cvc102019_classe.php");
require_once("classes/db_cvc202019_classe.php");
require_once("classes/db_cvc302019_classe.php");
require_once("classes/db_cvc402019_classe.php");
require_once("model/contabilidade/arquivos/sicom/mensal/geradores/2019/GerarCVC.model.php");
...
class SicomArquivoCadastroVeiculos extends SicomArquivoBase implements iPadArquivoBaseCSV
{
    ...
    public function gerarDados()
    {
         ...
         */
        $clcvc10 = new cl_cvc102019();
        $clcvc20 = new cl_cvc202019();
        $clcvc30 = new cl_cvc302019();
        $clcvc40 = new cl_cvc402019();
      ...
      SELECT DISTINCT '10' AS tipoRegistro,
                      ...
                      CASE
                          WHEN veiculostransf.ve81_codigonovo is NULL THEN veiculostransf.ve81_codigo
      ...
      INNER JOIN(select ve81_codigo, max(ve81_transferencia) ve81_transferencia, ve81_codunidadesubatual,ve81_codigonovo
                      from veiculostransferencia
                        group by ve81_codigo, ve81_codunidadesubatual, ve81_codigonovo
...
//db_criatabela($rsResult10);die($sSql);
            ...
            for ($iCont10 = 0; $iCont10 < pg_num_rows($rsResult10); $iCont10++) {
             ...
             select ve04_codigo
              from veicbaixa
                ...
                $sSqlVerifica = "select si146_sequencial from cvc102019 where si146_codveiculo = '{$oDados10->codveiculo}'
        and si146_mes <= " . $this->sDataFinal['5'] . $this->sDataFinal['6'];
                ...
                $sSqlNaoRepete = "
          select * from cvc102019 where si146_codveiculo = '{$oDados10->codveiculo}' and si146_codunidadesub = '{$oDados10->ve01_codunidadesub}'
        ";
...
//        echo $sSql; db_criatabela($rsResult20); die();
      ...
      SELECT DISTINCT '40' AS tipoRegistro,
                ...
                CASE
                    ...
                    ELSE veiculostransferencia.ve81_codigoant
                END AS codVeiculo,
      ...
      INNER JOIN db_departorg ON db01_coddepto = db_depart.coddepto
      LEFT JOIN veiculos.veicbaixa AS veicbaixa ON (veiculos.ve01_codigo=veicbaixa.ve04_veiculo)
      AND db01_anousu = " . db_getsession("DB_anousu") . "
      LEFT JOIN veiculostransferencia on ve81_codigo=ve01_codigo
      INNER JOIN orcunidade unveic ON db01_orgao = unveic.o41_orgao
      AND db01_unidade = unveic.o41_unidade
