...
class SicomArquivoAberturaLicitacao extends SicomArquivoBase implements iPadArquivoBaseCSV
{
  ...
  (SELECT CASE
   ...
   WHERE db01_coddepto=l20_codepartamento and db01_anousu=" . db_getsession("DB_anousu") . " LIMIT 1) as codUnidadeSubResp,
       ...
       CASE
            WHEN liclicita.l20_tipliticacao = 8 THEN 2
    ...
    ELSE 2
     ...
     AND pctipocompratribunal.l44_sequencial IN ('48',
        ...
        for ($iCont10 = 0; $iCont10 < pg_num_rows($rsResult10); $iCont10++) {
            ...
            $claberlic10->si46_tiporegistro = 10;//1
            $claberlic10->si46_clausulaprorrogacao = $oDados10->clausulaprorrogacao;//2
            $claberlic10->si46_codmodalidadelicitacao = $oDados10->codmodalidadelicitacao;//3
            $claberlic10->si46_codorgaoresp = $oDados10->codorgaoresp;//4
            if ($oDados10->codunidsubant != null || $oDados10->codunidsubant != '') {
                $claberlic10->si46_codunidadesubresp = $oDados10->codunidsubant;//5
                $claberlic10->si46_codunidadesubedital = $oDados10->codunidsubant;//6
            } else {
                $claberlic10->si46_codunidadesubresp = $oDados10->codunidadesubresp;//5
                $claberlic10->si46_codunidadesubedital = $oDados10->codunidadesubresp;//6
            }
            $claberlic10->si46_criterioaceitabilidade = substr($this->removeCaracteres($oDados10->criterioaceitabilidade), 0, 80);//7
            $claberlic10->si46_criteriodesempate = $oDados10->criteriodesempate;//8
            $claberlic10->si46_nroedital = $oDados10->l20_nroedital;//9
            $claberlic10->si46_exercicioedital = $oDados10->l20_exercicioedital;//10
            $claberlic10->si46_descontotabela = $oDados10->criterioadjudicacao;//11
            $claberlic10->si46_destinacaoexclusiva = $oDados10->destinacaoexclusiva;//12
            $claberlic10->si46_dtabertura = $oDados10->dtabertura;//13
            $claberlic10->si46_dteditalconvite = $oDados10->dteditalconvite;//14
            $claberlic10->si46_dtpublicacaoeditaldo = $oDados10->dtpublicacaoeditaldo;//15
            $claberlic10->si46_dtpublicacaoeditalveiculo1 = $oDados10->dtpublicacaoeditalveiculo1;//16
            $claberlic10->si46_dtpublicacaoeditalveiculo2 = $oDados10->dtpublicacaoeditalveiculo2;//17
            $claberlic10->si46_dtrecebimentodoc = $oDados10->dtrecebimentodoc;//18
            $claberlic10->si46_exerciciolicitacao = $oDados10->exerciciolicitacao;//18
            $claberlic10->si46_formapagamento = substr($this->removeCaracteres($oDados10->formapagamento), 0, 80);//20
            $claberlic10->si46_limitecontratacao = $oDados10->limitecontratacao;//21
            $claberlic10->si46_mes = $this->sDataFinal['5'] . $this->sDataFinal['6'];//22
            $claberlic10->si46_naturezaobjeto = $oDados10->naturezaobjeto;//23
            $claberlic10->si46_naturezaprocedimento = $oDados10->naturezaprocedimento;//24
            $claberlic10->si46_nroconvidado = $oDados10->nroconvidado;//25
            $claberlic10->si46_nromodalidade = $oDados10->nromodalidade;//26
            $claberlic10->si46_nroprocessolicitatorio = $oDados10->nroprocessolicitatorio;//27
            $claberlic10->si46_objeto = substr($this->removeCaracteres($oDados10->objeto), 0, 500);//28
            $claberlic10->si46_prazoexecucao = $oDados10->prazoexecucao;//29
            $claberlic10->si46_processoporlote = $oDados10->processoporlote;//30
            $claberlic10->si46_regimeexecucaoobras = $oDados10->regimeexecucaoobras;//31
            $claberlic10->si46_subcontratacao = $oDados10->subcontratacao;//32
            $claberlic10->si46_tipolicitacao = $oDados10->tipolicitacao;//33
            $claberlic10->si46_unidademedidaprazoexecucao = $oDados10->unidademedidaprazoexecucao;//34
            $claberlic10->si46_criterioadjudicacao = $oDados10->criterioadjudicacao;//35
            $claberlic10->si46_veiculo1publicacao = $this->removeCaracteres($oDados10->veiculo1publicacao);//36
            $claberlic10->si46_veiculo2publicacao = $this->removeCaracteres($oDados10->veiculo2Publicacao);//36

            $claberlic10->si46_leidalicitacao = $oDados10->lei;//38
            $claberlic10->si46_dtpulicacaopncp = $oDados10->dataPncp;//39
            $claberlic10->si46_linkpncp = $oDados10->linnPncp;//40
            $claberlic10->si46_dtpulicacaoedital = $oDados10->dataPubli;//41
            $claberlic10->si46_linkedital = $oDados10->linkPublic;//42
            $claberlic10->si46_diariooficialdivulgacao = $oDados10->divulgacaoDo;//43
            if ($oDados10->lei == 1) {
                $claberlic10->si46_mododisputa = $oDados10->mododisputa;//44
            } else {
