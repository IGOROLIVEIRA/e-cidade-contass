  ...
  public function gerarDados() {
    ...
    for ($iCont = 0;$iCont < pg_num_rows($rsVeiculos); $iCont++) {
      ...
      if ($sTrataCodUnidade == "01") {
    	...
    	/**
    	 * campo a ser comparado no reg 30
    	 */
    	$icodVeiculo = '';
    	if (($oVeiculos->ve01_dtaquis >= $this->sDataInicial && $oVeiculos->ve01_dtaquis <= $this->sDataFinal)
	    	...
	    	if ($sTipo != "03") {
	    		$oVeiculos->ve01_placa   = '';
	    		$oVeiculos->ve01_chassi  = '';
	    		$oVeiculos->ve01_ranavam = '';
	    	}
	    	...
	    	$icodVeiculo = $oDadosVeiculos->codVeiculo;
      ...
      $rsPecas = db_query($sSql);//db_criatabela($rsPecas);
    	...
    	if ($oPecas->pecas > 0) {
    		...
    		$oDadosPecas->MarcacaoInicial   = round($sMarcacaoInicial);
    		$oDadosPecas->MarcacaoFinal     = round($sMarcacaoFinal);
    		$oDadosPecas->tipoGasto         = "08";
    		$oDadosPecas->qtdeUtilizada     = number_format($oPecas->quantidade, 4, "", "");
    		...
    		for ($iCont2 = 0;$iCont2 < pg_num_rows($rsCombustivel); $iCont2++) {
    		  ...
    		  $oDadosCombustivel->MarcacaoInicial   = round($sMarcacaoInicial);
    		  $oDadosCombustivel->MarcacaoFinal     = round($sMarcacaoFinal);
    		  $oDadosCombustivel->tipoGasto         = $sTipoCombustivel;
    		  $oDadosCombustivel->qtdeUtilizada     = number_format($oCombustivel->qtd_litros, 4, "", "");
       ...
       foreach ($oCadVeiculos as $oCadVeiculo) {
    	  ...
    	  if ($oCadVeiculo->getAttribute('codVeiculo') == $oVeiculos->ve01_codigo && $icodVeiculo != '') {
    	    ...
    	    $oDadosCadVeiculo->codUnidadeSub   				   = $sCodUnidade;
	  	    ...
	  	    $oDadosCadVeiculo->localidade         			 = utf8_encode(substr($oCadVeiculo->getAttribute("localidade"), 0, 250));
