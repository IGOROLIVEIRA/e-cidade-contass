...
require_once("classes/db_homolic202022_classe.php");
require_once("classes/db_homolic302022_classe.php");
require_once("classes/db_homolic402022_classe.php");
require_once("model/contabilidade/arquivos/sicom/SicomArquivoBase.model.php");
require_once("model/contabilidade/arquivos/sicom/mensal/geradores/2022/GerarHOMOLIC.model.php");
...
class SicomArquivoHomologacaoLicitacao extends SicomArquivoBase implements iPadArquivoBaseCSV
{
	...
	public function gerarDados()
	{
		...
		$clhomolic10 = new cl_homolic102022();
		$clhomolic20 = new cl_homolic202022();
		$clhomolic30 = new cl_homolic302022();
		$clhomolic40 = new cl_homolic402022();
       ...
       WHERE db01_coddepto=l20_codepartamento and db01_anousu=" . db_getsession("DB_anousu") . " LIMIT 1) as codUnidadeSubResp,
    	...
    	CASE WHEN liclicita.l20_tipojulg = 3 THEN aberlic112022.si47_nrolote ELSE 0 END AS nroLote,
       ...
       CASE
    	...
    	LEFT JOIN aberlic112022 on (liclicitemlote.l04_descricao = aberlic112022.si47_dsclote  and aberlic112022.si47_nroprocessolicitatorio = liclicita.l20_edital::varchar)
			...
			if($oResult10->criterioadjudicacao != 2 && $oResult10->criterioadjudicacao != 1){
				...
				if (!$aDadosAgrupados[$sHash]) {
					...
					if($oResult10->codunidsubant!= null || $oResult10->codunidsubant!=''){
						$oDados10->si63_codunidadesub = $oResult10->codunidsubant;    
					}else{
			...
			if($oDados20->criterioadjudicacao == 1){
				...
				if(!$aDadosAgrupados20[$sHash20]){
					...
					if($oDados20->codunidsubant!= null || $oDados20->codunidsubant!=''){
						$clhomolic20->si64_codunidadesub = $oDados20->codunidsubant;    
					}else{
		...
		for ($iCont30 = 0; $iCont30 < pg_num_rows($rsResult30); $iCont30++) {
			...
			if($oDados30->criterioadjudicacao == 2){
				...
				if(!$aDadosAgrupados30[$sHash30]){
					...
					if($oDados30->codunidsubant!= null || $oDados30->codunidsubant!=''){
						$clhomolic30->si65_codunidadesub = $oDados30->codunidsubant;    
					}else{
   ...
   WHERE db01_coddepto=l20_codepartamento and db01_anousu=" . db_getsession("DB_anousu") . " LIMIT 1) as codUnidadeSubResp,
	...
	AND DATE_PART('YEAR',homologacaoadjudica.l202_datahomologacao) =" . db_getsession("DB_anousu") . "
		...
		for ($iCont40 = 0; $iCont40 < pg_num_rows($rsResult40); $iCont40++) {
			...
			if(!$aDadosAgrupados40[$sHash40]){
				...
				if($oDados40->codunidsubant!= null || $oDados40->codunidsubant!=''){
					$clhomolic40->si65_codunidadesub = $oDados40->codunidsubant;    
				}else{
		...
		foreach ($aDadosAgrupados as $oDadosAgrupados) {
			...
			if($oDadosAgrupados->codunidsubant!= null || $oDadosAgrupados->codunidsubant!=''){
				$clhomolic10->si63_codunidadesub = $oDadosAgrupados->codunidsubant;    
			}else{
		...
		foreach ($aDadosAgrupados20 as $oDadosAgrupados) {
			...
			if($oDadosAgrupados->codunidsubant!= null || $oDadosAgrupados->codunidsubant!=''){
				$clhomolic20->si64_codunidadesub = $oDadosAgrupados->codunidsubant;    
			}else{
		...
		foreach ($aDadosAgrupados30 as $oDadosAgrupados) {
			...
			if($oDadosAgrupados->codunidsubant!= null || $oDadosAgrupados->codunidsubant!=''){
				$clhomolic30->si65_codunidadesub = $oDadosAgrupados->codunidsubant;    
			}else{
		...
		foreach ($aDadosAgrupados40 as $oDadosAgrupados) {
			...
			if($oDadosAgrupados->codunidsubant!= null || $oDadosAgrupados->codunidsubant!=''){
				$clhomolic40->si65_codunidadesub = $oDadosAgrupados->codunidsubant;    
			}else{
