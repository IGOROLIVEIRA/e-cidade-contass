...
require_once ("classes/db_caixa102014_classe.php");
require_once ("classes/db_caixa112014_classe.php");
require_once ("classes/db_caixa122014_classe.php");
require_once ("model/contabilidade/arquivos/sicom/mensal/geradores/2014/GerarCAIXA.model.php");
...
class SicomArquivoCaixa extends SicomArquivoBase implements iPadArquivoBaseCSV {
  ...
  public function gerarDados() {
    ...
    $clcaixa122014 = new cl_caixa122014();
    $clcaixa112014 = new cl_caixa112014();
    $clcaixa102014 = new cl_caixa102014();
         ...
         */
	    db_inicio_transacao();
	    $result = $clcaixa102014->sql_record($clcaixa102014->sql_query(NULL,"*",NULL,"si103_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']." 
	     and si103_instit = ".db_getsession("DB_instit")));
	    if (pg_num_rows($result) > 0) {
	    	
	    	$clcaixa122014->excluir(NULL,"si105_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']." 
	    	and si105_instit = ".db_getsession("DB_instit"));
	    	$clcaixa112014->excluir(NULL,"si104_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']." 
	    	and si104_instit = ".db_getsession("DB_instit"));
	    	$clcaixa102014->excluir(NULL,"si103_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']." 
	    	and si103_instit = ".db_getsession("DB_instit"));
	   
	      if ($clcaixa102014->erro_status == 0) {
	    	  throw new Exception($clcaixa102014->erro_msg);
	      }
	    }
	    $result = $clcaixa102014->sql_record($clcaixa102014->sql_query(NULL,"*",NULL,"si103_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']." 
	     and si103_instit = ".db_getsession("DB_instit")));
    ...
    for ($iCont = 0;$iCont < pg_num_rows($rsContasCaixa); $iCont++) {
        ...
        db_inicio_transacao();
    	  ...
    	  $oDadosCaixa = new cl_caixa102014();
    	...
    	$sSql = "select  11 as tiporegistro,
    		...
    		for ($iCont2 = 0; $iCont2 < pg_num_rows($rsMovi); $iCont2++) {
    		  ...
    		  $oMovi  = db_utils::fieldsMemory($rsMovi, $iCont2);
    		    ...
    		    $sSqlCodCtb = "select distinct si95_codctb,si96_codfontrecursos  
							from conplanoreduz
							join conplanocontabancaria on conplanoreduz.c61_codcon = conplanocontabancaria.c56_codcon 
							and conplanoreduz.c61_anousu = conplanocontabancaria.c56_anousu
							join conplanoconta on conplanocontabancaria.c56_codcon = conplanoconta.c63_codcon
							...
							join ctb102014 on si95_banco = c63_banco and 
							 si95_agencia  = c63_agencia and 
							 si95_digitoverificadoragencia = c63_dvagencia and
							 si95_contabancaria::varchar = c63_conta and 
							 si95_digitoverificadorcontabancaria = c63_dvconta and 
							 si95_tipoconta::integer = (case when db83_tipoconta in (2,3) then 2 else 1 end)
							 join ctb202014 on ctb102014.si95_codctb = ctb202014.si96_codctb
							where c61_reduz = ".$oMovi->codctbtransf;
    		    $rsCodCtb = db_query($sSqlCodCtb);//db_criatabela($rsCodCtb);

    		    $oMovi->codctbtransf      = db_utils::fieldsMemory($rsCodCtb, 0)->si95_codctb;
    		    $oMovi->codfontectbtransf = db_utils::fieldsMemory($rsCodCtb, 0)->si96_codfontrecursos;
    		...
    		$sSql ="select 12 as tiporegistro,
					...
					left join orcreceita on c74_codrec = o70_codrec and o70_anousu = 2014     
					left join orcfontes on o70_codfon = o57_codfon and o70_anousu = o57_anousu
    		 ...
    		 */
		    		 ...
		    		 $sHash12 = $aDadosAgrupados[$sHash]->si104_codreduzido.$oRecita->ededucaodereceita.$oRecita->identificadordeducao.$oRecita->naturezareceita;
			    	 if (!isset($aDadosAgrupados[$sHash]->registro12[$sHash12])) {
			    	  ...
			    	  $aDadosAgrupados[$sHash]->registro12[$sHash12] = $oDadosReceita;
		    		  
			    	 } else {
			    	  $aDadosAgrupados[$sHash]->registro12[$sHash12]->si105_vlrreceitacont += $oRecita->vlrreceitacont;	
			    	 }
    		...
    		foreach ($aDadosAgrupados as $mov) {
    			...
    			$clcaixa112014->si104_tiporegistro            = $mov->si104_tiporegistro;
	    		$clcaixa112014->si104_codreduzido             = $mov->si104_codreduzido;
	    		$clcaixa112014->si104_tipomovimentacao        = $mov->si104_tipomovimentacao;
	    		$clcaixa112014->si104_tipoentrsaida           = $mov->si104_tipoentrsaida;
	    		$clcaixa112014->si104_descrmovimentacao       = $mov->si104_descrmovimentacao;
	    		$clcaixa112014->si104_valorentrsaida          = abs($mov->si104_valorentrsaida);
	    		$clcaixa112014->si104_codctbtransf            = $mov->si104_codctbtransf;
	    		$clcaixa112014->si104_codfontectbtransf  	  = $mov->si104_codfontectbtransf;
	    		$clcaixa112014->si104_mes  				      = $mov->si104_mes;
	    		$clcaixa112014->si104_reg10  			      = $mov->si104_reg10;
	    		$clcaixa112014->si104_instit                  = db_getsession("DB_instit");
	    		    			
    			$clcaixa112014->incluir(null);
    			
    		    if ($clcaixa112014->erro_status == 0) {
					throw new Exception($clcaixa112014->erro_msg);
				}
    		    ...
    		    if($mov->si104_tipoentrsaida == 1){
    				...
    				foreach ($mov->registro12 as $reg12){
    					...
    					$clcaixa122014->si105_tiporegistro                      = $reg12->si105_tiporegistro;
				    	$clcaixa122014->si105_codreduzido                       = $reg12->si105_codreduzido;
				    	$clcaixa122014->si105_ededucaodereceita                 = $reg12->si105_ededucaodereceita;
				    	$clcaixa122014->si105_identificadordeducao              = $reg12->si105_identificadordeducao;
				    	$clcaixa122014->si105_naturezareceita                   = $reg12->si105_naturezareceita;
				    	$clcaixa122014->si105_vlrreceitacont                    = $reg12->si105_vlrreceitacont;
				    	$clcaixa122014->si105_mes                               = $reg12->si105_mes;
				    	$clcaixa122014->si105_reg10                             = $oDadosCaixa->si103_sequencial;
				    	$clcaixa122014->si105_instit                            = db_getsession("DB_instit");
				    		
				    	$clcaixa122014->incluir(null);
