...
require_once ("classes/db_ctb102014_classe.php");
require_once ("classes/db_ctb202014_classe.php");
require_once ("classes/db_ctb212014_classe.php");
require_once ("classes/db_ctb222014_classe.php");
require_once ("classes/db_ctb302014_classe.php");
require_once ("classes/db_ctb312014_classe.php");
require_once ("classes/db_ctb402014_classe.php");
require_once ("classes/db_ctb412014_classe.php");
require_once ("classes/db_ctb502014_classe.php");
require_once ("classes/db_ctb602014_classe.php");
require_once ("model/contabilidade/arquivos/sicom/mensal/geradores/2014/GerarCTB.model.php");
...
class SicomArquivoContasBancarias extends SicomArquivoBase implements iPadArquivoBaseCSV {
  ...
  public function gerarDados() {
  	...
  	$cCtb10 = new cl_ctb102014();
	$cCtb20 = new cl_ctb202014();
	$cCtb21 = new cl_ctb212014();
	$cCtb22 = new cl_ctb222014();
	$cCtb30 = new cl_ctb302014();
	$cCtb31 = new cl_ctb312014();
	$cCtb40 = new cl_ctb402014();
	$cCtb41 = new cl_ctb412014();
	$cCtb50 = new cl_ctb502014();
   ...
   /**
    ...
    $sSqlContaJainformada ="select si95_codorgao||si95_banco||si95_agencia||si95_digitoverificadoragencia||si95_contabancaria
          					 ...
          					 from ctb102014 where si95_mes < ".$this->sDataFinal['5'].$this->sDataFinal['6'];
      ...
      if($oRegistro10->si09_tipoinstit != 5){
      	...
      	if(!isset($aBancosAgrupodos[$aHash])){
            ...
            $cCtb10->contas								  = array();
      	    ...
      	    $sSqlVerifica = "SELECT * FROM ctb102014 WHERE si95_codorgao = '$oRegistro10->si09_codorgaotce' AND si95_banco = '$oRegistro10->c63_banco' 
    ...
    //echo "<pre>";print_r($aBancosAgrupodos);
    foreach ($aBancosAgrupodos as $oContaAgrupada) {
    	...
    	foreach ($oContaAgrupada->contas as $nConta){
    		...
    		if($this->sDataFinal['5'].$this->sDataFinal['6'] == 1){
	       ...
	       if( !isset($oCtb20->si96_codctb) ){
			    ...
			    $oCtb20->si96_vlsaldofinalfonte    	= ($nSaldoInicial + $nTotalMov);
	       ...
	       $sSqlReg21="select  21 as tiporegistro,
    		...
    		if(pg_num_rows($rsMovi21) != 0){
    			...
    			for ($iCont21 = 0; $iCont21 < pg_num_rows($rsMovi21); $iCont21++) {
			    	...
			    	/*
			    	 * Quando codctbtransf for diferente de nada e de zero este movimento e uma transferencia bancaria
			    	 */
			    	$iCodSis     = 0;
			    	$conta       = 0;
			    	$contaTransf = 0;
			    	...
			    	if($oMovi->codctbtransf != 0 && $oMovi->codctbtransf != ''){ 
			    		...
			    		//echo $sqlcontatransf;db_criatabela($rsConta);
			    		if (pg_num_rows($rsConta) == 0) {
			    			...
			    			$rsCodSis = db_query($sSql);//db_criatabela($rsCodSis);
			    			/**
			    		  ...
			    		  $conta   = $aBancosAgrupodos[$contaTransf]->si95_codctb;
			    		  //$recurso = db_utils::fieldsMemory($rsConta, 0)->o15_codtri;
			    		  $recurso = $aBancosAgrupodos[$contaTransf]->recurso;
			    		 // echo "CONTA TRANSF".$conta." - ".$recurso;
			    	...
			    	$sHash .= ($iCodSis == 5 ? '99' : $oMovi->tipoentrsaida);
			    	$sHash .= ( (($oMovi->tipoentrsaida == 5 || $oMovi->tipoentrsaida == 6 || $oMovi->tipoentrsaida == 7 || $oMovi->tipoentrsaida == 9) && ($iCodSis != 5)) ? $conta : 0);
			    	$sHash .= ( (($oMovi->tipoentrsaida == 5 || $oMovi->tipoentrsaida == 6 || $oMovi->tipoentrsaida == 7 || $oMovi->tipoentrsaida == 9) && ($iCodSis != 5)) ? $recurso : 0);
			    	...
			    	if (!isset($oCtb20->ext21[$sHash])) {
				    		  ...
				    		  $oDadosMovi21->si97_tipoentrsaida        = $iCodSis == 5 ? '99' : $oMovi->tipoentrsaida;
				    		  $oDadosMovi21->si97_valorentrsaida       = $nValor;
				    		  $oDadosMovi21->si97_codctbtransf  	   = ($oDadosMovi21->si97_tipoentrsaida == 5 || $oDadosMovi21->si97_tipoentrsaida == 6 || $oDadosMovi21->si97_tipoentrsaida == 7 || $oDadosMovi21->si97_tipoentrsaida == 9) ? $conta : 0;
				    		  $oDadosMovi21->si97_codfontectbtransf    = ($oDadosMovi21->si97_tipoentrsaida == 5 || $oDadosMovi21->si97_tipoentrsaida == 6 || $oDadosMovi21->si97_tipoentrsaida == 7 || $oDadosMovi21->si97_tipoentrsaida == 9) ? $recurso : 0;
    	...
    	//echo "<pre>";print_r($oCtb20);
			    ...
			    foreach ($oCtb20->ext21 as $oCtb21agrupado){
			    	//echo "<pre>"; print_r($oCtb21agrupado);
			    	if ($oCtb21agrupado->si97_codctb == $oCtb21agrupado->si97_codctbtransf) {
			    		continue;
			    	}
			    	 $cCtb21 = new cl_ctb212014();
