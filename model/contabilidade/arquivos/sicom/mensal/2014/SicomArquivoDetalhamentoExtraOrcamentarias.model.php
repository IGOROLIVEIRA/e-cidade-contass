...
require_once ("classes/db_ext102014_classe.php");
require_once ("classes/db_ext202014_classe.php");
require_once ("classes/db_ext212014_classe.php");
require_once ("classes/db_ext222014_classe.php");
require_once ("classes/db_ext232014_classe.php");

require_once ("model/contabilidade/arquivos/sicom/mensal/geradores/2014/GerarEXT.model.php");
...
class SicomArquivoDetalhamentoExtraOrcamentarias extends SicomArquivoBase implements iPadArquivoBaseCSV {
  ...
  public function gerarDados() {
  	...
  	$cExt10 = new cl_ext102014();
  	$cExt20 = new cl_ext202014();
  	$cExt21 = new cl_ext212014();
  	$cExt22 = new cl_ext222014();
  	$cExt23 = new cl_ext232014();
  	/*
  	    ...
  	    $sSqlExt = "select 10 as tiporegistro,c61_codcon, 
					  ...
					  where o41_instit = ".db_getsession("DB_instit")." and o40_anousu = ".db_getsession("DB_anousu")." order by o40_orgao limit 1) as codUnidadeSub,
				       ...
				       case when c60_tipolancamento = 1 and c60_subtipolancamento not in (1,2,3,4) then c61_reduz
				            ...
				            when c60_tipolancamento = 99 and c60_subtipolancamento = 9999 then c61_reduz
    ...
    $by   = array('','','', 'a','a','a','a','a','e','e','e','e','i','i','i','o','o','o','o','o','u','u','u','u','A','A','A','E','I','O','U','n','n','c','C',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ' );
	    ...
	    for ($iContador = 0;$iContador < pg_num_rows($rsContasExtra); $iContador++) {
	        ...
	        for ($iContPlano = 0; $iContPlano < pg_num_rows($rsPlanoContasSaldo);$iContPlano++) {
		      	...
		      	if (db_utils::fieldsMemory($rsPlanoContasSaldo, $iContPlano)->c61_reduz != 0) {
		      	  ...
		      	  if($oPlanoContas->sinal_anterior == 'D'){
		      	  	$oSaldoInicioFim->sdini = $oPlanoContas->saldo_anterior * -1;	
		      	  }else{
		      	  	$oSaldoInicioFim->sdini = $oPlanoContas->saldo_anterior;
		      	  }
		      	  
		      	  if($oPlanoContas->sinal_final == 'D'){
		      	  	$oSaldoInicioFim->sdfim = $oPlanoContas->saldo_final * -1; 	
		      	  }else{
	    ...
	    for ($iCont10 = 0;$iCont10 < pg_num_rows($rsContasExtra); $iCont10++) {
	       ...
	       if (pg_num_rows($result) == 0) {*/
	       		...
	       		if(!isset($aExt10Agrupodo[$aHash])){
		       		...
		       		$cExt10->si124_subtipo 			= substr($oContaExtra->subtipo,0,4);
		       		...
		       		$sSqlVerifica = "SELECT * FROM ext102014 WHERE si124_codorgao = '$oContaExtra->codorgao' AND si124_codunidadesub = '$oContaExtra->codunidadesub'
		       		AND si124_tipolancamento = '$oContaExtra->tipolancamento' AND si124_subtipo = '$oContaExtra->subtipo' 
		       		AND si124_desdobrasubtipo = '$oContaExtra->desdobrasubtipo' and si124_mes < ".$this->sDataFinal['5'].$this->sDataFinal['6'];
				    ...
				    $cExt10->extras[]= $cExt10->si124_codext;
	    ...
	    foreach ($aExt10Agrupodo as $oExt10Agrupado) {
			...
			$sSql21 = "select   '21' as tiporegitro,
					...
					/*if ( in_array($oExt10Agrupado->si124_codext, array(3152,4945,4951) )) {
						echo $sSql21; db_criatabela($rsExt21);	
					}*/
					...
					for($linha = 0; $linha < pg_num_rows($rsExt21); $linha++){
						 ...
						 //if ($oExt10Agrupado->si124_codext == 2103) echo $oExt21->codext." - ".$oExt21->codreduzidomov." - ".$Hash."<BR>";
