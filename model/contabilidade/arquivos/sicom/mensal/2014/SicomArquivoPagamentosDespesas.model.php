...
require_once ("classes/db_ops102014_classe.php");
require_once ("classes/db_ops112014_classe.php");
require_once ("classes/db_ops122014_classe.php");
require_once ("classes/db_ops132014_classe.php");

require_once ("model/contabilidade/arquivos/sicom/mensal/geradores/2014/GerarOPS.model.php");
 /**
...
class SicomArquivoPagamentosDespesas extends SicomArquivoBase implements iPadArquivoBaseCSV {
  ...
  public function gerarDados() {
    ...
    $clops10 = new cl_ops102014();
    $clops11 = new cl_ops112014();
    $clops12 = new cl_ops122014();
    $clops13 = new cl_ops132014();
    ...
    join conlancamord on c71_codlan =  c80_codlan join conlancam on c70_codlan = c71_codlan where c53_tipo in (21,20) and c80_codord = {$oEmpPago->ordem}";
    	...
    	if (db_utils::fieldsMemory($rsQuantExtornos, 0)->valor > 0) {
			...
			/**
			 * PRIMEIRA VEZ DO LOOP ONDE NÃO EXISTE NENHUM LANÇAMENTO DE PAGAMENTO REGISTRADO NO ARQUIVO
			 */
			if(!isset($aInformado[$sHash])){	
			        ...
			        //echo $sSql11;db_criatabela($rsPagOrd11);
			     ...
			     if (pg_num_rows($rsPagOrd11) > 0) {
		      	 ...
		      	 */
			        ...
			        if(pg_num_rows($rsPagOrd12) > 0 && $reg12->codctb != ''){
						...
						$clops12->si134_tipodocumentoop = $reg12->tipodocumentoop;
						$clops12->si134_nrodocumento    = $reg12->nrodocumento;
						$clops12->si134_codctb 			= $reg12->tipodocumentoop == 99 ? '' : $ContaPag; 
						$clops12->si134_codfontectb 	= $reg12->tipodocumentoop == 99 ? '' : $FontContaPag;
						$clops12->si134_dtemissao 		= $reg12->dtemissao;
						$clops12->si134_vldocumento 	= $nVolorOp;
						$clops12->si134_mes 			= $this->sDataFinal['5'].$this->sDataFinal['6'];
				        $clops12->si134_reg10			= $clops10->si132_sequencial;
