...
require_once ("classes/db_emp102014_classe.php");
require_once ("classes/db_emp112014_classe.php");
require_once ("model/contabilidade/arquivos/sicom/mensal/geradores/2014/GerarEMP.model.php");
		    ...
		    $result = $cEmp102014->sql_record($cEmp102014->sql_query(NULL,"*",NULL,"si106_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6'])
		     ." and si106_instit = ".db_getsession("DB_instit"));
		      ...
		      if (pg_num_rows($result) > 0) {
		    	...
		    	$cEmp112014->excluir(NULL,"si107_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']
		    	." and si107_instit = ".db_getsession("DB_instit"));
		    	$cEmp102014->excluir(NULL,"si106_mes = ".$this->sDataFinal['5'].$this->sDataFinal['6']
		    	." and si106_instit = ".db_getsession("DB_instit"));
		      if ($cEmp102014->erro_status == 0) {
		...
		for ($iCont = 0; $iCont < pg_num_rows($rsEmpenho); $iCont++) {
	     ...
	     /* if ($oEmpenho->exercicioprocessolicitatorio < 2014) {
          ...
          db_inicio_transacao();
			...
			$oDadosEmpenho->si106_idsubacao					           = is_numeric($oEmpenho->idsubacao) ? $oEmpenho->idsubacao : '';
