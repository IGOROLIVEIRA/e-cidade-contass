...
class GerarHABLIC extends GerarAM {
  ...
  */
  ...
  public function gerarDados() {
    ...
    $sSql = "select * from hablic102014 where si57_mes = ". $this->iMes." and si57_instit=".db_getsession("DB_instit");
    $rsHABLIC10    = db_query($sSql);

    $sSql2 = "select * from hablic112014 where si58_mes = ". $this->iMes." and si58_instit=".db_getsession("DB_instit");
    $rsHABLIC11    = db_query($sSql2);

    $sSql3 = "select * from hablic202014 where si59_mes = ". $this->iMes." and si59_instit=".db_getsession("DB_instit");
  ...
  if (pg_num_rows($rsHABLIC10) == 0 && pg_num_rows($rsHABLIC20) == 0) {
      ...
      */
      for ($iCont = 0;$iCont < pg_num_rows($rsHABLIC10); $iCont++) {
        ...
        $aCSVHABLIC10['si57_orgaorespregistro']                          =   str_pad($aHABLIC10['si57_orgaorespregistro'], 1, "0", STR_PAD_LEFT);
        ...
        $aCSVHABLIC10['si57_ufinscricaoestadual']                        =   str_pad($aHABLIC10['si57_ufinscricaoestadual'], 2, "0", STR_PAD_LEFT);
