...
class GerarAFAST extends GerarAM {
	...
	public function gerarDados() {
        ...
        $sSql     = "select * from afast102024 where si199_mes = {$this->iMes} and si199_inst = ".db_getsession("DB_instit");
        $rsAFAST    = db_query($sSql); //db_criatabela($rsAFAST);

        $sSql2     = "select * from afast202024 where si200_mes = {$this->iMes} and si200_inst = ".db_getsession("DB_instit");
        $rsAFAST2    = db_query($sSql2);

//        $sSql3delete = "delete from afast302024 where si201_mes = {$this->iMes}";
//        db_query($sSql3delete);

        $sSql3 = "select * from afast302024 where si201_mes = {$this->iMes} and si201_inst = ".db_getsession("DB_instit");
