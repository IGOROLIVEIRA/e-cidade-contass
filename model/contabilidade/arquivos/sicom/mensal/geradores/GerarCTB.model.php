...
class GerarCTB extends GerarAM {
  ...
  */
  ...
  public function gerarDados() {
    ...
    $sSql = "select * from ctb102014 where si95_mes = ". $this->iMes ." and si95_instit = ". db_getsession("DB_instit");
    $rsCTB10    = db_query($sSql);

    $sSql2 = "select * from ctb202014 where si96_mes = ". $this->iMes ." and si96_instit = ". db_getsession("DB_instit");
    $rsCTB20    = db_query($sSql2);

    $sSql3 = "select * from ctb212014 where si97_mes = ". $this->iMes ." and si97_instit = ". db_getsession("DB_instit");
    $rsCTB21    = db_query($sSql3);

    $sSql4 = "select * from ctb222014 where si98_mes = ". $this->iMes ." and si98_instit = ". db_getsession("DB_instit");
    $rsCTB22    = db_query($sSql4);

    $sSql5 = "select * from ctb302014 where si99_mes = ". $this->iMes ." and si99_instit = ". db_getsession("DB_instit");
    $rsCTB30    = db_query($sSql5);

    $sSql6 = "select * from ctb312014 where si100_mes = ". $this->iMes ." and si100_instit = ". db_getsession("DB_instit");
    $rsCTB31    = db_query($sSql6);

    $sSql7 = "select * from ctb402014 where si101_mes = ". $this->iMes ." and si101_instit = ". db_getsession("DB_instit");
    $rsCTB40    = db_query($sSql7);

    $sSql8 = "select * from ctb502014 where si102_mes = ". $this->iMes ." and si102_instit = ". db_getsession("DB_instit");
  ...
  if (pg_num_rows($rsCTB10) == 0 && pg_num_rows($rsCTB20) == 0 && pg_num_rows($rsCTB30) == 0 && pg_num_rows($rsCTB40) == 0 && pg_num_rows($rsCTB50) == 0) {
      ...
      * Registros 50      */
      for ($iCont8 = 0;$iCont8 < pg_num_rows($rsCTB50); $iCont8++) {
        ...
        $aCSVCTB50['si102_dataencerramento']         =   implode("", array_reverse(explode("-", $aCTB50['si102_dataencerramento'])));
