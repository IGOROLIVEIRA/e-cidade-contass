...
class GerarDDC extends GerarAM
{
    ...
    public function gerarDados()
    {
        ...
        $sSql = "select * from ddc102016 where si150_mes = " . $this->iMes." and si150_instit = " . db_getsession("DB_instit");
        $rsDDC10 = db_query($sSql);

        $sSql2 = "select * from ddc202016 where si153_mes = " . $this->iMes." and si153_instit = " . db_getsession("DB_instit");
        $rsDDC20 = db_query($sSql2);

        $sSql3 = "select * from ddc302016 where si154_mes = " . $this->iMes." and si154_instit = " . db_getsession("DB_instit");
        $rsDDC30 = db_query($sSql3);

        $sSql4 = "select * from ddc402016 where si178_mes = " . $this->iMes." and si178_instit = " . db_getsession("DB_instit");
        ...
        if (pg_num_rows($rsDDC10) == 0 && pg_num_rows($rsDDC20) == 0 && pg_num_rows($rsDDC30) == 0 && pg_num_rows($rsDDC40) == 0) {
            ...
            for ($iCont4 = 0; $iCont4 < pg_num_rows($rsDDC40); $iCont4++) {
                ...
                $aCSVDDC40['si178_tiporegistro']    = str_pad($aDDC40['si178_tiporegistro'], 2, "0", STR_PAD_LEFT);
                $aCSVDDC40['si178_codorgao']        = str_pad($aDDC40['si178_codorgao'], 2, "0", STR_PAD_LEFT);
                $aCSVDDC40['si178_passivoatuarial'] = $aDDC40['si178_passivoatuarial'];
                $aCSVDDC40['si178_vlsaldoanterior'] = number_format($aDDC40['si178_vlsaldoanterior'], 2, ",", "");
                $aCSVDDC40['si178_vlsaldoatual']    = number_format($aDDC40['si178_vlsaldoatual'], 2, ",", "");
