...
class GerarEXT extends GerarAM
{
  ...
  public function gerarDados()
  {
    ...
    $sSql = "select * from ext102023 where si124_mes = " . $this->iMes . " and  si124_instit = " . db_getsession("DB_instit");
    $rsEXT10 = db_query($sSql);
    
    $sSql = "select * from ext202023 where si165_mes = " . $this->iMes . " and  si165_instit = " . db_getsession("DB_instit");
    $rsEXT20 = db_query($sSql);
    
    $sSql3 = "select * from EXT302023 where si126_mes = " . $this->iMes . " and  si126_instit = " . db_getsession("DB_instit");
    $rsEXT30 = db_query($sSql3);
    
    $sSql4 = "select * from EXT312023 where si127_mes = " . $this->iMes . " and  si127_instit = " . db_getsession("DB_instit");
    $rsEXT31 = db_query($sSql4);
    
    $sSql5 = "select * from EXT322023 where si128_mes = " . $this->iMes . " and  si128_instit = " . db_getsession("DB_instit");
    ...
    if (pg_num_rows($rsEXT10) == 0 && pg_num_rows($rsEXT20) == 0) {
      ...
      for ($iCont = 0; $iCont < pg_num_rows($rsEXT20); $iCont++) {
        ...
        $bCorrecaoFonte     = ($bFonteEncerrada && $aEXT20['si165_mes'] == '01' && db_getsession("DB_anousu") == 2023);
        ...
        if($aEXT20['si165_vlsaldoatualfonte'] == 0 && !$bFonteEncerrada && !$bCorrecaoFonte){
          ...
          $result = $clconplano->sql_record($clconplano->sql_query(null, null, "CASE WHEN c60_tipolancamento = 3 THEN 'D' WHEN c60_tipolancamento = 4 AND (substr(c60_estrut, 1, 1)) = '3' THEN 'D' ELSE 'C' END AS c60_naturezasaldo ", "", " c61_anousu = 2023 and c61_codtce = ".$aEXT20['si165_codext']." and c60_codsis = 7"));
