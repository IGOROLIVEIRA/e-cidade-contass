...
class GerarNTF extends GerarAM {
  ...
  */
  ...
  public function gerarDados() {
    ...
    $sSql = "select * from ntf102014 where si143_mes = ". $this->iMes ." and si143_instit = ". db_getsession("DB_instit");
    $rsNTF10    = db_query($sSql);

    $sSql2 = "select * from ntf112014 where si144_mes = ". $this->iMes ." and si144_instit = ". db_getsession("DB_instit");
    $rsNTF11    = db_query($sSql2);

    $sSql3 = "select * from ntf122014 where si145_mes = ". $this->iMes ." and si145_instit = ". db_getsession("DB_instit");
    $rsNTF12    = db_query($sSql3);
  ...
  if (pg_num_rows($rsNTF10) == 0) {
      ...
      */
      for ($iCont = 0;$iCont < pg_num_rows($rsNTF10); $iCont++) {
        ...
        $aCSVNTF10['si143_nfnumero']                   =   substr($aNTF10['si143_codnotafiscal'], 0, 20);
        ...
        for ($iCont3 = 0;$iCont3 < pg_num_rows($rsNTF12); $iCont3++) {        

          $aNTF12  = pg_fetch_array($rsNTF12,$iCont3);
          
          if ($aNTF10['si143_sequencial'] == $aNTF12['si145_reg10']) {

            $aCSVNTF12['si145_tiporegistro']             =    str_pad($aNTF12['si145_tiporegistro'], 2, "0", STR_PAD_LEFT);
            $aCSVNTF12['si145_codnotafiscal']            =    substr($aNTF12['si145_codnotafiscal'], 0, 15);
            $aCSVNTF12['si145_codunidadesub']            =    $aNTF12['si145_codunidadesub'];
            $aCSVNTF12['si145_dtempenho']                =    implode("", array_reverse(explode("-", $aNTF12['si145_dtempenho'])));
            $aCSVNTF12['si145_nroempenho']               =    substr($aNTF12['si145_nroempenho'], 0, 22);
            $aCSVNTF12['si145_dtliquidacao']             =    implode("", array_reverse(explode("-", $aNTF12['si145_dtliquidacao'])));
            $aCSVNTF12['si145_nroliquidacao']            =    substr($aNTF12['si145_nroliquidacao'], 0, 22);
            
            $this->sLinha = $aCSVNTF12;
            $this->adicionaLinha();
