...
class SicomArquivoDetalhamentoExtraOrcamentarias extends SicomArquivoBase implements iPadArquivoBaseCSV {
  ...
  public function gerarDados() {
    ...
    $sSql = "select k17_data,k17_codigo, k17_dtaut, k17_valor, c60_estrut,k153_slipoperacaotipo,k17_debito,k17_credito,k17_texto
  			  from  slip  
	...
	for ($iCont = 0;$iCont < pg_num_rows($rsReceitasDesp); $iCont++) {
   	...
   	$sHash  = substr($oReceitasDesp->k17_data, 0, 4);
		$sHash .= $sOrgao."2".$sTipoLancamento.$sSubTipo.$desdobraSubTipo;
		$sHash .= implode(array_reverse(explode("-", $oReceitasDesp->k17_data)));
		$codReduzidoEOReg10 = $oReceitasDesp->k17_codigo.substr($oReceitasDesp->k17_data, 0, 4);
		...
		if (!isset($aDadosAgrupados[$sHash])) {
		  ...
		  $oDadosReceitasDesp->codReduzidoEO                   = $codReduzidoEOReg10;
		  ...
		  $oDadosReceitasDesp->dtLancamento                    = implode(array_reverse(explode("-", $oReceitasDesp->k17_data)));
		  $oDadosReceitasDesp->descExtraOrc                    = substr($oReceitasDesp->k17_texto, 0, 50);
		  ...
		  $oDetalhaFonte = db_utils::fieldsMemory($rsDetalhaFonte,$iCont2);
			...
			$sHasReg11  = $sOrgao.$oDetalhaFonte->o15_codtri;
		        ...
		        $oDadosDetalhaFonte->codReduzidoEO                   = $aDadosAgrupados[$sHash]->codReduzidoEO;
		   	    $oDadosDetalhaFonte->codFontRecursos                 = substr($oDetalhaFonte->o15_codtri, 0, 3);
	    ...
	    $sSql12 = "select sp.k17_texto,sp.k17_data,sp.k17_situacao,sp.k17_codigo, sp.k17_dtanu, sp.k17_valor,c60_estrut,k153_slipoperacaotipo,sp.k17_debito,sp.k17_credito,sp.k17_dtaut ,z01_cgccpf,z01_nome
					  from slip sp  
	    ...
	    for ($iCont3 = 0;$iCont3 < pg_num_rows($rsDetalhaPag); $iCont3++) {
		  ...
		  $oDetalhaPag = db_utils::fieldsMemory($rsDetalhaPag,$iCont3);
			...
			$rsNomeRespPgto = db_query($sSqlCgm);
		        ...
		        $oDadosDetalhaPag->codReduzidoOP                   = $codReduzidoEO;	
			    ...
			    $oDadosDetalhaPag->nomeCredor                      = substr($oDetalhaPag->z01_nome, 0, 120);
			    $oDadosDetalhaPag->vlOP                            = $oDetalhaPag->k17_valor;
			    $oDadosDetalhaPag->especificacaoOP                 = substr($oDetalhaPag->k17_texto, 0, 200);
	    ...
	    $sSql13 = "SELECT k17_situacao,k17_codigo,k17_data,c63_banco,c63_agencia,c63_dvagencia,c63_conta,c63_dvconta,k12_id, e96_codigo, k17_valor,e91_cheque 
      from slip
...
where k17_codigo =  {$oReceitasDesp->k17_codigo}  and c60_anousu = ".db_getsession("DB_anousu");
	    ...
	    for ($iCont4 = 0;$iCont4 < pg_num_rows($rsMovFinanceira); $iCont4++) {
		  ...
		  $nroDocumento = " ";
		  	...
		  	if ( $oMovFinanceira->e96_codigo == '2' ) {
		  	  $tipoDocumentoOP = "01";
		  	  $nroDocumento = $oMovFinanceira->e91_cheque;	
		  	}
			...
			$sHasReg13  = '13';
	   	  	...
	   	  	if (!isset($aDadosAgrupados[$sHash]->Reg13[$sHasReg13])) {
		   	    ...
		   	    $oDadosMovFinanceira->codReduzidoOP                   = $codReduzidoEO;
			    ...
			    $aDadosAgrupados[$sHash]->Reg13[$sHasReg13] = $oDadosMovFinanceira;
	   	  		...
	   	  		$aDadosAgrupados[$sHash]->Reg13[$sHasReg13]->vlDocumento += $oReceitasDesp->k17_valor;
	   	  	}
	...
	//Segundo Reg 10
	/*
	 * SQL PARA EXTRA-ORCAMENTARIA DO TIPO 1 RECEITAS
	 */
	...
	if ( $oReceitasDesp2->k02_tipo == 'E' ) {
        ...
        $sSqlSubTipo = "SELECT c61_reduz
   	    ...
   	    $sDesdobraSubTipo = " ";
   	  	...
   	  	if (substr($oReceitasDesp2->estrutural, 0, 5) == "21881") {
   	  	    ...
   	  	    if (substr($oReceitasDesp2->estrutural, 0, 4) == "45") {
   	  		    ...
   	  		    foreach ($aRepasseCamara as $iRepasseCamara) {
   	  			    ...
   	  			    if ($oSubTipo->codrec == $iRepasseCamara) {
   	  			   	 ...
   	  			   	 $sSubTipo        = $oReceitasDesp2->codrec;
   	  			     $sDesdobraSubTipo = $oReceitasDesp2->codrec;
   	  			   }
   	  	...
   	  	//$sHash = $oReceitasDesp2->codrec.$oReceitasDesp2->k02_codigo.$oReceitasDesp2->k12_numpre;
	        ...
	        $oDadosReceitasDesp2->codUnidadeSub                   = " ";
		    ...
		    $oDadosReceitasDesp2->descExtraOrc                    = substr($oReceitasDesp2->k02_drecei, 0, 50);
		    $oDadosReceitasDesp2->vlLancamento                    = $oReceitasDesp2->valor;
	        $oDadosReceitasDesp2->Reg11 = array();
	   ...
	   // REG 11
   	  	...
   	  	$sSql11 = "SELECT o15_codtri
	                      ...
	                      join conplanoreduz on c61_codcon = c60_codcon and c61_anousu = c60_anousu
                                  join orctiporec on c61_codigo = o15_codigo
	                 where c61_reduz = '". $oReceitasDesp2->codrec."' and c60_anousu = '". db_getsession("DB_anousu") ."'";
  ...
  $sSql3 = "select k17_codigo, k17_dtanu, k17_valor,c60_estrut,k153_slipoperacaotipo,k17_debito,k17_credito,k17_texto
  from  slip 
 ...
 where k17_dtanu between '". $this->sDataInicial ."' and '".$this->sDataFinal. "'";
	...
	//db_criatabela($rsReceitasDesp3);exit;
	for ($iCont = 0;$iCont < pg_num_rows($rsReceitasDesp3); $iCont++) {
	  ...
	  if ($oReceitasDesp3->k153_slipoperacaotipo == "13") {
   	  		...
   	  		foreach ($aINSS as $iINSS) {
   	  			...
   	  			if ($oReceitasDesp3->k17_credito == $iINSS) {
   	  	  ...
   	  	  /**
   	  		...
   	  		foreach ($aRPPS as $iRPPS) {
   	  			...
   	  			if ($oReceitasDesp3->k17_credito == $iRPPS) {
   	  	  ...
   	  	  /**
   	  		...
   	  		foreach ($aIRRF as $iIRRF) {
   	  			...
   	  			if ($oReceitasDesp3->k17_credito == $iIRRF) {
   	  	  ...
   	  	  /**
   	  		...
   	  		foreach ($aISSQN as $iISSQN) {
   	  			...
   	  			if ($oReceitasDesp3->k17_credito == $iISSQN) {
   	  	  ...
   	  	  if (substr($oReceitasDesp3->c60_estrut, 0, 3) == "112") {
   	  	  	...
   	  	  	$sTipoLancamento = "03";
   	  		  $sSubTipo = $oReceitasDesp3->k17_credito;
   	  	    ...
   	  	    if (substr($oReceitasDesp3->c60_estrut, 0, 7) == "3510201") {
   	  		    ...
   	  		    foreach ($aRepasseCamara as $iRepasseCamara) {
   	  			    ...
   	  			    if ($oReceitasDesp3->k17_credito == $iRepasseCamara) {
   	  			     ...
   	  			     $sDesdobraSubTipo = $oReceitasDesp3->k17_credito;
   	  			   }
   	  	...
   	  	$sHash   = $oReceitasDesp3->k17_codigo.substr($oReceitasDesp3->k17_data, 0, 4);
   	  	$sHash  .= $sOrgao."1".$sTipoLancamento.$sSubTipo.$desdobraSubTipo;
		$sHash .= implode(array_reverse(explode("-", $oReceitasDesp3->k17_dtanu)));
	        ...
	        $oDadosReceitasDesp3->codUnidadeSub     = " ";
		    ...
		    $oDadosReceitasDesp3->descExtraOrc                    = substr($oReceitasDesp3->k17_texto, 0, 50);
		    $oDadosReceitasDesp3->vlLancamento                    = $oReceitasDesp3->k17_valor;
	        $oDadosReceitasDesp3->Reg11 = array();
	        
            $aDadosAgrupados[$sHash] = $oDadosReceitasDesp3;
      
   	  	} else {
   	  		$aDadosAgrupados[$sHash]->vlLancamento += $oReceitasDesp3->k17_valor;
   	  	}
    ...
    foreach ($aDadosAgrupados as $oDadosAgrupados2) {
    	...
    	unset($oDadosClone->Reg13);
    	
    	$oDadosClone->vlLancamento = number_format(abs($oDadosClone->vlLancamento), 2, "", "");
    	
    	$this->aDados[] = $oDadosClone;//echo "<pre>";print_r($oDadosAgrupados2);echo "<br><br>";
