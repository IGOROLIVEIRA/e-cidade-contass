...
class MSC {
  ...
  public function gerarLinhas($oRegistro) {
    /*'conta'=>'0', 'po'=>'1', 'fp'=>'2', 'fr'=>'3', 'nr'=>'4', 'nd'=>'5', 'fs'=>'6', 'ai'=>'7'
        ...
        if ($oRegistro[$key-1] > 0) {
            ...
            for ($ii = 1; $ii < 8; $ii++) {
              ...
              if (!empty($oRegistro[$ii]) || $oRegistro[$ii] != 0) {
                  $oNovoResgistro->{$IC} = $oRegistro[$ii];
                  $oNovoResgistro->{$TipoIC} = $aTipoIC[$ii-1];
              }
            }
  ...
  public function getDadosIC($IC, $rsResult) {

      $aDadosIC  = "aDadosIC{$IC}";
      $$aDadosIC = array();
      $sCampos   = array("conta", "po", "fp", "fr", "nr", "nd", "fs", "ai", "saldoinicial", "tipovalor_si", "nat_vlr_si", "debito", "tipovalordeb", "credito", "tipovalorcred", "saldofinal", "tipovalor_sf", "nat_vlr_sf");
      $aIC       = "aIC{$IC}";
      for ($iCont = 0; $iCont < pg_num_rows($rsResult); $iCont++) {
          $oReg = db_utils::fieldsMemory($rsResult, $iCont);
          $sHash = $oReg->{$sCampos[0]}.$oReg->{$sCampos[$IC]};
          if (!isset(${$aDadosIC}[$sHash])) {
              $$aIC = array();
              for ($i = 0; $i < 18; $i++) {
                ${$aIC}[$i] = $oReg->{$sCampos[$i]};
              }
              ${$aDadosIC}[$sHash] = $$aIC;
          } else {
  ...
  public function getDadosIC01($iAno, $DataInicio, $DataFim) {
    ...
    $sSQL = "select * from ( 
      ...
      return $this->getDadosIC(1, $rsResult);
