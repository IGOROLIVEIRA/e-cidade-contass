...
$sql = "SELECT c241_fonte, SUM(c241_valor) valor FROM quadrosuperavitdeficit WHERE c241_valor > 0 AND c241_ano = {$anousu} AND c241_instit IN (" . str_replace('-', ', ', $db_selinstit) . ") GROUP BY c241_fonte ORDER BY c241_fonte";

$result = pg_exec($sql);
$arrayExcesso = array();
for ($i = 0; $i < pg_numrows($result); $i++) {
    db_fieldsmemory($result, $i);
    $c241_fonte = 2 . substr($c241_fonte, 1, 2);
    if (array_key_exists($c241_fonte, $arrayExcesso)) {

            $arrayExcesso[$c241_fonte]["A"] += $valor;
            $arrayExcesso[$c241_fonte]["D"] = 0;
